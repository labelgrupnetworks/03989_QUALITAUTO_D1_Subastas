...
class FgDvc1l extends Model
{
	...
	public function scopeWithBuyerLotsInfo($query)
	{
		...
		->leftJoin('FGHCES1 HCES1', function ($join) {
			$join->on('tl_dvc1l', '=', "'P'")
				->on('HCES1.emp_hces1', '=', 'EMP_DVC1L')
				->on('HCES1.num_hces1', '=', 'numhces_dvc1l')
				->on('HCES1.lin_hces1', '=', 'linhces_dvc1l');
		})
		...
		->when(Config::get('app.locale') != Config::get('app.fallback_locale'), function ($query) {
			...
			return $query
				...
				->selectRaw('NVL(FGSUB_LANG.DES_SUB, FGSUB.DES_SUB) AS DES_SUB')
				->leftJoin('FGHCES1_LANG', "HCES1_LANG.EMP_HCES1_LANG = HCES1.EMP_HCES1 AND HCES1_LANG.NUM_HCES1_LANG =  HCES1.NUM_HCES1 AND HCES1_LANG.LIN_HCES1_LANG = HCES1.LIN_HCES1 AND HCES1_LANG.LANG_HCES1_LANG = '$lang'")
				->leftJoin('FGSUB_LANG', "FGSUB_LANG.EMP_SUB_LANG = FGSUB.EMP_SUB AND FGSUB_LANG.COD_SUB = FGSUB.COD_SUB AND FGSUB_LANG.LANG_SUB_LANG = '$lang'");
		}, function ($query) {
			return $query->addSelect('HCES1.TITULO_HCES1','HCES1.DESC_HCES1', 'HCES1.DESCWEB_HCES1', 'HCES1.WEBFRIEND_HCES1')
