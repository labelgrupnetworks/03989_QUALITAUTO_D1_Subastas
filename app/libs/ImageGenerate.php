    ...
    switch ($size) {
        case 'real':
                if(file_exists($imagenOriginal)){
                    ...
                    if($compresion <  floatval(\Config::get("app.compresion_img",'0.29')))
                    {
                      $comprimir = false;
                    }
                    $width_size = $imgWidth;
...
private function image_type($imagenOriginal)
    ...
    public function generateMini($img, $size){
        ...
        if(file_exists($imageThumb) ){
           ...
           if( strtotime($fechaOriginal) > strtotime($fechaThumb) )
            {       // echo  "image original mas nueva " .date("Y-m-d H:i:s", filectime ($imagenOriginal))."<br> $imageThumb fecha nueva ".date ("Y-m-d H:i:s", filectime ($imageThumb));      die();
                   $generateThumb = true;
            }
            else{
