		...
		foreach ($lang as $key => $value) {
			...
			foreach ($index as $value) {
				...
				if ($key_sec != $value->key_ortsec0) {
					...
					$url_categ = substr(RoutingServiceProvider::translateSeo('subastas'), 4) . $value->key_ortsec0;
					$xml[] = $this->xml($url, $key, $url_categ, $fechaactual, $priority_categ);
				}
				$url_categ = substr(RoutingServiceProvider::translateSeo('subastas'), 4) . $value->key_ortsec0 . '/' . $value->key_sec;
				$xml[] = $this->xml($url, $key, $url_categ, $fechaactual, $priority_categ);
			}
			...
			foreach ($index as $value) {
				$xml[] = $this->xml($url, $key, substr(RoutingServiceProvider::translateSeo('pagina'), 4) . $value->key_web_page, $fechaactual, $priority_page);
			}



			$lote = DB::select(
				"Select FGSUB.cod_sub, auc.*, hces1.ref_hces1,hces1.num_hces1,hces1.webfriend_hces1  from FGSUB "
			...
			foreach ($lote as $key_lote => $value_lote) {
				...
				if (!empty($subasta->getLoteLight())) {
					$url_lote = substr(RoutingServiceProvider::translateSeo('lote'), 4) . $value_lote->cod_sub . "-" . $value_lote->id_auc_sessions . "-" . $value_lote->id_auc_sessions . '/' . $value_lote->ref_hces1 . '-' . $value_lote->num_hces1 . '-' . $value_lote->webfriend_hces1;
