...

{
	...
	public function newXmlUrl()
	{
		...
		foreach ($langs as $lang => $valueLang) {
			...
			foreach ($pages as $page) {
				$xml[] = $this->xml($url, $lang, substr(\Routing::translateSeo('pagina'), 4) . $page->key_web_page, $fechaactual, $priority_page);
			}
			...
			if (config('app.gridLots', '') == "new") {
				...
				$xml[] = $this->xml($url, $lang, substr(\Routing::translateSeo('todas-subastas'), 4), $fechaactual, $priority_lot);
			}

			//grids subastas
			if ($subastas->where('subc_sub', FgSub::SUBC_SUB_ACTIVO)->contains('tipo_sub', FgSub::TIPO_SUB_PRESENCIAL)) {
				$xml[] = $this->xml($url, $lang, substr(\Routing::translateSeo('presenciales'), 4), $fechaactual, $priority_lot);
			}
			if ($subastas->where('subc_sub', FgSub::SUBC_SUB_ACTIVO)->contains('tipo_sub', FgSub::TIPO_SUB_ONLINE)) {
				$xml[] = $this->xml($url, $lang, substr(\Routing::translateSeo('subastas-online'), 4), $fechaactual, $priority_lot);
			}
			if ($subastas->where('subc_sub', FgSub::SUBC_SUB_ACTIVO)->contains('tipo_sub', FgSub::TIPO_SUB_PERMANENTE)) {
				$xml[] = $this->xml($url, $lang, substr(\Routing::translateSeo('subastas-permanentes'), 4), $fechaactual, $priority_lot);
			}
			if ($subastas->where('subc_sub', FgSub::SUBC_SUB_ACTIVO)->contains('tipo_sub', FgSub::TIPO_SUB_VENTA_DIRECTA)) {
				$xml[] = $this->xml($url, $lang, substr(\Routing::translateSeo('venta-directa'), 4), $fechaactual, $priority_lot);
			}
			if ($subastas->contains('subc_sub', FgSub::SUBC_SUB_HISTORICO)) {
				$xml[] = $this->xml($url, $lang, substr(\Routing::translateSeo('subastas-historicas'), 4), $fechaactual, $priority_lot);
			}
			if ($subastas->where('subc_sub', FgSub::SUBC_SUB_HISTORICO)->contains('tipo_sub', FgSub::TIPO_SUB_PERMANENTE)) {
				$xml[] = $this->xml($url, $lang, substr(\Routing::translateSeo('subastas-historicas-presenciales'), 4), $fechaactual, $priority_lot);
			}
			if ($subastas->where('subc_sub', FgSub::SUBC_SUB_HISTORICO)->contains('tipo_sub', FgSub::TIPO_SUB_ONLINE)) {
				$xml[] = $this->xml($url, $lang, substr(\Routing::translateSeo('subastas-historicas-online'), 4), $fechaactual, $priority_lot);
			}
			...
			foreach ($categorias as $categoria) {
				if (config('app.gridLots', '') == "new") {
					...
					$xml[] = $this->xml($url, $lang, substr(\Routing::translateSeo('subastas') . "$categoria->key_ortsec0", 4), $fechaactual, $priority_lot);
				}
			}

			//lotes
			foreach ($lotes as $lote) {
				$url_lote = \Tools::url_lot($lote->sub_asigl0, $lote->id_auc_sessions, $lote->name, $lote->ref_asigl0, $lote->num_hces1, $lote->webfriend_hces1, !empty($lote->descweb_hces1) ? $lote->descweb_hces1 : $lote->titulo_hces1);
    ...
    public function xmlURL(){
       ...
       foreach($lang as $key =>$value){
           $idioma = strtoupper($key);
           \App::setLocale(strtolower($key));
           $sql="SELECT
                ...
                JOIN FGORTSEC1 ORTSEC1 ON ORTSEC1.SUB_ORTSEC1 = ORTSEC0.SUB_ORTSEC0 AND ORTSEC0.LIN_ORTSEC0 = ORTSEC1.LIN_ORTSEC1
                JOIN FXSEC SEC ON SEC.COD_SEC = ORTSEC1.SEC_ORTSEC1 AND SEC.GEMP_SEC = :gemp
                ...
                WHERE ORTSEC0.EMP_ORTSEC0 = :emp AND ORTSEC0.SUB_ORTSEC0 = :cod_sub ORDER BY ORDEN_ORTSEC0";
           ...
           $index = DB::select($sql, $params);

           foreach($index as $value){

                if($key_sec != $value->key_ortsec0){
                    $key_sec = $value->key_ortsec0;
                    $url_categ = substr(\Routing::translateSeo('subastas'),4).$value->key_ortsec0;
                    $xml[]= $this->xml($url,$key,$url_categ,$fechaactual,$priority_categ);
                }
                $url_categ = substr(\Routing::translateSeo('subastas'),4).$value->key_ortsec0.'/'.$value->key_sec;
                $xml[]= $this->xml($url,$key,$url_categ,$fechaactual,$priority_categ);
           }
            ...
            foreach($index as $value){
                $xml[]= $this->xml($url,$key,substr(\Routing::translateSeo('subastas'),4).$value->key_name,$fechaactual,$priority_categ);
            }

            $index = DB::table('WEB_PAGE')
                    ->select('key_web_page')
                    ->where('LANG_WEB_PAGE',$idioma)
                    ->where('EMP_WEB_PAGE',$emp)
                    ->where('WEBNOINDEX_WEB_PAGE','=','0')
                    ->get();

             foreach($index as $value){
                $xml[]= $this->xml($url,$key,substr(\Routing::translateSeo('pagina'),4).$value->key_web_page,$fechaactual,$priority_page);
            }



            $lote = DB::select("Select FGSUB.cod_sub, auc.*, hces1.ref_hces1,hces1.num_hces1,hces1.webfriend_hces1  from FGSUB "
                    . "JOIN FGASIGL0 asigl0 ON FGSUB.cod_sub = asigl0.SUB_ASIGL0 and asigl0.emp_asigl0 = :emp "
                    . "JOIN FGHCES1  hces1 ON (hces1.EMP_HCES1 = :emp AND hces1.NUM_HCES1 = asigl0.NUMHCES_ASIGL0  AND hces1.LIN_HCES1 = asigl0.LINHCES_ASIGL0) "
                    . "JOIN \"auc_sessions\" auc ON FGSUB.cod_sub = auc.\"auction\" and auc.\"company\" = :emp "
                    . "Where FGSUB.tipo_sub in ('O','P') "
                    . "and FGSUB.emp_sub = :emp "
                    . "and FGSUB.subc_sub = 'S' ",
                     array(
                        'emp'       => Config::get('app.emp')
                        )
                    );

            foreach($lote as $key_lote => $value_lote){
                $subasta->cod = $value_lote->cod_sub;
                $subasta->lote = $value_lote->ref_hces1;
                if(!empty($subasta->getLoteLight())){
                    $url_lote = substr(\Routing::translateSeo('lote'),4).$value_lote->cod_sub."-".$value_lote->id_auc_sessions."-".$value_lote->id_auc_sessions.'/'.$value_lote->ref_hces1.'-'.$value_lote->num_hces1.'-'.$value_lote->webfriend_hces1;
                    $xml[]= $this->xml($url,$key,$url_lote,$fechaactual,$priority_lot);
                }
            }
       ...
       $file_name = $_SERVER['DOCUMENT_ROOT']."/files/sitemap.xml";
    ...
    function xml($url,$key,$key_name,$fechaactual,$priority){
       return $value = "
           <url>
	...
	function xmlWithUrl($url, $fechaactual, $priority){
		return $value = "
			<url>
				 <loc>".$url."</loc>
				 <lastmod>".$fechaactual."</lastmod>
    ...
    public function emailCedeneteAMedida(){
       ...
       /* foreach ($lots as $lot){
        ...
        foreach($lots as $key_user=>$user){
            print_r($user);
            die();
           $inf_user = DB::select("SELECT * FROM FXCLI cl
                            WHERE
                            cl.cod_cli = :cod_cli
                            AND
                            cl.GEMP_CLI = :gemp
...
//marcar como enviado
     ...
     ";
           ...
           $emailOptions['content']->url_button =  Config::get('app.url').\Routing::slug('user/panel/allotments/outstanding');
           $emailOptions['content']->hide_thanks = true;
    ...
    function generateProductFeedFacebook($sql){
        $langs=array('ES' => 'es-ES','EN' => 'en-GB');
            ...
            foreach($langs as $key_lang =>$lang){
                 ...
                 $params = array(
                           'emp'       =>  Config::get('app.emp'),
                           'gemp'       =>  Config::get('app.gemp'),
                           'lang'      => $lang //\Tools::getLanguageComplete(Config::get('app.locale'))
                           );
    ...
    function generateProductFeedGoogle($sql){
        $langs=array('ES' => 'es-ES','EN' => 'en-GB');
        ...
        foreach($days_limit as $day_limit){
         ...
         function genXMLProductFeedFacebook($inf_lot){
            ...
            $url_friendly =Config::get('app.url'). \Routing::translateSeo('lote').$inf_lot->cod_sub."-".$inf_lot->id_auc_sessions.'-'.$inf_lot->id_auc_sessions."/".$inf_lot->ref_asigl0.'-'.$inf_lot->num_hces1.'-'.$webfriend;
         ...
         function genXMLProductFeedGoogle($inf_lot){
             $img= Config::get('app.url').'/img/load/lote_medium_large/'.Config::get('app.emp').'-'.$inf_lot->num_hces1.'-'.$inf_lot->lin_hces1.'.jpg';
             $webfriend = !empty($inf_lot->webfriend_hces1)? $inf_lot->webfriend_hces1 :  str_slug($inf_lot->titulo_hces1);
             $url_friendly =Config::get('app.url'). \Routing::translateSeo('lote').$inf_lot->cod_sub."-".$inf_lot->id_auc_sessions.'-'.$inf_lot->id_auc_sessions."/".$inf_lot->ref_asigl0.'-'.$inf_lot->num_hces1.'-'.$webfriend;
             $titulo = !empty($inf_lot->webmetat_hces1)? $inf_lot->webmetat_hces1 : $inf_lot->titulo_hces1;
