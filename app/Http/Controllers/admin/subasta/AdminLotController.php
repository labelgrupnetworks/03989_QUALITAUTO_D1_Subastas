...
class AdminLotController extends Controller
{
	...
	public function index(Request $request, $cod_sub)
	{
		...
		$tableParams = [
			'sub_asigl0' => 0,
			'ref_asigl0' => Config::get('external_id', 1),
			'idorigen_asigl0' => Config::get('external_id', 0),
			'prop_hces1' => 1,
			'descweb_hces1' => 0,
			'artist_name' => 0,
			'impsalhces_asigl0' => 1,
		...
		try {
			...
			if ($request->has('images')) {
				try {
					...
					return redirect(route("$this->parent_name.$this->resource_name.edit", ['cod_sub' => $cod_sub, 'lote' => $request->reflot]))
						->with(['warning' => $th->getMessage(), 'success' => array(trans('admin-app.title.created_ok'))]);
				}
			}

			return redirect(route("$this->parent_name.$this->resource_name.edit", ['cod_sub' => $cod_sub, 'lote' => $request->reflot]))
				->with(['success' => array(trans('admin-app.title.created_ok'))]);
		} catch (\Throwable $th) {
			DB::rollBack();
