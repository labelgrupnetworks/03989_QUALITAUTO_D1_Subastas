...
class AdminBidsController extends Controller
{
	...
	function index(Request $request)
	{
		...
		$pujas = $this->filtersQueryBuilder($request, $pujas)
			->orderby(request('order_bids', 'fec_asigl1'), request('order_bids_dir', 'desc'));
		...
		if($request->input('export', false)){
			...
			return Excel::download(new CollectionExport($pujas->get()), "$fileName.xlsx");
		}
	...
	private function filtersQueryBuilder(Request $request, $bids)
	{
		$bids->when($request->sub_asigl1, function ($query, $sub_asigl1) {
