...
<div class="tr_user_info">
    ...
    <div class="prices border p-1 mb-2">
        ...
        <div class="tuorden block-price">
            <p>{{ trans("$theme-app.sheet_tr.your_actual_order") }}</p>
            @if ($currencySimbol == "$")
                {{ $currencySimbol }}
            @endif
            <span id="tuorden">
            ...
            @if (Session::has('user') && !empty(Config::get('app.DeleteOrdersAnyTime')))
                <input id="cancelarOrdenUser" type="button" value="{{ trans("$theme-app.user_panel.delete_orden") }}"
                    @class([
                        'btn',
    ...
    <div class="pujar">
        ...
        @else
            <button class="btn btn-lb-secondary w-100 mb-2" onclick="clickLogin()">
				<h4>{{ trans("$theme-app.msg_error.login_required") }}</h4>
				<h4 class="fw-bold">{{ trans("$theme-app.msg_error.log_in") }}</h4>
            </button>
        @endif
        ...
        <div class="input-puja input-group mb-2">
            ...
            @if (Session::has('user'))
                <button class="add_bid btn btn-lb-primary">
                    <i class="fa fa-gavel"></i> {{ trans("$theme-app.sheet_tr.place_bid") }}
                </button>
                <input id="tiempo_real" type="hidden" value="1" readonly>
            @else
                <button class="btn button btn-lb-primary" onclick="initSesion();"><i class="fa fa-gavel"></i>
                    {{ trans("$theme-app.sheet_tr.place_bid") }}</button>
            @endif
        </div>
