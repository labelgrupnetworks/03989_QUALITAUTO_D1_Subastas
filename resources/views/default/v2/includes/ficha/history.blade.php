...
<input id="trans_lot_i" type="hidden" value="{{ trans('web.lot.I') }}">
<input id="trans_lot_puja_automatica" type="hidden" value="{{ trans('web.lot.puja_automatica') }}">

@if ($lote_actual->inversa_sub=='S')
	<input id="trans_minimal_price" type="hidden" value="{{ trans('web.lot.maximal-price') }}">
@else
	<input id="trans_minimal_price" type="hidden" value="{{ trans('web.lot.minimal-price') }}">
@endif

<section id="historial_pujas" @class(['hidden' => $num_pujas == 0])>

	<h5 class="hist_title">
		{{ trans('web.lot.history') }} (<span id="num_pujas" class="num_pujas"></span> {{ trim(trans('web.lot.bidding')) }})
	</h5>
	<div class="hist card">
	...
	<div id="view_more" class="more more-historic-bids hidden">
		<a title="ver todas" data-toggle="collapse" data-target="#pujas-collapse" href="javascript:view_all_bids();">
			<span id="view_more_text">{{ trans('web.lot.see-all') }} </span>
			<span id="hide_bids_text" class="hidden">{{ trans('web.lot.hidden') }} </span> <i class="fa fa-angle-down"></i>
		</a>
	</div>
</section>
