...
<?php } ?>
@if( \Config::get("app.PropietarioTR"))
	<?php if(!empty($data['js_item']['user']['is_gestor'])){ ?>
		var cedente = {
		<?php foreach ($data['cedentes'] as $key => $value) : ?>
				'<?php echo $key; ?>': '<?php echo str_replace("'", "",$value);?>',
		<?php endforeach; ?>
		};
    ...
    <div style="margin-top:20px; background:white" class="global-content">
            ...
            <div id="clock" style="display: block">
                ...
                <div style="margin-top: 50px">
                 ...
                 <div data-countdown="{{strtotime($tiempo) - getdate()[0] }}"  data-format="%D {{trans("web.msg_neutral.days")}} <br> %H:%M:%S  {{trans("web.msg_neutral.hours")}}" data-txtend ="{{trans("web.msg_neutral.auction_coming_soon")}}" class="tiempo wait-time text-center"></div>
            ...
            <div class="botonclock">
                <button class="btn btn-primary btn-lg start" data-to="iniciar_subasta"  start='1'>{{ trans('web.sheet_tr.start_auction') }}</button>
            </div>
            @endif
            ...
            <div class="col-lg-8 col-md-8 ">
                <div class="fondo1 aside" style="margin-top:0;">
                    ...
                    <div class="col-lg-6" style="border-right:0;">
                        ...
                        <div class="" id="main_lot_box" style="position:relative;">
                            <div id="main_image_box">
								...
								@if( \Config::get("app.PropietarioTR"))
									<p id="inf_prop">
										<span class="inf_cod_prop">
											{{$data['subasta_info']->lote_actual->prop_hces1}}
										</span>
										-
										<span class="inf_name_prop">

											@if(array_key_exists($data['subasta_info']->lote_actual->prop_hces1,$data['cedentes']))
												<?=  $data['cedentes'][$data['subasta_info']->lote_actual->prop_hces1] ?>
											@endif
										</span>
									</p>
								@endif
					...
					<div class="col-lg-6" style="border-left:0;">
						...
						<span id="lote_actual_main" class=""
								style="display:block">{{ trans('web.sheet_tr.lot') }} <strong><span
										id="info_lot_actual">
						...
						<div class="scrollable" >
								...
								@if(!empty($data['text_lang'][$data['js_item']['lang_code']]->titulo_hces1) && config('app.tr_priority_to_title', true))
								<span id="actual_titulo" style="font-size: 20px;" class="@if(config('app.tr_hidden_title', false)) hidden @endif">
									{!! $data['text_lang'][$data['js_item']['lang_code']]->titulo_hces1 !!}
								</span>
								@else
								<span id="actual_descweb" style="font-size: 20px;" class="@if(config('app.tr_hidden_title', false)) hidden @endif">
									{!! $data['text_lang'][$data['js_item']['lang_code']]->descweb_hces1 !!}
								</span>
								@endif
							...
							<div class="autor_tr" > <strong>	{{ trans('web.sheet_tr.author') }}:</strong> <span id="autor_JS">{{ $data['subasta_info']->lote_actual->autor }} </span></div>
							...
							<div class=" descripcion started hidden">
								...
								<div id="tusOrdenes" class="tusOrdenes hidden-sm hidden-md hidden-lg">
									<p class="yourBid">{{ trans('web.sheet_tr.your_actual_bid') }}: </p>
									<p class="yourOrder">{{ trans('web.sheet_tr.your_actual_order') }}: </p>
								</div>
								...
								<div id="actual_descripcion" style="" class="@if(config('app.tr_hidden_description', false)) hidden @endif">
                            ...
                            <?php /* quitar preico estimado
                             <div id="precioestimado" class="text-center" style=" padding-top: 10px;">
                                <p>
                                    <strong>{{ trans('web.subastas.estimate') }}:</strong>
                            ...
                            */ ?>
                            <div id="precioSalida" class="precioSalida salida text-center">
                                <p>
                                    <strong>{{ trans('web.sheet_tr.start_price') }}:</strong>
							...
							<div id="precioReserva" class="precioReserve salida text-center">
                                <p>
                                    <strong>{{ trans('web.sheet_tr.reserve_price') }}:</strong>
									@if($data['js_item']['subasta']['currency']->symbol == "$")
							...
							{{-- reducir en el css de tiempo real del cliente el tamaño del texto y el padding para que quepa bien   #ficha .salida p {	font-size: 24px;  }  #ficha .salida {	padding: 0px;} --}}
							@if (\Config::get('app.tr_cost_price'))
								<div id="precioCoste" class="salida text-center">
									<p>
										<strong>{{ trans('web.sheet_tr.cost_price') }}:</strong>
										@if($data['js_item']['subasta']['currency']->symbol == "$")
											{{ $data['js_item']['subasta']['currency']->symbol }}<span>{{ $data['subasta_info']->lote_actual->pc_hces1 }}</span>

										@else
											<span>{{ $data['subasta_info']->lote_actual->pc_hces1  }}</span> {{ $data['js_item']['subasta']['currency']->symbol }}
										@endif
									</p>
								</div>
							@endif
                            ...
                            <div class="pactual salida text-center">
                                <p>
                                    ...
                                    @if(Session::has('user') && $data['js_item']['user']['is_gestor'])
                                        ...
                                        <span id="cancelarPuja" >{{ trans('web.sheet_tr.cancel_bid') }}</span>
                                        <span id="cancelarOrden" >{{ trans('web.sheet_tr.cancel_order') }}</span>

                                    @endif
                                    @if(\Config::get('app.tr_show_canel_bid_client') && Session::has('user') && !$data['js_item']['user']['is_gestor'])

                                    <span id="cancelarPujaUser" class="@if (!empty($data['js_item']['user']) && !empty($data['subasta_info']->lote_actual->max_puja) &&  $data['subasta_info']->lote_actual->max_puja->cod_licit == $data['js_item']['user']['cod_licit'])  @else hidden  @endif" >{{ trans('web.sheet_tr.cancel_bid') }}</span>
                                    @endif
                                </p>
                            ...
                            @if(Session::has('user') && !$data['js_item']['user']['is_gestor'])
                                <div class="col-lg-6">{{ trans('web.sheet_tr.your_actual_bid') }}: <span id="tupuja"><?php if (!empty($data['js_item']['user']['maxPuja']))  { echo $data['js_item']['user']['maxPuja']->formatted_imp_asigl1; } ?></span></div>

                                <div class="col-lg-6">{{ trans('web.sheet_tr.your_actual_order') }}: <span id="tuorden"><?php if (!empty($data['js_item']['user']['maxOrden']))  { echo $data['js_item']['user']['maxOrden']->himp_orlic; } ?></span></div>
                            @endif
                                ...
                                <div class="col-lg-12">
                                    <div class="col-lg-6">
                                        ...
                                        <div class="noti">
                                            ...
                                            <div class="row">
                                                <div class="col-lg-6">
                                                    ...
                                                    <div class="gestor_radios">
                                                        <label class="radio">
                                                            <input checked="checked" type="radio" name="puja_opts" value="normal"> {{ trans('web.sheet_tr.order_bid') }}
                                                        </label>
                                                        <label class="radio">
                                                            <input type="radio" name="puja_opts" value="firme"> {{ trans('web.sheet_tr.direct_bid') }}
                                                        </label>
                                                    </div>
                                            ...
                                            <div class="col-lg-6">
                                                <a class="add_bid btn btn-success btn-custom-save"><i class="fa fa-gavel"></i> {{ trans('web.sheet_tr.place_bid') }}</a>
            ...
            <div class="col-sm-4 col-lg-4 fondo2">
                ...
                <div class="started hidden">
                    <div class="aside pujas">
                        ...
                        <h2>{{ trans('web.sheet_tr.last_bids') }}</h2>
                            <div id="pujas_list">
                                ...
                                ?>
                                    <div class="pujas_model col-xs-12">
                                        <div class="col-lg-6 tipoPuja">
											...
											<p data-type="R" class=" trBidReserve @if ($puja->pujrep_asigl1 != 'R') hidden @endif " ><i class="fa fa-desktop" aria-hidden="true"></i> {{ trans('web.sheet_tr.bid-reserve') }}</p>

										</div>
                                        <div class="col-lg-6 importePuja">
                                            <p>
											@if($data['js_item']['subasta']['currency']->symbol == "$")
												<span>{{ $data['js_item']['subasta']['currency']->symbol }}</span><span>{{ $puja->formatted_imp_asigl1 }}</span>
											@else
                            ...
                            <div class="pujas_model hidden col-xs-12" id="type_bid_model">
                                <div class="col-lg-6 tipoPuja">
									...
									<p data-type="R" class=" trBidReserve " ><i class="fa fa-desktop" aria-hidden="true"></i> {{ trans('web.sheet_tr.bid-reserve') }}</p>

									</div>
                                <div class="col-lg-6 importePuja">
                                    <p>
										@if($data['js_item']['subasta']['currency']->symbol == "$")
											<span>{{ $data['js_item']['subasta']['currency']->symbol }}</span><span class="puj_imp"></span>
										@else
            ...
            <div class="col-sm-4 col-lg-4 started hidden">
                <div class="aside adjudicaciones">
                    <h2>{{ trans('web.sheet_tr.your_adjudications') }}</h2>
                    <div id="adjudicaciones_list">
                            ...
                            <?php foreach ($data['js_item']['user']['adjudicaciones'] as $key => $val): ?>
                                <div class="adjudicaciones_model">
                                    <div class="col-lg-6 adj_ref">
                                        <p>{{ trans('web.sheet_tr.lot') }}</i> <span>{{ $val->ref_asigl1 }}</span></p>
                                    </div>
                                    <div class="col-lg-6">
                                        <p>
											@if($data['js_item']['subasta']['currency']->symbol == "$")
											<span>{{ $data['js_item']['subasta']['currency']->symbol }}</span><span class="adj_imp">{{ $val->imp_asigl1 }}</span>
											@else
												<span class="adj_imp">{{ $val->imp_asigl1 }}</span> <span>{{ $data['js_item']['subasta']['currency']->symbol }}</span>
											@endif
										</p>
                                    </div>
                                </div>
                        ...
                        <div class="adjudicaciones_model hidden" id="type_adj_model">
                                <div class="col-lg-6 adj_ref">
                                    <p>{{ trans('web.sheet_tr.lot') }}</i> <span></span></p>
                                </div>
                                <div class="col-lg-6">
                                    <p>
										@if($data['js_item']['subasta']['currency']->symbol == "$")
											<span>{{ $data['js_item']['subasta']['currency']->symbol }}</span><span class="adj_imp"></span>
										@else
            ...
            <div class="col-lg-4 started hidden">
                <div class="aside ol">
                    ...
                    <h2>{{ trans('web.sheet_tr.orders') }}</h2>
                        <div id="ol_list">
                        ...
                        <?php foreach ($data['subasta_info']->lote_actual->ordenes as $orden) : ?>
                                ...
                                <div class="ol_model">
                                    <div class="col-lg-6 tipoOrden">
                                        <p data-type="I" @if ($orden->tipop_orlic != 'I')class="hidden" @endif><i class="fa fa-globe" aria-hidden="true"></i> {{ trans('web.sheet_tr.bid-internacional') }}</p>
                                        <p data-type="W" @if ($orden->tipop_orlic != 'W')class="hidden" @endif ><i class="fa fa-globe" aria-hidden="true"></i> {{ trans('web.sheet_tr.bid-web') }}</p>
                                        <p data-type="S" @if ($orden->tipop_orlic != 'S')class="hidden" @endif><i class="fa fa-hand-paper-o" aria-hidden="true"></i> {{ trans('web.sheet_tr.bid-sala') }}</p>
                                        <p data-type="X" @if ($orden->tipop_orlic != 'X')class="hidden" @endif><i class="fa fa-phone" aria-hidden="true"></i> {{ trans('web.sheet_tr.bid-telf') }} Web</p>
                                        <p data-type="T" @if ($orden->tipop_orlic != 'T')class="hidden" @endif><i class="fa fa-phone" aria-hidden="true"></i> {{ trans('web.sheet_tr.bid-telf') }}</p>
                                        <p data-type="E" @if ($orden->tipop_orlic != 'E' && $orden->tipop_orlic != 'P')class="hidden" @endif><i class="fa fa-desktop" aria-hidden="true"></i> {{ trans('web.sheet_tr.books_bid') }}</p>
                                        <p data-type="O" @if ($orden->tipop_orlic != 'O')class="hidden" @endif><i class="fa fa-desktop" aria-hidden="true"></i> {{ trans('web.sheet_tr.books_bid') }}</p>
										<p data-type="U" @if ($orden->tipop_orlic != 'U')class="hidden" @endif><i class="fab fa-stripe-s" aria-hidden="true"></i> SUBALIA</p>
                                        <p data-type="R" class=" trBidReserve @if ($orden->tipop_orlic != 'R') hidden @endif " ><i class="fa fa-desktop" aria-hidden="true"></i> {{ trans('web.sheet_tr.bid-reserve') }}</p>
                                 </div>

                                    <div class="col-lg-6 importeOrden">
                                        <p>
											@if($data['js_item']['subasta']['currency']->symbol == "$")
												<span>{{ $data['js_item']['subasta']['currency']->symbol }}</span><span class="puj_imp_order">{{ \Tools::moneyFormat($orden->himp_orlic) }}</span>
                                            @else
                        ...
                        <div class="ol_model hidden" id="type_bid_model_order">
                            <div class="col-lg-6 tipoOrden">
                                <p data-type="I"><i class="fa fa-globe" aria-hidden="true"></i> {{ trans('web.sheet_tr.bid-internacional') }}</p>
                                <p data-type="W"><i class="fa fa-globe" aria-hidden="true"></i> {{ trans('web.sheet_tr.bid-web') }}</p>
                                <p data-type="S" class="hidden"><i class="fa fa-hand-paper-o" aria-hidden="true"></i> {{ trans('web.sheet_tr.bid-sala') }}</p>
                                <p data-type="X" class="hidden"><i class="fa fa-phone" aria-hidden="true"></i> {{ trans('web.sheet_tr.bid-telf') }} Web</p>
                                <p data-type="T" class="hidden"><i class="fa fa-phone" aria-hidden="true"></i> {{ trans('web.sheet_tr.bid-telf') }}</p>
                                <p data-type="E" class="hidden"><i class="fa fa-desktop" aria-hidden="true"></i> {{ trans('web.sheet_tr.books_bid') }}</p>
                                <p data-type="P" class="hidden"><i class="fa fa-desktop" aria-hidden="true"></i> {{ trans('web.sheet_tr.books_bid') }}</p>
								<p data-type="O" class="hidden"><i class="fa fa-desktop" aria-hidden="true"></i> {{ trans('web.sheet_tr.books_bid') }}</p>
     ...
     <!-- mensajes en sala chat -->
            ...
            @if (\Config::get('app.tr_show_chat') and Session::has('user'))
                @include('content.tr.tiempo_real_admin.msg_sala')
            @endif
            </div>
            ...
            <div class="col-sm-12 col-lg-6 hidden started">
                @if (\Config::get('app.tr_show_buscador') and Session::has('user'))
                    @include('content.tr.tiempo_real_admin.buscador')
                @endif
            </div>
    ...
    @if(!empty($data['js_item']['user']['is_gestor']))
       <div id="controles_gestor_box">
           <div class="gestor_buttons">
               <p>{{ trans('web.sheet_tr.user_conectet') }} <span id="users_conectet"></span></p>
                <button class="change_end_lot btn" data-status="end" type="button">{{ trans('web.sheet_tr.end_lot') }}</button>

				<button class="change_end_lot btn hidden" data-status="cancel" type="button">{{ trans('web.sheet_tr.cancel_end_lot') }}</button>

                <button class="change_auction_status btn @if ($data['js_item']['subasta']['status'] == 'stopped') hidden @endif" data-status="stopped" class="btn" type="button">{{ trans('web.sheet_tr.stop_auction') }}</button>
                <button class="change_auction_status btn @if ($data['js_item']['subasta']['status'] == 'stopped') hidden @endif" data-status="stopped-time" class="btn" type="button">{{ trans('web.sheet_tr.put_off_auction') }}</button>
                <button class="change_auction_status btn @if ($data['js_item']['subasta']['status'] == 'in_progress') hidden @endif" data-status="in_progress" class="btn" type="button">{{ trans('web.sheet_tr.restart_lot') }}</button>

                <button id="msg_predef" class="btn" type="button">{{ trans('web.sheet_tr.msg_predef') }}</button>
                <button id="show_stopped_lots" class="btn" type="button">{{ trans('web.sheet_tr.show_stopped_lots') }}</button>
                <button id="show_stopped_lots_disabled" class="btn hidden" style="background:red" type="button">{{ trans('web.sheet_tr.show_stopped_lots') }}</button>
                <button id="jump_to_lots" class="btn" type="button">{{ trans('web.sheet_tr.jump_to_lots') }}</button>
                <button id="jump_to_lots_disabled" class="btn hidden" style="background:red" type="button">{{ trans('web.sheet_tr.jump_to_lots') }}</button>
                <button id="baja_client" class="btn" type="button">{{ trans('web.sheet_tr.baja_client') }}</button>
                <button id="baja_client_disabled" class="btn hidden" style="background:red" type="button">{{ trans('web.sheet_tr.baja_client') }}</button>
                ...
                <div>
                    <button id="confirm_automatic_auction" style="background: #337ab7;    display: inline-block;    width: 170px;" class="btn" type="button">{{ trans('web.sheet_tr.automatic_auction') }}</button>
