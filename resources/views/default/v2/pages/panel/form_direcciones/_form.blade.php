...
<div class="row">
	...
	<div class="col-xs-12 col-md-6 mb-2">
		...
		@else
		<label for="nombre">{{ trans(\Config::get('app.theme').'-app.login_register.contact') }}</label>
		@endif
	...
	@if ($fxcli->fisjur_cli == 'J')
		<div class="col-xs-12 col-md-6 mb-2">
			<label for="nombre">{{ trans(\Config::get('app.theme').'-app.login_register.company') }}</label>
...
<div class="row">
	<div class="col-xs-12">
		<button type="button" class="btn btn-color fav-address" cod="{{$address->codd_clid}}">{{ trans(\Config::get('app.theme').'-app.user_panel.define_default') }}</button>

		<button type="button" class="btn btn-color delete-address" cod="{{$address->codd_clid}}">{{ trans(\Config::get('app.theme').'-app.user_panel.delete') }}</button>
	</div>
</div>
@endif
...
<div class="row">
	<div class="col-xs-12">
		<button type="submit" class="btn btn-color" cod="">{{ trans(\Config::get('app.theme').'-app.user_panel.save') }}</button>
	</div>
</div>
@endif
