@extends('layouts.default')

@section('title')
	{{ trans('web.head.title_app') }}
@stop

@section('content')

	<div class="color-letter">
		<div class="container">
			<div class="row">
				<div class="col-xs-12 text-center">
					<h1 class="titlePage">{{ trans('web.user_panel.mi_cuenta') }}</h1>
				</div>
			</div>
		</div>
	</div>

	<div class="account-user color-letter  panel-user">
		<div class="container">
			<div class="row">
				<div class="col-xs-12 col-md-3 col-lg-3 account-user-menu">
					<?php $tab = 'favorites'; ?>
					@include('pages.panel.menu_micuenta')
				</div>
				<div class="col-xs-12 col-md-9 col-lg-9 ">
					<div class="user-account-title-content">
						<div class="user-account-menu-title">
							{{ trans('web.user_panel.favorites') }}</div>
					</div>
					<div class="col-xs-12 no-padding ">
						<div class="panel-group" id="accordion">
							<div class="panel panel-default">

								@foreach ($data['favoritos'] as $key_sub => $all_inf)
									<div class="panel-heading">
										<div class="panel-title">
											<a class="d-flex justify-content-space-between" data-toggle="collapse"
												href="#{{ $all_inf['inf']['cod_sub'] }}">
												<span class="title-sub-list">{{ $all_inf['inf']['des_sub'] }}</span>
												<img width=10
													src="data:image/svg+xml;utf8;base64," />
											</a>
										</div>
										<div id="{{ $all_inf['inf']['cod_sub'] }}"
											class="panel-collapse collapse <?= count($data['favoritos']) == '1' ? 'in' : ' ' ?>">

											<div class="user-account-heading hidden-xs d-flex align-items-center justify-content-space-between">
												<div class="col-xs-12 col-sm-7 col-lg-8 col-one user-account-item">
													{{ trans('web.user_panel.lot') }}
												</div>
												<div class="col-xs-12 col-sm-2 col-one user-account-fecha">
													{{ trans('web.lot.puja_actual') }}
												</div>
												<div class="col-xs-12 col-sm-3 col-lg-2 col-one user-account-max-bid">

												</div>
											</div>
											<div class="user-accout-items-content">
												@foreach ($all_inf['lotes'] as $inf_lot)
													<div class="user-accout-item-wrapper  col-xs-12 no-padding">
														<div class="d-flex">
															<div class="col-xs-12 col-sm-7 col-lg-8 col-one user-account-item ">
																<a href='{{ $inf_lot->url_lot }}'>
																	<div class="col-xs-12 col-sm-3 no-padding ">
																		<img src="{{ $inf_lot->url_img }}" class="img-responsive">
																	</div>
																	<div class="col-xs-12 col-sm-9 no-padding">
																		@if (strtoupper($inf_lot->tipo_sub) == 'O' || strtoupper($inf_lot->tipo_sub) == 'P')
																			<div class="user-account-item-auction text-right">
																				<small>{{ trans('web.user_panel.auctions_online') }}</small>
																			</div>
																		@endif
																		<div class="user-account-item-title">
																			{{ $inf_lot->titulo_hces1 }}</div>

																		<div class="user-account-item-lot">
																			<span>{{ trans('web.user_panel.lot') }}

																				@php
																					$refLot = $inf_lot->ref_asigl0;
																					#si  tiene el . decimal hay que ver si se debe separar
																					if (strpos($refLot, '.') !== false) {
																					    $refLot = str_replace(['.1', '.2', '.3', '.4', '.5'], ['-A', '-B', '-C', '-D', '-E'], $refLot);

																					    #si hay que recortar
																					} elseif (\config::get('app.substrRef')) {
																					    #cogemos solo los últimos x numeros, ya que se usaran hasta 9, los  primeros para diferenciar un lote cuando se ha vuelto a subir a subasta
																					    #le sumamos 0 para convertirlo en numero y así eliminamos los 0 a la izquierda
																					    $refLot = substr($refLot, -\config::get('app.substrRef')) + 0;
																					}
																				@endphp
																				{{ $refLot }}


																			</span>
																		</div>
																		<div class="user-account-item-text">
																			<div>{{ $inf_lot->cod_sub }}</div>
																		</div>
																	</div>
																</a>
															</div>
															<div class="col-xs-12 col-sm-2 col-lg-2 account-item-border">
																<div class="user-account-item-price d-flex align-items-center">
																	<div class="visible-xs">
																		{{ trans('web.lot.puja_actual') }}
																	</div>
																	<div
																		@if (!empty($inf_lot->implic_hces1) && !empty($inf_lot->cod_licit) && $inf_lot->cod_licit == $inf_lot->licit_ganador) class="mine"
															@else
															class="other" @endif>
																		<?= empty($inf_lot->implic_hces1) ? '-' : \Tools::moneyFormat($inf_lot->implic_hces1) . ' ' . trans('web.subastas.euros') ?>
																	</div>
																	{{-- <div @if (!empty($inf_lot->pujas[0]) && !empty($data['codigos_licitador'][$inf_lot->cod_sub]) && $data['codigos_licitador'][$inf_lot->cod_sub] == $inf_lot->pujas[0]->cod_licit)
																		class="mine"
																		@else
																		class="other"
																		@endif
																		>< ?= empty($inf_lot->pujas[0]->formatted_imp_asigl1)? '-': $inf_lot->pujas[0]->formatted_imp_asigl1.' '. trans('web.subastas.euros'); ?>
																	</div> --}}
																</div>
															</div>
															<div class="col-xs-12 col-sm-3 col-lg-2 account-item-border">
																<div class="user-account-item-price  d-flex align-items-center">
																	<div><a title="{{ trans('web.lot.del_from_fav') }}" class="d-block"
																			href="javascript:action_fav_lote_new('remove','{{ $inf_lot->ref_asigl0 }}','{{ $inf_lot->id_sub }}')">{{ trans('web.lot.del_from_fav') }}</a>
																	</div>
																	{{-- <div><a title="{{trans('web.lot.del_from_fav')}}"
																			class="d-block"
																			href="javascript:action_fav_lote('remove','{{ $inf_lot->ref_asigl0 }}','{{$inf_lot->cod_sub }}',' < ?= $data['codigos_licitador'][$inf_lot->cod_sub] ?>')">{{trans('web.lot.del_from_fav')}}</a>
																	</div> --}}
																</div>
															</div>
														</div>
													</div>
												@endforeach
											</div>
										</div>
									</div>
								@endforeach



							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>


	<section class="panel-user hide">
		<div class="container panel">
			<div class="row">
				<div class="col-xs-12 col-sm-12">
					<?php $tab = 'favorites'; ?>
					@include('pages.panel.menu_micuenta')
					<div class="panel-group" id="accordion">
						<div class="panel panel-default">
							@foreach ($data['favoritos'] as $key_sub => $all_inf)
								<div class="panel-heading">
									<h4 class="panel-title">
										<a data-toggle="collapse" href="#{{ $all_inf['inf']['cod_sub'] }}">{{ $all_inf['inf']['des_sub'] }}</a>
									</h4>
								</div>
								<div id="{{ $all_inf['inf']['cod_sub'] }}"
									class="panel-collapse collapse <?= count($data['favoritos']) == '1' ? 'in' : ' ' ?>">
									<div class="panel-body">
										<div class="table-responsive">
											<table class="table table-striped table-custom">
												<thead>
													<tr>
													<tr>
														<th> </th>
														<th>{{ trans('web.user_panel.lot') }}</th>
														<th>{{ trans('web.user_panel.auction') }}
														</th>
														<th>{{ trans('web.user_panel.name') }}</th>
														<th>{{ trans('web.user_panel.date') }}</th>
														<th>{{ trans('web.user_panel.mi_puja') }}
														</th>
													</tr>
													</tr>
												</thead>
												<tbody>
													@foreach ($all_inf['lotes'] as $inf_lot)
														<?php
														$url_friendly = str_slug($inf_lot->titulo_hces1);
														$url_friendly = \Routing::translateSeo('lote') . $inf_lot->cod_sub . '-' . str_slug($inf_lot->name) . '-' . $inf_lot->id_auc_sessions . '/' . $inf_lot->ref_asigl0 . '-' . $inf_lot->num_hces1 . '-' . $url_friendly;
														?>

														<tr class='{{ $inf_lot->ref_asigl0 }}-{{ $inf_lot->cod_sub }}'>
															<td><img src="{{ \Tools::url_img('lote_small', $inf_lot->num_hces1, $inf_lot->lin_hces1) }}"
																	height="42" width="42"></td>
															<td>{{ $inf_lot->ref_asigl0 }}</td>
															@if (strtoupper($inf_lot->tipo_sub) == 'O' || strtoupper($inf_lot->tipo_sub) == 'P')
																<td>{{ trans('web.user_panel.auctions_online') }}
																</td>
															@else
																<td>{{ $inf_lot->cod_sub }}</td>
															@endif
															<td>{{ $inf_lot->titulo_hces1 }}</td>
															<td>
																<?= empty($inf_lot->implic_hces1) ? '-' : \Tools::moneyFormat($inf_lot->implic_hces1) . ' ' . trans('web.subastas.euros') ?>
															</td>
															<td><a title="{{ trans('web.lot.del_from_fav') }}" class="btn btn-del"
																	href="javascript:action_fav_lote_new('remove','{{ $inf_lot->ref_asigl0 }}','{{ $inf_lot->id_sub }}')">{{ trans('web.lot.del_from_fav') }}</a>
															</td>
														</tr>
													@endforeach
												</tbody>
											</table>
										</div>
									</div>
								</div>
							@endforeach
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>
@stop
