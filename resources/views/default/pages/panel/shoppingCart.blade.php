        ...
        <div class="row">
			...
			@endif
				...
				<div class="user-account-title-content mt-2">
					<div class="user-account-menu-title">{{ trans(\Config::get('app.theme').'-app.shopping_cart.myCart') }}: 	{{head($auction)->des_sub}} </div>
								...
								<div class="adj color-letter  align-items-center justify-content-space-between">
									...
									@if( !empty(Config::get('app.web_gastos_envio')) )
										<div class="col-xs-12 col-sm-3 gastos_envio" >
											...
											<div id="envioPosible_carrito_{{$codSub}}_js" <?= ($gastosEnvio== "-1")? 'class="hidden"' :''   ?>>
												...
												<label for="envio_agencia_carrito_{{$codSub}}_js"> {{ trans(\Config::get('app.theme').'-app.user_panel.gastos_envio') }}:  <span id="coste-envio-carrito_{{$codSub}}_js"> {{ \Tools::moneyFormat($gastosEnvio,false,2)}} </span> {{ trans(\Config::get('app.theme').'-app.lot.eur') }}</label>
												...
												@if (!empty(Config::get("app.porcentaje_seguro_envio")))
													...
													<label for="seguro_carrito_{{$codSub}}_js"> {{ trans(\Config::get('app.theme').'-app.user_panel.seguro_envio') }}:  <span id="coste-seguro-carrito_js"> {{ \Tools::moneyFormat($totalSeguro[$codSub],false,2)}} </span> {{ trans(\Config::get('app.theme').'-app.lot.eur') }}</label>
											...
											<br>
											<input type="radio" class=" change_envio_carrito_js" <?= ($gastosEnvio== "-1")? 'checked="checked"' :''   ?> data-sub="{{$codSub}}" id="recogida_almacen_carrito_js" name="envio_carrito" value="0"> <label  for="recogida_almacen">{{ trans(\Config::get('app.theme').'-app.user_panel.sala_almacen') }}</label>
									...
									<div class="col-xs-12 col-sm-4 total-price" >
										...
										@if( !empty(Config::get('app.SeguroCarrito')) )
											...
											<br><br>
										@endif
										{{ trans(\Config::get('app.theme').'-app.shopping_cart.total_articles') }} <br>{{ \Tools::moneyFormat($totalLotes[$codSub], trans(\Config::get('app.theme').'-app.subastas.euros'),2)}}

										<br>	<br><br>

										{{ trans(\Config::get('app.theme').'-app.shopping_cart.total_pay') }} <br><span class="precio_final_carrito_{{$codSub}}">{{ \Tools::moneyFormat($pagar, trans(\Config::get('app.theme').'-app.subastas.euros'), 2)}}</span>
									...
									<div class="col-xs-12" >
										...
										@endif
										<button style="float:right" type="button" class="secondary-button   submitShoppingCart_JS "  cod_sub="{{$codSub}}" class="btn btn-step-reg" >{{ trans(\Config::get('app.theme').'-app.user_panel.pay') }}</button>
										<br><br>
									</div>
