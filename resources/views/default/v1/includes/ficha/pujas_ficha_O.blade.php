    ...
    <div class="col-xs-12 no-padding info-ficha-buy-info-price d-flex">
            ...
            <div class="pre">
				@if ($lote_actual->ocultarps_asigl0 != 'S')
					...
					<p class="pre-price">{{$lote_actual->formatted_impsalhces_asigl0}} {{ trans($theme.'-app.subastas.euros') }}
					@if(\Config::get("app.exchange"))
            ...
            <div class="pre">
                ...
                <p class="pre-price">{{ \Tools::moneyFormat($lote_actual->imptash_asigl0)}} {{ trans($theme.'-app.subastas.euros') }}
    ...
    <div class="col-xs-12 no-padding info-ficha-buy-info-price border-top-bottom">
        <div class="pre d-flex mt-2 mb-2 ">
            ...
            <div class="col-xs-12 col-sm-9 no-padding">
				@if ($hay_pujas)
					<p class='explanation_bid t_insert pre-title' >{{ trans($theme.'-app.lot.next_min_bid') }}  </p>
				@else
					<p class='explanation_bid t_insert pre-title'>{{ trans($theme.'-app.lot.min_puja') }}  </p>
				@endif
				<strong><span class="siguiente_puja"> </span>{{ trans($theme.'-app.subastas.euros') }}
					@if(\Config::get("app.exchange"))
				...
				<div class="input-group d-block group-pujar-custom ">
					...
					<div class="d-flex mb-2">
						...
						@if(1 == 2 && Session::has('user') && !empty(\Config::get("app.DeleteOrdersAnyTime")))
							<button style="width:100%" id="cancelarOrdenUser"   class="ficha-btn-bid-height button-principal  @if(empty($data['js_item']['user']['ordenMaxima']))  hidden @endif" type="button" ref="{{$data['subasta_info']->lote_actual->ref_asigl0}}" sub="{{$data['subasta_info']->lote_actual->cod_sub}}" >  {{ trans($theme.'-app.user_panel.delete_orden') }}
							</button>
						@endif
