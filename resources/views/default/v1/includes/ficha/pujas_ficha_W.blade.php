...
<div class=" col-xs-12 no-padding info-ficha-buy-info-price d-flex">
    <div class="pre">
        @if ($subasta_web && $subasta_abierta_O && $cerrado_N  )
            ...
            <div  id="text_actual_no_bid" class="pre <?=  $lote_actual->open_price >0? 'hidden':'' ?>"> <p class="pre-title-principal">{{ trans($theme.'-app.lot_list.no_bids') }}</p></div>
...
<div class="ficha-info-item-for-pay col-xs-12 no-padding">
    <div class="info_single_content">
        ...
        @if( $cerrado_N && $fact_N &&  $start_orders  &&   !$end_orders && !$subasta_abierta_P)
			...
			@else
				<div class="insert-max-bid"><?=trans($theme.'-app.lot.insert_max_puja_start')?></div>
				<div class="input-group group-pujar-custom d-flex justify-content-space-between">
						...
						<div class="input-group-btn">
							<button id="pujar_ordenes_w" data-from="modal" type="button" class="ficha-btn-bid button-principal" ref="{{ $data['subasta_info']->lote_actual->ref_asigl0 }}" codsub="{{ $data['subasta_info']->lote_actual->cod_sub }}">{{ trans($theme.'-app.lot.place_bid') }}</button>
						</div>
						@if(!empty($lote_actual->ordentel_sub) && $lote_actual->ordentel_sub <= $lote_actual->impsalhces_asigl0)
							<div >
								<button id="pujar_orden_telefonica" data-from="modal" type="button" class="ficha-btn-telephone-bid  button-principal" ref="{{ $data['subasta_info']->lote_actual->ref_asigl0 }}" codsub="{{ $data['subasta_info']->lote_actual->cod_sub }}">{{ trans($theme.'-app.lot.puja_telefonica') }}</button>
