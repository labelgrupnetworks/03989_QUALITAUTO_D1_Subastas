...
<div class="col-xs-12 col-md-4">
    <select class="form-control" id="total_selected" >
		@foreach(\Config::get("app.filter_total_shown_options") as $numLots)
        	<option value="{{$numLots}}" @if (request('total') == $numLots) selected @endif >    {{ trans($theme.'-app.lot_list.see_num_lots',["num" => $numLots]) }}   </option>
		@endforeach
    </select>
</div>
...
<div class="col-xs-12 pt-1 d-flex align-items-center mt-1">
    ...
    <p class="mr-2 mb-0 cantidad-res"  >{{ Tools::numberformat($count_lots) }} {{ trans($theme.'-app.lot_list.results') }}</p>
