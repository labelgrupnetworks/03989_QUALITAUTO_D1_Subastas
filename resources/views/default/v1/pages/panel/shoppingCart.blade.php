        ...
        <div class="row">
			...
			@if(count($auctions)== 1)
                <div class="user-account-title-content">
					...
					<div >{{ trans($theme.'-app.shopping_cart.text_reserve') }}</div>
                </div>
			@endif
				...
				@if(count($auctions)> 1)
				<div class="user-account-title-content mt-2">
					<div class="user-account-menu-title">{{ trans($theme.'-app.shopping_cart.myCart') }}: 	{{head($auction)->des_sub}} </div>

                </div>


					@endif
					<div id="7500" class="table-responsive panel-collapse collaps in  mb-4">
						<form id="pagar_lotes_{{$codSub}}" autocomplete="off">
							...
							?>
								<input type="hidden" id="totalLotes_{{$codSub}}_JS" value="{{$totalLotes[$codSub]}}">
								<input type="hidden" id="gastosEnvio_{{$codSub}}_JS" value="{{$gastosEnvio}}">
								<input type="hidden" id="seguro_{{$codSub}}_JS" value="{{$totalSeguro[$codSub]}}">
								...
								<div class="adj color-letter  align-items-center justify-content-space-between">
									...
									@if( !empty(Config::get('app.web_gastos_envio')) || !empty(Config::get('app.direccion_envio')))
										<div class="col-xs-12 col-sm-5 gastos_envio" >
											<strong> {{ trans($theme.'-app.user_panel.direccion-facturacion') }}</strong>
												<select id="clidd_carrito_{{$codSub}}"  name="clidd_carrito" class="change_address_carrito_js "   data-sub="{{$codSub}}" style="width: 90%;">
									...
									@if( !empty(Config::get('app.web_gastos_envio')) )
										<div class="col-xs-12 col-sm-3 gastos_envio" >
											...
											<div id="envioPosible_carrito_{{$codSub}}_js" <?= ($gastosEnvio== "-1")? 'class="hidden"' :''   ?>>
												<input type="radio" <?= ($gastosEnvio== "-1")? '' :'checked="checked"'   ?>  class=" change_envio_carrito_js" data-sub="{{$codSub}}" id="envio_agencia_carrito_{{$codSub}}_js" name="envio_carrito"  value="1">
												<label for="envio_agencia_carrito_{{$codSub}}_js"> {{ trans($theme.'-app.user_panel.gastos_envio') }}:  <span id="coste-envio-carrito_{{$codSub}}_js"> {{ \Tools::moneyFormat($gastosEnvio,false,2)}} </span> {{ trans($theme.'-app.lot.eur') }}</label>

												@if (!empty(Config::get("app.porcentaje_seguro_envio")))
													<br>
													<input type="checkbox" style="top: 0px;height: 10px;margin-right: 0px;"  class="check_seguro_js" data-sub="{{$codSub}}" id="seguro_carrito_{{$codSub}}_js" name="seguro_carrito"  value="1">
													<label for="seguro_carrito_{{$codSub}}_js"> {{ trans($theme.'-app.user_panel.seguro_envio') }}:  <span id="coste-seguro-carrito_js"> {{ \Tools::moneyFormat($totalSeguro[$codSub],false,2)}} </span> {{ trans($theme.'-app.lot.eur') }}</label>

												@endif

											</div>
											<div id="envioNoDisponible_carrito_{{$codSub}}_js" <?= ($gastosEnvio== "-1")? '' :'class="hidden"'   ?>>
											...
											<br>
											<input type="radio" class=" change_envio_carrito_js" <?= ($gastosEnvio== "-1")? 'checked="checked"' :''   ?> data-sub="{{$codSub}}" id="recogida_almacen_carrito_js" name="envio_carrito" value="0"> <label  for="recogida_almacen">{{ trans($theme.'-app.user_panel.sala_almacen') }}</label>
									...
									<div class="col-xs-12 col-sm-4 total-price" >
										...
										@if( !empty(Config::get('app.SeguroCarrito')) )
											<input type="checkbox" style="top: 0px;height: 10px;margin-right: 0px;"   data-sub="{{$codSub}}" id="seguro_carrito_info" name="seguro_carrito_info"  value="1">
											<label for="seguro_carrito_info"> {{ trans($theme.'-app.user_panel.seguro_envio') }}  </label>
											<br><br>
										@endif
										{{ trans($theme.'-app.shopping_cart.total_articles') }} <br>{{ \Tools::moneyFormat($totalLotes[$codSub], trans($theme.'-app.subastas.euros'),2)}}

										<br>	<br><br>

										{{ trans($theme.'-app.shopping_cart.total_pay') }} <br><span class="precio_final_carrito_{{$codSub}}">{{ \Tools::moneyFormat($pagar, trans($theme.'-app.subastas.euros'), 2)}}</span>
									...
									<div class="col-xs-12" >
										...
										<label style="float:right" for = "acceptCheck"> {!! trans($theme.'-app.shopping_cart.check') !!} </label>
										<input style="float:right" type="checkbox" id="acceptCheck"  name="acceptCheck" value=1>
										<br><br>
										<p class= "text-justify mt-1">
											{!! trans($theme.'-app.shopping_cart.text_condition') !!}
										</p>
										@else
										...
										@endif
										<button style="float:right" type="button" class="secondary-button   submitShoppingCart_JS "  cod_sub="{{$codSub}}" class="btn btn-step-reg" >{{ trans($theme.'-app.user_panel.pay') }}</button>
										<br><br>
									</div>
