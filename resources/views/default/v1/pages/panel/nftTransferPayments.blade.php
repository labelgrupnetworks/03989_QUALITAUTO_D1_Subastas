...
@if(count($pendingPay) > 0 && count($noTransfer) == 0 )
	...
	<div class="account-user color-letter  pendientes_pago mb-2">
			...
			<div class="row">
				<h2>{!! trans($theme.'-app.user_panel.title_pending_transfer_pays_NFT') !!}</h2>
				...
				@foreach($pendingPay as $pending)
					...
					$total+=$pending->cost_transfer_nft;
					$transfersIds[]=$pending->transfer_id_nft;

					@endphp
					<div class="col-xs-12  ">
						<div class="col-xs-12 col-md-9 payment-text-cell"> <strong> {{ trans($theme.'-app.user_panel.lot') }}:</strong>  {{$pending->descweb_hces1}} </div>
						<div class="col-xs-12 col-md-3 payment-money-cell">{{\Tools::moneyFormat($pending->cost_transfer_nft,"€",2)}}</div>
					</div>
				...
				<div class="col-xs-12 mt-4 mb-4 text-right">
					<a href="{{ Route("transferPayUrl", ["operationId" => implode("_",$transfersIds)])}}" class="button-principal"> {!! trans($theme.'-app.user_panel.pay') !!} </a>
