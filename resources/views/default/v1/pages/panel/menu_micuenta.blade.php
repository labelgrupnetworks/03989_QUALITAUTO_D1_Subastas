...
<div class="tabs-custom filters-auction-content">
    ...
    <ul id="user-account-ul" class="ul-format color-letter">
        ...
        <li class="text-uppercase<?php if($tab == 'favorites'){ echo(' tab-active'); } ?>" role="presentation" >
            ...
            <a class="" href="{{ \Routing::slug('user/panel/' . (empty(\Config('app.new_favorites_panel')) ? 'favorites' : \Config('app.new_favorites_panel'))) }}" data-title="{{ trans($theme.'-app.panel.my_adj') }}" >
 ...
 <?php /*<li role="presentation" <?php if($tab == 'datos-personales'){ echo('class="active"'); } ?>><a href="{{ \Routing::slug('user/panel/info') }}">{{ trans($theme.'-app.user_panel.info') }}</a></li> */?>
        <li class="text-uppercase"><a href="{{ \Routing::slug('logout') }}"><i class="fa fa-sign-out-alt"></i>{{ trans($theme.'-app.user_panel.exit') }}</a></li>
    </ul>
</div>
