							...
							<div class="panel panel-default">
								...
								@foreach ($subastas as $key_sub => $subasta)
									<div class="panel-heading">
										...
										<a data-toggle="collapse" href="#{{ $key_sub }}" aria-expanded="{{ $index == 0 ? 'true' : 'false' }}">
											<h4 class="panel-title">
												{{ $key_sub }} - {{ $subasta[0]->des_sub }} - {{ trans("$theme-app.global.from") }}
												{{ \Tools::getDateFormat($subasta[0]->dfec_sub, 'Y-m-d H:i:s', 'd/m/Y') }}
												{{ trans("$theme-app.global.to_the") }}
												{{ \Tools::getDateFormat($subasta[0]->hfec_sub, 'Y-m-d H:i:s', 'd/m/Y') }}
											</h4>
										</a>
									</div>

									<div id="{{ $key_sub }}" class="panel-collapse collapse {{ $index == 0 ? 'in' : '' }}"
												...
												<table class="table table-striped table-custom" style="width:100%">
													...
													<tbody>
														...
														@foreach ($subasta->sortBy('lin_hces1')->sortByDesc('num_hces1') as $lote)
															...
															<tr>
																<td>
																	<a href="{{ $url_friendly }}"><img
																			src="{{ \Tools::url_img('lote_small', $lote->num_hces1, $lote->lin_hces1) }}" height="42"></a>
																</td>
