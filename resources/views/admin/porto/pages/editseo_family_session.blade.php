                        ...
                        <div class="panel-body">
                            ...
                            <div class="tabs tabs-bottom tabs-primary">
                                ...
                                <div class="tab-content">
                                     <input type="hidden" name="id_auc" value="{{$data['auc_index']->id_web_auc_index}}">
                                        <?php $i = true;?>
                                        @foreach($data['idiomes'] as $idiomes => $keylang)
                                               ...
                                               <div class='row'>
                                                @if(!empty($data['content']))
                                                    @foreach($data['content'] as $content)
                                                         @if(strtoupper($idiomes) == $content->codlang_auc_seo)
                                                         <input id="id_{{strtoupper($idiomes)}}" type="hidden" name="id_{{strtoupper($idiomes)}}" value="<?= !empty($content->id_auc_seo)? $content->id_auc_seo :' ';?>">
                                                         <input type="hidden" id="webcont_{{strtoupper($idiomes)}}" name="webcont_{{strtoupper($idiomes)}}" value=" " >
                                                         <div class="col-md-6">
                                                              <p>{{ trans('admin-app.title.nombre') }}</p> 
                                                              <input type="text" name="webname_{{strtoupper($idiomes)}}" placeholder="{{ trans('admin-app.title.nombre') }}" class="form-control" value="<?= !empty($content->webname_auc_seo)? $content->webname_auc_seo :' ';?>">
                                                         </div>

                                                         <div class="col-lg-6 hidden">
                                                             <p>{{ trans('admin-app.title.url_amigable') }}</p>  
                                                             <input type="text" name="webfriend_{{strtoupper($idiomes)}}" placeholder="{{ trans('admin-app.title.url_amigable') }}" class="form-control" value="<?= !empty($content->webfriend_auc_seo)? $content->webfriend_auc_seo :' ';?>">
                                                         </div>
                                                         <div class="col-lg-12">
                                                             <br>
                                                         </div>
                                                         <div class="col-lg-6">
                                                             ...
                                                             <input type="text" name="webmetad_{{strtoupper($idiomes)}}" placeholder="{{ trans('admin-app.title.meta_description') }}" class="form-control" value="<?= !empty($content->webmetad_auc_seo)? $content->webmetad_auc_seo:' ';?>">
                                                         </div>
                                                         <div class="col-lg-6">
                                                             <p>{{ trans('admin-app.title.meta_titulo') }}</p>  
                                                             <input type="text" name="webmetat_{{strtoupper($idiomes)}}" placeholder="{{ trans('admin-app.title.meta_titulo') }}" class="form-control" value="<?= !empty($content->webmetat_auc_seo)? $content->webmetat_auc_seo :' ';?>">
                                                         </div>
                                                             <div class="col-lg-12">
                                                             <br>
                                                         </div>
                                                             <div class="col-md-12" id="content-summernote_{{strtoupper($idiomes)}}" >
                                                               ...
                                                               <textarea class="summernote content-summernote" data-plugin-summernote data-plugin-options='{ "height": 300, "codemirror": { "theme": "ambiance" } }' placeholder="{{ trans('admin-app.placeholder.consulta') }}" class="form-control" rows="5"><?= empty($content->webcont_auc_seo)? ' ' : $content->webcont_auc_seo; ?></textarea></div>
                    ...
                    <section class="panel">                                      
                        <div class="panel-body">
                                <div class="row form-group">
                                        <div class="col-lg-12">
                                            <button type="button" class="mb-xs mt-xs mr-xs btn btn-lg btn-primary save_seo_family_session">{{ trans('admin-app.title.save') }}</button>
                                        </div>
