@extends('admin::layouts.logged')
@section('content')

    @php
        $emp = \Config::get('app.emp');
        $linkToPrint = \Config::get('app.printExhibitionLabels');
    @endphp

    <section class="content-body" role="main">
        @csrf

        @include('admin::components.page_title', [
            'pageTitle' => trans('admin-app.title.auctions'),
        ])

        <x-admin::table.layout>
            <x-slot:toolbar>
                <x-admin::button.dropdown id="js-dropdownItems" label="{{ trans('admin-app.button.selecteds') }}">
                    <li>
                        <button class="btn" data-toggle="modal" data-target="#editMultpleAuctionsModal">
                            {{ trans('admin-app.button.modify') }}
                        </button>
                    </li>
                </x-admin::button.dropdown>

                @if ($exports->isNotEmpty())
                    <div class="btn-group">
                        {{-- <button type="button" class="btn btn-default btn-sm">Exportaciones personalizadas</button> --}}
                        <button class="btn btn-default btn-sm dropdown-toggle" data-toggle="dropdown" type="button"
                            aria-haspopup="true" aria-expanded="false">
                            Exportaciones personalizadas <span class="caret"></span>
                        </button>
                        <ul class="dropdown-menu">
                            @foreach ($exports as $export)
                                <li>
                                    <a
                                        href="{{ route('subasta.custom-reports.download', ['id' => $export]) }}">{{ $export }}</a>
                                </li>
                            @endforeach
                        </ul>
                    </div>
                @endif

                <a class="btn btn-sm btn-primary mr-1 ml-auto"
                    href="{{ route("$resource_name.create") }}">{{ trans('admin-app.button.new_fem') }}
                    {{ trans('admin-app.title.auction') }}</a>

                @include('admin::includes.config_table', [
                    'id' => $resource_name,
                    'params' => ((array) $formulario),
                ])
            </x-slot:toolbar>

            <div class="col-xs-12 table-responsive">
                <x-slot:table>
                    <table class="table table-striped table-condensed table-responsive" id="{{ $resource_name }}"
                        data-order-name="order" style="width:100%">
                        <thead>

                            <tr>
                                <th>
                                    <label>
                                        <input name="js-selectAll" data-objective="auc_ids" type="checkbox" value="true">
                                        <input id="urlAllSelected" name="url-allSelected" type="hidden"
                                            value="{{ route('subastas.update_with_filters') }}">
                                    </label>
                                </th>
                                <th class="cod_sub" data-order="cod_sub" style="cursor: pointer">
                                    {{ trans('admin-app.fields.cod_sub') }}
                                    @if (request()->order == 'cod_sub')
                                        <span style="margin-left: 5px; float: right;">
                                            @if (request()->order_dir == 'asc')
                                                <i class="fa fa-arrow-up" aria-hidden="true" style="color:green"></i>
                                            @else
                                                <i class="fa fa-arrow-down" aria-hidden="true" style="color:red"></i>
                                            @endif
                                        </span>
                                    @endif
                                </th>
                                <th class="des_sub" data-order="des_sub" style="cursor: pointer">
                                    {{ trans('admin-app.fields.des_sub') }}
                                    @if (request()->order == 'des_sub')
                                        <span style="margin-left: 5px; float: right;">
                                            @if (request()->order_dir == 'asc')
                                                <i class="fa fa-arrow-up" aria-hidden="true" style="color:green"></i>
                                            @else
                                                <i class="fa fa-arrow-down" aria-hidden="true" style="color:red"></i>
                                            @endif
                                        </span>
                                    @endif
                                </th>
                                @if (config('app.ArtistInExibition', false))
                                    <th class="artist_name" data-order="valorcol_sub" style="cursor: pointer">
                                        {{ trans('admin-app.fields.artist_name') }}
                                        @if (request()->order == 'valorcol_sub')
                                            <span style="margin-left: 5px; float: right;">
                                                @if (request()->order_dir == 'asc')
                                                    <i class="fa fa-arrow-up" aria-hidden="true" style="color:green"></i>
                                                @else
                                                    <i class="fa fa-arrow-down" aria-hidden="true" style="color:red"></i>
                                                @endif
                                            </span>
                                        @endif
                                    </th>
                                @endif
                                <th class="subc_sub" data-order="subc_sub" style="cursor: pointer">
                                    {{ trans('admin-app.fields.subc_sub') }}
                                    @if (request()->order == 'subc_sub')
                                        <span style="margin-left: 5px; float: right;">
                                            @if (request()->order_dir == 'asc')
                                                <i class="fa fa-arrow-up" aria-hidden="true" style="color:green"></i>
                                            @else
                                                <i class="fa fa-arrow-down" aria-hidden="true" style="color:red"></i>
                                            @endif
                                        </span>
                                    @endif
                                </th>
                                <th class="tipo_sub" data-order="tipo_sub" style="cursor: pointer">
                                    {{ trans('admin-app.fields.tipo_sub') }}
                                    @if (request()->order == 'tipo_sub')
                                        <span style="margin-left: 5px; float: right;">
                                            @if (request()->order_dir == 'asc')
                                                <i class="fa fa-arrow-up" aria-hidden="true" style="color:green"></i>
                                            @else
                                                <i class="fa fa-arrow-down" aria-hidden="true" style="color:red"></i>
                                            @endif
                                        </span>
                                    @endif
                                </th>
                                <th class="dfec_sub" data-order="dfec_sub" style="cursor: pointer">
                                    {{ trans('admin-app.fields.dfec_sub') }}
                                    @if (request()->order == 'dfec_sub')
                                        <span style="margin-left: 5px; float: right;">
                                            @if (request()->order_dir == 'asc')
                                                <i class="fa fa-arrow-up" aria-hidden="true" style="color:green"></i>
                                            @else
                                                <i class="fa fa-arrow-down" aria-hidden="true" style="color:red"></i>
                                            @endif
                                        </span>
                                    @endif
                                </th>
                                <th class="hfec_sub" data-order="hfec_sub" style="cursor: pointer">
                                    {{ trans('admin-app.fields.hfec_sub') }}
                                    @if (request()->order == 'hfec_sub')
                                        <span style="margin-left: 5px; float: right;">
                                            @if (request()->order_dir == 'asc')
                                                <i class="fa fa-arrow-up" aria-hidden="true" style="color:green"></i>
                                            @else
                                                <i class="fa fa-arrow-down" aria-hidden="true" style="color:red"></i>
                                            @endif
                                        </span>
                                    @endif
                                </th>
                                <th>
                                    <span>{{ trans('admin-app.fields.actions') }}</span>
                                </th>
                            </tr>
                        </thead>

                        <tbody>

                            <tr id="filters">
                                <form class="form-group" action="">
                                    <td></td>
                                    <input name="order" type="hidden" value="{{ request('order', 'dfec_sub') }}">
                                    <input name="order_dir" type="hidden" value="{{ request('order_dir', 'desc') }}">
                                    <td class="cod_sub">{!! $formulario->cod_sub !!}</td>
                                    <td class="des_sub">{!! $formulario->des_sub !!}</td>
                                    @if (config('app.ArtistInExibition', false))
                                        <td class="artist_name">{!! $formulario->artist_name !!}</td>
                                    @endif
                                    <td class="subc_sub">{!! $formulario->subc_sub !!}</td>
                                    <td class="tipo_sub">{!! $formulario->tipo_sub !!}</td>
                                    <td class="dfec_sub">{!! $formulario->dfec_sub !!}</td>
                                    <td class="hfec_sub">{!! $formulario->hfec_sub !!}</td>
                                    <td class="d-flex" style="width: 300px">
                                        <input class="btn btn-info w-100" type="submit"
                                            value="{{ trans('admin-app.button.search') }}"><a
                                            class="btn btn-warning w-100"
                                            href="{{ route("$resource_name.index") }}">{{ trans('admin-app.button.restart') }}</a>
                                    </td>
                                </form>
                            </tr>


                            @forelse ($fgSubs as $fgSub)
                                <tr id="fila{{ $fgSub->cod_sub }}">
                                    <td>
                                        <label>
                                            <input name="auc_ids" type="checkbox" value="{{ $fgSub->cod_sub }}">
                                        </label>
                                    </td>
                                    <td class="cod_sub">{{ $fgSub->cod_sub }}</td>
                                    <td class="des_sub">{{ $fgSub->des_sub }}</td>
                                    @if (config('app.ArtistInExibition', false))
                                        <td class="artist_name">{!! $artists[$fgSub->valorcol_sub] ?? null !!}</td>
                                    @endif
                                    <td class="subc_sub">{{ $fgSub->subc_sub_type }}</td>
                                    <td class="tipo_sub">{{ $fgSub->tipo_sub_type }}</td>
                                    <td class="dfec_sub">{{ $fgSub->desde_fecha_hora }}</td>
                                    <td class="hfec_sub">{{ $fgSub->hasta_fecha_hora }}</td>
                                    <td class="d-inline-flex justify-content-flex-end" style="gap: 2px;">
                                        <a class="btn btn-success btn-sm"
                                            href="{{ route("$resource_name.edit", $fgSub->cod_sub) }}"
                                            title="{{ trans('admin-app.button.edit') }}"><i class="fa fa-pencil-square-o"
                                                aria-hidden="true"></i>{{ trans('admin-app.button.edit') }}
                                            {{ trans('admin-app.title.auction') }}</a>
                                        <a class="btn btn-primary btn-sm"
                                            href="{{ route("$resource_name.show", $fgSub->cod_sub) }}"
                                            title="{{ trans('admin-app.button.show') }}"><i class="fa fa-eye"
                                                aria-hidden="true"></i> {{ trans('admin-app.button.show_content') }}</a>
                                        @if (\Config::get('app.printExhibitionLabels'))
                                            {{-- Botón de descarga PDF de ansorena --}}
                                            <a class="btn btn-info btn-sm"
                                                href="{{ \Config::get('app.printExhibitionLabels') }}?empresa={{ $emp }}&subasta={{ $fgSub->cod_sub }}"
                                                target="blank_"><i class="fa fa-print" aria-hidden="true"></i></a>
                                        @endif



                                        {{-- si esta en subalia --}}
                                        @if ($auchouse)
                                            <button class="show_subalia_JS btn btn-warning btn-sm"
                                                data-url="{{ $auchouse->importAuctionUrl($fgSub->cod_sub) }}">
                                                {{ trans('admin-app.title.show_in_subalia') }}
                                            </button>

                                            <button class="hide_subalia_JS btn btn-info btn-sm"
                                                data-url="{{ $auchouse->hideAuctionUrl($fgSub->cod_sub) }}">
                                                {{ trans('admin-app.title.hide_in_subalia') }}
                                            </button>
                                        @endif
                                    </td>
                                </tr>

                            @empty

                                <tr>
                                    <td colspan="6">
                                        <h3 class="text-center">{{ trans('admin-app.title.without_results') }}</h3>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>

                    <div class="col-xs-12 d-flex justify-content-center">
                        {{ $fgSubs->appends(Request::query())->links() }}
                    </div>
                </x-slot:table>
            </div>
        </x-admin::table.layout>

    </section>

    @include('admin::pages.subasta.subastas._edit_selecteds')

@stop
