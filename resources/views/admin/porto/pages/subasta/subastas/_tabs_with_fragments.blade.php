<ul class="nav nav-tabs" id="show-auction-tab" role="tablist">
    @if (!in_array('noLots', Config::get('app.config_menu_admin')))
        <li class="nav-item active">
            <a class="nav-link" id="lotes-tab" data-toggle="tab" href="#lotes" role="tab" aria-controls="lotes"
                aria-selected="false">{{ trans('admin-app.title.lots') }}</a>
        </li>
    @endif
    ...
    @if (!in_array('noOrders', Config::get('app.config_menu_admin')) && !empty($ordersTable))
        <li class="nav-item">
            <a class="nav-link" id="ordenes-tab" data-toggle="tab" href="#ordenes" role="tab"
                aria-controls="ordenes" aria-selected="false">{{ trans('admin-app.title.orders') }}</a>
        </li>
    @endif
    @if (!in_array('noAwards', Config::get('app.config_menu_admin')))
        <li class="nav-item">
            <a class="nav-link" id="adjudicaciones-tab" data-toggle="tab" href="#adjudicaciones" role="tab"
                aria-controls="adjudicaciones" aria-selected="false">{{ trans('admin-app.title.awards') }}</a>
        </li>
    @endif

    @if (in_array('lotsNotAwards', Config::get('app.config_menu_admin')))
        <li class="nav-item">
            <a class="nav-link" id="notAwards-tab" data-toggle="tab" href="#notAwards" role="tab"
                aria-controls="notAwards" aria-selected="false">{{ trans('admin-app.title.not_awards') }}</a>
        </li>
    @endif
