    ...
    <div id="cms">
        ...
        <section class="panel">
            <form id="traducciones">
                ...
                <input type='hidden' value='{{head($data['original'])->key_header}}' name='key_header'>
                @endif
                                    ...
                                    <tr role="row" class="odd">
                                        <td style="width: 100%; ">
                                            <p style="word-break: break-all;">
                                                ...
                                                <span>{{$data['original'][$key]->key_translate}}</span> - <span style='font-size: 14px;    word-break: break-all;'>{{$data['original'][$key]->web_translation}}</span>
                                                @else
                                                <span>{{$data[$data['lang']][$key]->key_translate}}</span> - <span style='font-size: 14px;    word-break: break-all;'>{{ $data[$data['lang']][$key]->web_translation }}</span>
                                                @endif

                                                @if(!empty($data[$data['lang']][$key]->id_key_translate) && $data[$data['lang']][$key]->id_key_translate !== null)
                                                <input type="text" name="{{head($data['original'])->key_header}}**{{$data[$data['lang']][$key]->key_translate}}" class="form-control" value="{{$trad->web_translation}}">
                                                @else
                                                <input type="text" name="{{head($data['original'])->key_header}}**{{$data[$data['lang']][$key]->key_translate}}" class="form-control" value="">
                                                @endif
