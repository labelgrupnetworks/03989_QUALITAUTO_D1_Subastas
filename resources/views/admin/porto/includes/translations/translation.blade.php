		...
		<div class="d-flex align-items-center">
			...
			<div class="trans-title">
				@if ($has_translation)
					@if (!empty($data['original'][$key]->web_translation))
						...
						<span style='font-size: 14px;    word-break: break-all;'>{{ $data['original'][$key]->web_translation }}</span>
					@else
						<span>{{ $data[$data['lang']][$key]->key_translate }}</span>
						- <span style='font-size: 14px;    word-break: break-all;'>{{ $data[$data['lang']][$key]->web_translation }}</span>
					@endif
				@else
					<span>|key_translate|</span> - <span style='font-size: 14px;word-break: break-all;'>|web_translation|</span>
				@endif
			</div>
			...
			<div class="d-flex align-items-center" style="margin-left: auto">
				@php
					if ($has_translation) {
						$keySection = $data['key'];
						$key = $data['original'][$key]->key_translate ?? $data[$data['lang']][$key]->key_translate;
						$textToPhp = "trans(\"\$theme-app.{$keySection}.{$key}\")";
						$textToBlade = "{{ $textToPhp }}";
					} else {
						$textToPhp = "trans(\"\$theme-app.|key_headers|.|key_translate|\")";
						$textToBlade = "{{ $textToPhp }}";
					}
				@endphp
				<button class="copy-link btn btn-link p-0" type="button">
					<svg height="24" viewBox="0 -0.114 49.742 51.317" width="24" xmlns="http://www.w3.org/2000/svg">
						<path
				...
				<input type="text" value="{{ $textToBlade }}" style="opacity: 0; width: 15px">

				<button class="copy-link btn btn-link p-0" type="button">
					<svg width="32" viewBox="0 0 256 134" style="height: auto" xmlns="http://www.w3.org/2000/svg"
				...
				<input type="text" value="{{ $textToPhp }}" style="opacity: 0; width: 5px">
		...
		@if ($has_translation)
			@if (!empty($data[$data['lang']][$key]->id_key_translate) && $data[$data['lang']][$key]->id_key_translate !== null)
				<input class="form-control"
					name="{{ head($data['original'])->key_header }}**{{ $data[$data['lang']][$key]->key_translate }}" type="text"
					value="{{ $trad->web_translation }}">
			@else
				<input class="form-control"
					name="{{ head($data['original'])->key_header }}**{{ $data[$data['lang']][$key]->key_translate }}" type="text"
					value="">
			@endif
		@else
			<input class="form-control" name="|key_headers|**|key_translate|" type="text" value="|web_translation|">
		@endif
