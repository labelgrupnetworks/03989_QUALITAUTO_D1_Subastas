<div class="lot_list_ajax" data-lot_page="{{ $page }}" style="display: none;">
	<div class="row">
		@foreach ($bann_list as $bann)
			<?php
			$url = '';
			...
			?>
			<div class="ajax-lot col-xs-12 col-sm-6 col-lg-4 square">
				<div class="item_home">
					<a title="{{ $titulo }}" <?= $url ?>>
					...
					<div class="data-price">
						@if ($bann->retirado_asigl0 == 'N')
							@if (\Config::get('app.estimacion') || \Config::get('app.impsalhces_asigl0'))
								<p class="salida">
									@if (\Config::get('app.estimacion'))
										{{ trans(\Config::get('app.theme') . '-app.lot.estimate') }} <span class="num-lot"> {{ $bann->imptas_asigl0 }}
											-
											{{ $bann->imptash_asigl0 }} {{ trans(\Config::get('app.theme') . '-app.subastas.euros') }}</span>
									@elseif(\Config::get('app.impsalhces_asigl0'))
										{{ trans(\Config::get('app.theme') . '-app.lot.lot-price') }} <span class="num-lot">
											{{ $bann->impsalhces_asigl0 }} {{ trans(\Config::get('app.theme') . '-app.subastas.euros') }}</span>
									@endif
								</p>
							@endif
							@if (\Config::get('app.awarded'))
								<p class="salida">
									...
									@elseif($bann->cerrado_asigl0 == 'S' && !empty($bann->max_puja) && $bann->remate_asigl0 == 'S')
										{{ trans(\Config::get('app.theme') . '-app.subastas.buy_to') }}: <span>{{ $bann->max_puja }}
							...
							<div class="lot-btn-clock">
								<div class="lot-btn"><a title="{{ $titulo }}"
										<?= $url ?>>{{ trans(\Config::get('app.theme') . '-app.sheet_tr.view') }}</a></div>
