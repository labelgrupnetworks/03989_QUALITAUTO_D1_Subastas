...
<div class="col-xs-12 info_single">
    ...
    @if (Session::has('user') && $lote_actual->retirado_asigl0 == 'N')
        <a class="btn btn-add-fav <?= $data['subasta_info']->lote_actual->favorito ? 'hidden' : '' ?>" id="add_fav"
            href="javascript:action_fav_modal('add')">
            <p>{{ trans(\Config::get('app.theme') . '-app.lot.add_to_fav') }} </p>
        </a>
        <a class="btn btn-del-fav <?= $data['subasta_info']->lote_actual->favorito ? '' : 'hidden' ?>" id="del_fav"
