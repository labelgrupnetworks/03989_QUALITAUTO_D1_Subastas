				...
				@if ($item->fac_hces1 != 'D' && $item->fac_hces1 != 'R')
					...
					@if ($item->tipo_sub != 'V')
						@if ($item->tipo_sub == 'W' && !empty($item->formatted_imptas_asigl0) && !empty($item->formatted_imptash_asigl0))
							<p class="salida" id="estimacion" style="margin-bottom: 15px;">
								{{ trans(\Config::get('app.theme') . '-app.lot.estimate') }} <span> {{ $item->formatted_imptas_asigl0 }} -
									{{ $item->formatted_imptash_asigl0 }} {{ trans(\Config::get('app.theme') . '-app.subastas.euros') }}</span></p>
						@else
						...
						<p class="salida" id="precioSalida"
							...
							@if ($item->cerrado_asigl0 != 'S')
								{{ trans(\Config::get('app.theme') . '-app.lot.lot-price') }} <span> {{ $item->formatted_impsalhces_asigl0 }}
					...
					@if (($item->tipo_sub == 'P' || $item->tipo_sub == 'O') && $item->cerrado_asigl0 == 'N' && !empty($item->max_puja))
						<p class="salida" id="maxPuja">{{ trans(\Config::get('app.theme') . '-app.lot.puja_actual') }}<span>
					...
					@if (\Config::get('app.awarded'))
						<p class="salida" id="awarded">
							...
							@elseif(
								...
								@endif
								{{ trans(\Config::get('app.theme') . '-app.subastas.buy_to') }}: <span>{{ \Tools::moneyFormat($precio_venta) }}
									{{ trans(\Config::get('app.theme') . '-app.subastas.euros') }}</span>
