...
$categories = $fgortsec0->GetAllFgOrtsec0('DEP')->get()->toarray();
        ...
        <div class="container-fluid">
            <div class="row">
                ...
                @if(Session::has('user'))
                    @if(Session::get('user.admin'))
                        <div class="admin-item" style="position: relative">
                            <a href="/admin"  target = "_blank" style="color: white;"> {{ trans(\Config::get('app.theme').'-app.login_register.admin') }}</a>
                        </div>
                    @endif
                @endif
                <div class="login-item" style="position: relative">
                    ...
                    @else
                        <a href="{{ \Routing::slug('user/panel/orders') }}" ><i class="fa fa-user fa-2x hidden-lg"></i><span class="visible-lg">{{ trans(\Config::get('app.theme').'-app.login_register.my_panel') }}</span></a>
                    @endif
        ...
        <div class="row">
            <div class="col-xs-12 header-gutinvest no-padding">
                ...
                <div class="col-lg-7 col-xs-7 visible-lg menu-item">
                    <ul>
                        <li>
                            <a title="{{ trans(\Config::get('app.theme').'-app.home.home')}}" href="{{($lang == 'es')? url('/es') : url('/en')}}">{{ trans(\Config::get('app.theme').'-app.home.home')}}</a>
                        </li>
                        <?php
                        ...
                        ?>
                         ...
                         <li class="open-menu-especial" style="position: relative">
                                <a href="{{ \Routing::translateSeo('todas-subastas') }}">{{ trans(\Config::get('app.theme').'-app.foot.sells')}} <i class="fa fa-caret-down"></i></a>
                                <div class="menu-especial">
									<a href="{{ \Routing::translateSeo('todas-subastas') }}">{{ trans(\Config::get('app.theme').'-app.subastas.next_sell')}}</a>
                                    <?php
                                    ...
                                    @if(!empty($has_subasta))
                                        <a href="{{ \Routing::translateSeo('subastas-online') }}">{{ trans(\Config::get('app.theme').'-app.foot.online_auction')}}</a>
                                    @endif
                                    ...
                                    ?>
                                    @if(!empty($has_subasta))
										<a href="{{ \Routing::translateSeo('presenciales') }}">{{ trans(\Config::get('app.theme').'-app.foot.presenciales')}}</a>
                                    @endif
                                    <a href="{{ \Routing::translateSeo('busqueda') }}">{{ trans(\Config::get('app.theme').'-app.foot.search_actives')}}</a>
                                    <?php
                        ...
                        * */
						...
						<li>
                            <a title="{{ $sectorInmuebles["des_ortsec0"] }}" href="{{ route("department", ["text" => $sectorInmuebles["key_ortsec0"]]) }}">{{ $sectorInmuebles["des_ortsec0"] }}</a>
                        </li>
                        <li>
                            <a title="{{ trans(\Config::get('app.theme').'-app.foot.about_us')}}" href="<?= \Routing::translateSeo('pagina').trans(\Config::get('app.theme').'-app.links.about_us')?>">{{ trans(\Config::get('app.theme').'-app.foot.about_us')}}</a>
                        </li>

                        <li>
                            <a title="{{ trans(\Config::get('app.theme').'-app.foot.services')}}" href="<?= \Routing::translateSeo('pagina').trans(\Config::get('app.theme').'-app.links.services')?>">{{ trans(\Config::get('app.theme').'-app.foot.services')}}</a>
                        </li>
                        <li>
                            <a title="{{ trans(\Config::get('app.theme').'-app.foot.valorar_producto')}}" href="<?= \Routing::translateSeo('valoracion-articulos')?>">{{ trans(\Config::get('app.theme').'-app.foot.valorar_producto')}}</a>
                        </li>
                        <li>
            ...
            <div class="col-lg-3 col-xs-9 no-padding group-menu-item">
                <div class="search-item">
                    ...
                    <img
                        src="/themes/{{\Config::get('app.theme')}}/assets/img/search.png"
                        alt="{{(\Config::get( 'app.name' ))}}"
                        class="img-responsive search-icon"
                    >
                    <img
                        src="/themes/{{\Config::get('app.theme')}}/assets/img/cancel.png"
                        alt="{{(\Config::get( 'app.name' ))}}"
                        class="img-responsive cancel-icon"
                        style="display: none;"
                    >
                    </div>
                ...
                <div class="login-item hidden-xs" style="position: relative">
                    ...
                    @else
                    <a href="{{ \Routing::slug('user/panel/orders') }}" ><i class="fa fa-user fa-3x hidden-lg"></i><span class="visible-lg">{{ trans(\Config::get('app.theme').'-app.login_register.my_panel') }}</span></a>
                    ...
                    <div class="login_desktop" style="display: none;">
                        ...
                        <a class="btn-register-modal" href="{{ \Routing::translateSeo('login') }}">{{ trans(\Config::get('app.theme').'-app.login_register.register') }}</a>
                        </div>
                </div>
                ...
                <div>
                        ...
                        <div>
                            <a title="<?= trans(\Config::get('app.theme').'-app.head.language_'.$key) ?>" href="/<?=$key;?>">
                                <img class="img-responsive" src="/themes/{{\Config::get('app.theme')}}/assets/img/flag_<?=$key;?>.png" width="30px"/>
                            </a>
                        </div>
                            ...
                            @elseif(\Config::get( 'app.google_translate' ))
                                ...
                                <script type="text/javascript">
                                    function googleTranslateElementInit() {
                                        new google.translate.TranslateElement({pageLanguage: 'es', includedLanguages: 'es,bg,cs,da,de,el,et,fi,fr,ga,hr,hu,it,lt,lv,mt,nl,pl,pt,ro,ru,sk,sl,sv,tr', layout: google.translate.TranslateElement.InlineLayout.SIMPLE}, 'google_translate_element');
                                    }
                                </script>
...
<div id="menuResponsive" class="hidden-lg">
	...
	<ul class="nav navbar-nav navbar-right navbar-responsive">
            <li><a title="{{ trans(\Config::get('app.theme').'-app.home.home')}}" href="{{($lang == 'es')? url('/es') : url('/en')}}">{{ trans(\Config::get('app.theme').'-app.home.home')}}</a></li>
            <?php
                ...
                <li>
                            <a title="{{ trans(\Config::get('app.theme').'-app.foot.contact')}}" href="<?= \Routing::translateSeo('pagina').trans(\Config::get('app.theme').'-app.links.about_us')?>">{{ trans(\Config::get('app.theme').'-app.foot.about_us')}}</a>
                        </li>

                        <li>
                            <a title="{{ trans(\Config::get('app.theme').'-app.foot.contact')}}" href="<?= \Routing::translateSeo('pagina').trans(\Config::get('app.theme').'-app.links.services')?>">{{ trans(\Config::get('app.theme').'-app.foot.services')}}</a>
                        </li>
                        <li>
                            <a title="{{ trans(\Config::get('app.theme').'-app.foot.contact')}}" href="<?= \Routing::translateSeo('valoracion-articulos')?>">{{ trans(\Config::get('app.theme').'-app.foot.valorar_producto')}}</a>
                        </li>
