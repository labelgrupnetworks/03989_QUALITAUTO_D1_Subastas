<div class="tabs-custom filters-auction-content">
	...
	<ul id="user-account-ul" class="ul-format color-letter">
		...
		{{--
		<li class="text-uppercase @php if($tab == 'orders'){ echo(' tab-active'); } @endphp" role="presentation">
			...
			<a class="" data-title="{{ trans(\Config::get('app.theme').'-app.panel.my_orders') }}"
				...
				@else
				<img width="20px" src="/themes/{{\Config::get('app.theme')}}/assets/img/hammer_no_select.png"
					alt="{{ trans(\Config::get('app.theme').'-app.panel.my_orders') }}">
				@endif
		...
		<li class="text-uppercase @php if($tab == 'allotments' || $tab == 'bills'){ echo(' tab-active'); } @endphp"
			role="presentation">
