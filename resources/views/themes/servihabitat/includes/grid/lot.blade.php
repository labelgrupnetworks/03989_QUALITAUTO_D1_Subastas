...
<div class="{{$class_square}} square" {!! $codeScrollBack !!}>
	...
	<a title="{{ $titulo }}" class="lote-destacado-link secondary-color-text" @if(!$cerrado) {!!$url!!} @else style="cursor: auto" @endif >
        ...
        <div class="item_lot">
			...
			@if($cerrado)
			<p class="salida-time">
				<span>{{ trans("$theme-app.lot_list.end_lot")  }}</span>
			</p>
			@elseif($subasta_online && !$cerrado)
				<p class="salida-time">
					<i class="fa fa-clock-o"></i>
					<span data-txtend="{{ trans("$theme-app.lot_list.end_lot")  }}" data-countdown="{{strtotime($item->close_at) - getdate()[0] }}" data-format="<?= \Tools::down_timer($item->close_at); ?>" class="timer"></span>
				</p>
			@elseif ($subasta_web && strtotime($item->orders_start) <= getdate()[0])
			<p class="salida-time">
				<i class="fa fa-clock-o"></i>
				<span data-txtend="{{ trans("$theme-app.lot_list.end_lot") }} " data-countdown="{{ strtotime($item->orders_end) - getdate()[0] }}" data-format="<?= \Tools::down_timer($item->orders_end); ?>" class="timer"></span>
			</p>
			@endif
            ...
            <div class="data-container">
					...
					<div class="data-price text-center mb-1">
				...
				<div class="subsec-wrapper mb-1" style="min-height: 48px">
					@if(!empty($subSec))
						<p class="text-center lot-subsec m-0">{{ $subSec }}</p>
					@endif
				</div>


				<div class="init-time-container d-flex justify-content-space-between">
					<p>{{ trans(\Config::get('app.theme').'-app.home.home') }}</p>
					<p>
						@if ($subasta_web)
						{{ \Tools::getDateFormat($item->orders_start, 'Y-m-d H:i:s', 'd/m/Y H:i') }}
						@else
						{{ \Tools::getDateFormat($item->start, 'Y-m-d H:i:s', 'd/m/Y H:i') }}
						@endif
					</p>
				</div>
