...
<div class="col-xs-12 col-md-4 order-auction-lot">
    <select class="form-control" id="order_selected" >
		...
		<!-- -->
		<option value="active" @if ($filters["order"] == 'active') selected @endif >
            {{ trans(\Config::get('app.theme').'-app.lot_list.order') }}:    {{ trans("$theme-app.subastas.en_curso") }}
        </option>
		<option value="pending" @if ($filters["order"] == 'pending') selected @endif >
            {{ trans(\Config::get('app.theme').'-app.lot_list.order') }}:    {{ trans("$theme-app.subastas.proximamente") }}
        </option>
		<option value="finish" @if ($filters["order"] == 'finish') selected @endif >
            {{ trans(\Config::get('app.theme').'-app.lot_list.order') }}:    {{ trans("$theme-app.subastas.cerrada") }}
        </option>
...
<div class="col-xs-12 pt-1 d-flex align-items-center">
