    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-sm-7 col-xs-12" style="position: relative">
                ...
                if ($subasta_web && $start_orders) {
                    $nameCountdown = 'countdown';
                    $timeCountdown = $lote_actual->orders_end;
                ...
                {{-- Por el momento no activamos los favoritos
                ...
                <div class="col-xs-12 no-padding hidden-xs">
                    ...
                    {{-- Quitamos favoritos por el momento
                        @if (Session::has('user') && !$retirado)
                    ...
                    <div class="col-xs-12 no-padding">
                        <div class="minis-content d-flex flex-wrap">
                            @if (!empty($lote_actual->imagenes) && count($lote_actual->imagenes) > 2)
                                @foreach ($lote_actual->imagenes as $key => $imagen)
            ...
            <div class="col-sm-5 col-xs-12 content-right-ficha d-flex justify-content-space-between flex-column">
                ...
                <div class="d-flex  flex-column">
                    <div class="ficha-info-title col-xs-12 no-padding">
                        <div class="titleficha col-xs-12 no-padding  secondary-color-text no-padding">
                            {{ $lote_actual->ref_asigl0 }} - {!! $lote_actual->descweb_hces1 ?? $lote_actual->titulo_hces1 !!}
                        </div>
                    </div>

                    <div class="init-time-container">
                        <p class="time-text">{{ trans(\Config::get('app.theme') . '-app.lot.start_date') }}</p>
                        <p class="time-value"
                            data-date="{{ $subasta_web ? $lote_actual->orders_start : $lote_actual->start_session }}">
                        </p>
                    </div>
                    <div class="finish-time-container">
                        <p class="time-text">{{ trans(\Config::get('app.theme') . '-app.lot.end_date') }}</p>
                        <p class="time-value"
                            data-date="{{ $subasta_web ? $lote_actual->orders_end : $lote_actual->end_session }}"></p>
                    </div>
                    <div class="propery-container">
                        <p class="propery-text">{{ trans(\Config::get('app.theme') . '-app.lot.property') }}: </p>
                        <p class="propery-value">
                            {{ \App\Models\V5\FxCli::select('rsoc_cli')->where('cod_cli', $lote_actual->prop_hces1)->value('rsoc_cli') }}
                        </p>
                    </div>
                    ...
                    @if (count($tipo_sec) !== 0)
                        <div class="col-xs-12 no-padding fincha-info-cats">
                            <div class="cat">{{ trans(\Config::get('app.theme') . '-app.lot.categories') }}
                            </div>
                    ...
                    <div
                        class="ficha-info-content col-xs-12 no-padding h-100 flex-column justify-content-center d-flex">
                        ...
                        @if (!$retirado && !$devuelto && !$fact_devuelta)
                            <div class="ficha-info-items">
                                ...
                                @elseif(!$deposito && !$cerrado)
                                    @include(
                                        'includes.ficha.pujas_ficha_deposito'
                                    )
                                @elseif($subasta_venta && !$cerrado && !$end_session)
                                    @if (\Config::get('app.shoppingCart'))
                                        @include(
                                            'includes.ficha.pujas_ficha_ShoppingCart'
                                        )
                                    @else
                ...
                <div class="col-xs-12 col-sm-12 no-padding">
                    @if ((($subasta_online && $deposito) || ($subasta_web && $subasta_abierta_P)) && !$cerrado && !$retirado)
...
<div class="container mb-2">
	...
	<div id="files-container" class="col-xs-12 no-padding files-container">

		<div class="col-xs-12 no-padding desc-lot-title d-flex justify-content-space-between">
			<p class="desc-lot-profile-title m-0">{{ trans(\Config::get('app.theme') . '-app.lot.documentos') }}
			</p>
		</div>

		<div class="col-xs-12 no-padding desc-lot-profile-content mt-2">
...
@php
$txtCliobj = \DB::table('FXCLIOBJ')
    ->where('GEMP_CLIOBJ', config('app.gemp'))
    ->where('cli_cliobj', $lote_actual->prop_hces1)
    ->orderBy('lin_cliobj')
    ->pluck('txt_cliobj');
...
<div class="container mb-5">
    <div id="prop" class="col-xs-12 no-padding ficha-tipo-v">
        <div class="col-xs-12 no-padding desc-lot-title d-flex justify-content-space-between">
            <p class="desc-lot-profile-title m-0">{{ trans("$theme-app.lot.property_owner") }}</p>
        </div>
...
@if ($obsdet_hces1)
    <div id="obs" class="container mb-2 mt-5">

        <div class="col-xs-12 no-padding ficha-tipo-v">
            <div class="col-xs-12 no-padding desc-lot-title d-flex justify-content-space-between">
                <p class="desc-lot-profile-title m-0">
                    {{ trans(\Config::get('app.theme') . '-app.subastas.auction_conditions') }}</p>
            </div>
            <div class="col-xs-12 no-padding desc-lot-profile-content mt-2">
                <p>{!! $obsdet_hces1 !!}</p>
            </div>
...
@include('front::includes.ficha.modals_ficha')

<script>
    $(document).ready(function() {

        $(".init-time-container .time-value").html(format_date_large(new Date($(
            ".init-time-container .time-value").data('date').replace(/-/g, "/")), ''));
        $(".finish-time-container .time-value").html(format_date_large(new Date($(
            ".finish-time-container .time-value").data('date').replace(/-/g, "/")), ''));
