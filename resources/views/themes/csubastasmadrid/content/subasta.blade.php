    ...
    <div class="row">
        <div class="col-xs-12 col-sm-12 title-head-grid">
            <div class="col-xs-5 col-sm-3 col-md-3 col-lg-3">
               @php /* Si quieren mostrar nombre de la subasta o que se vea texto Lotes */ @endphp
...
<div class="container">
    <div class="row">
		...
		<div class="col-xs-12 col-sm-8 col-md-9 mb-2 form-top-grid-container d-flex align-items-stretch hidden-xs">
			<div class="col-xs-12 col-lg-6 align-self-end button-auction-grid-container" style="flex: 1">
				@if($data['sub_data']->tipo_sub =='W' && strtotime($data['sub_data']->end) > time())
					<p class="text-center btn-color grid-buttons">
						<a class="btn btn-lot-grid" href="{{ \Routing::translateSeo('api/subasta').$data['sub_data']->cod_sub."-".str_slug($data['sub_data']->name)."-".$data['sub_data']->id_auc_sessions }}"
							target="_blank">{{ trans("$theme-app.subastas.lot_subasta_online") }}</a>
					</p>
				@endif
				@if(!Session::has('user'))
				<p class="text-center btn-color grid-buttons">
					<a class="btn btn-lot-grid"
