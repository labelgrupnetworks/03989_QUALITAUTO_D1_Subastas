            ...
            <div class="col-xs-12">
                ...
                <div class="panel-group" id="accordion">
                    <div class="panel panel-default">
                        ...
                        <div id="{{$all_inf['inf']->cod_sub}}"  class="table-responsive panel-collapse collaps in" style="padding: 10px;">
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th style="font-weight: 100; color: grey;">{{ trans(\Config::get('app.theme').'-app.user_panel.description') }}</th>

                                    @if($all_inf['inf']->tipo_sub != 'V')
                                        <th style="font-weight: 100; color: grey;" class="text-right">{{ trans(\Config::get('app.theme').'-app.user_panel.price') }}</th>
                                        <th style="font-weight: 100; color: grey;" class="text-right">{{ trans(\Config::get('app.theme').'-app.user_panel.price_comision') }}</th>
                                    @endif
                                    @if($all_inf['inf']->tipo_sub != 'V')
                                    <th style="font-weight: 100; color: grey;" class="text-right">{{ trans(\Config::get('app.theme').'-app.user_panel.price_clean') }}</th>
                                    @else
                                    <th style="font-weight: 100; color: grey;" class="text-right">{{ trans(\Config::get('app.theme').'-app.user_panel.sale_price') }}</th>
                                    @endif
                         ...
                         <!-- /Cabeceras grices con titulos-->
                          <?php $countBid=1; ?>
                            ...
                            <tbody>
                                <tr>
                                    @php($sub_ref = $inf_lot->cod_sub.'-'.$inf_lot->ref_asigl0)
                                    <th>
										{{-- Un lote que ya esta en factura no se puede pagar de manera individual --}}
										@if( empty(\Config::get( 'app.pasarela_web' )) && $inf_lot->fac_csub == 'N')
                                    	<input type="checkbox" checked="" id="add-carrito-{{$sub_ref}}" class="filled-in add-carrito form-control" name="carrito[{{$inf_lot->sub_csub}}][{{$inf_lot->ref_csub}}][pagar]" >
                                        <label for="add-carrito-{{$sub_ref}}"></label>
                                    @endif
                                    </th>
                                    <th>
                                        <div class="img-dat img-data-customs flex valign" role="button" id="">
                                            <img class="img-responsive" src="{{ \Tools::url_img("lote_medium", $inf_lot->num_hces1, $inf_lot->lin_hces1) }}" style="width: 50px;">
                                        </div>
                                    </th>
                                    <th>
                                    ...
                                    <th width="50%">
                                        ...
                                        <?= $inf_lot->desc_hces1?>
										@if( empty(\Config::get( 'app.pasarela_web' )) && $inf_lot->fac_csub != 'N')
										<small style="color: red">{{ trans("$theme-app.user_panel.to_make_payment") }}</small>
										@endif
                                    </div>
                        ...
                        @if($data['user']->envcorr_cli != 'B')
                                <div class="adj" >
                                    <span style="font-size: 23px;">{{ trans(\Config::get('app.theme').'-app.user_panel.env_seguro') }}:</span>
                                    <div class="adj-inline">
                                        <p class="titlecat" style="font-size: 24px;"><span class="text-gasto-envio-{{$all_inf['inf']->cod_sub}}">0</span> {{ trans(\Config::get('app.theme').'-app.lot.eur') }}</p>
                                    </div>
                                </div>
                                <div class="adj" >
                                    <span style="font-size: 23px;">{{ trans(\Config::get('app.theme').'-app.user_panel.total_price') }}:</span>
                                    <div class="adj-inline">
                                        <p class="titlecat" style="font-size: 24px;"><span class="precio_final_{{$all_inf['inf']->cod_sub}}">0</span> {{ trans(\Config::get('app.theme').'-app.lot.eur') }}</p>
                                        @if($all_inf['inf']->compraweb_sub == 'S')
                                        <button style="margin-left: 15px;" type="button" class="submit_carrito btn btn-step-reg2"  cod_sub="{{$all_inf['inf']->cod_sub}}" class="btn btn-step-reg" disabled>{{ trans(\Config::get('app.theme').'-app.user_panel.pay') }}</button>
                                        @endif
                                    </div>
                                </div>
                            @elseif($data['user']->envcorr_cli == 'B')
                                <div class="adj">
                                    <p style="color: #283747;font-size: 18px;">{{ trans(\Config::get('app.theme').'-app.user_panel.contact') }}</p></div>
                            @endif
                        </form>
                        </div>
                        @endforeach
