			...
			<div class="row">
				...
				<div class="col-xs-12 col-md-9 col-lg-9 ">
					...
					<div class="user-accounte-titles-link" data-id="collapse_pays">
						<ul class="panel-collapse ul-format d-flex justify-content-space-between align-items-center" role="tablist" role="button"
							data-toggle="collapse" href="#collapse_pays" aria-expanded="false" aria-controls="collapse_pays">
							<li role="pagar"class="active"><a class="color-letter" href="{{ \Routing::slug('user/panel/pending_bills') }}"
									style="text-transform: uppercase">{{ trans(\Config::get('app.theme') . '-app.user_panel.still_paid') }}</a></li>
							<span>
								<span class="label-open"
									style="display: none">{{ trans(\Config::get('app.theme') . '-app.user_panel.open') }}</span>
						...
						<div class="col-xs-12" style="padding-left:39px;">
							<div class="user-account-heading hidden-xs d-flex align-items-center justify-content-space-between">
								<div class="col-xs-12 col-sm-8  col-one user-account-item">
									{{ trans(\Config::get('app.theme') . '-app.user_panel.pending_bills') }}
								</div>
									...
									<div class="d-flex">
										...
										<div>
											<div class="importe_total adj" style="">
												...
												<span id="total_bills">00</span><span> {{ trans(\Config::get('app.theme') . '-app.subastas.euros') }}</span>
											</div>
											@if (\Config::get('app.PayBizum') || \Config::get('app.PayTransfer') || \Config::get('app.paymentPaypal'))
												...
												<div class="mt-1">
													...
													@if (\Config::get('app.paymentUP2') || Config::get('app.paymentRedsys'))
														...
														<label for="paycreditcard"> {{ trans(\Config::get('app.theme') . '-app.user_panel.pay_creditcard') }}
															<?php /* <span class="fab fa-cc-visa" style="font-size: 20px;"></span>*/ ?> </span></label>
														<br>
													@endif

													@if (\Config::get('app.PayBizum'))
														<input id="paybizum" type="radio" name="paymethod" value="bizum">
														<label for="paybizum"> {{ trans(\Config::get('app.theme') . '-app.user_panel.pay_bizum') }}
															<?php /*<img src="/default/img/logos/bizum-blue.png" style="height: 20px;">*/ ?> </label>
														<br>
													@endif
													...
													@if (\Config::get('app.paymentPaypal'))
														...
														<label for="paypaypal"><i class="fa fa-paypal" aria-hidden="true"></i>
															{{ trans(\Config::get('app.theme') . '-app.user_panel.pay_paypal') }} </label>
													@endif
											...
											<div class="mt-1 text-right">
												...
												<button id="submit_fact" style="" type="button"
													class="secondary-button ">{{ trans(\Config::get('app.theme') . '-app.user_panel.pay') }}</button>
					...
					<div class="user-accounte-titles-link col-xs-12 no-padding" data-id="collapse_fac_pag">
						<ul class="panel-collapse ul-format d-flex justify-content-space-between align-items-center" style="text-transform: uppercase"
							role="tablist" role="button" data-toggle="collapse" href="#collapse_fac_pag" aria-expanded="false"
							aria-controls="collapse_fac_pag">
							<li role="pagadas"><a class="color-letter"
									style="text-transform: uppercase">{{ trans(\Config::get('app.theme') . '-app.user_panel.bills') }}</a></li>
							<span>
								<span class="label-open">{{ trans(\Config::get('app.theme') . '-app.user_panel.open') }}</span>
								<span class="label-close"
									style="display: none">{{ trans(\Config::get('app.theme') . '-app.user_panel.hide') }}</span>
								<img width="10"
									src="data:image/svg+xml;utf8;base64," />
							</span>
						</ul>
					...
					<div class="col-xs-12 no-padding panel-collapse collapse" id="collapse_fac_pag" aria-expanded="false">
						<div class="col-xs-12" style="padding-left:39px;">
							<div class="user-account-heading hidden-xs d-flex align-items-center justify-content-space-between">
								...
								<div class="col-xs-12 col-sm-2 col-one user-account-max-bid text-right">
									{{ trans(\Config::get('app.theme') . '-app.user_panel.paid_out') }}
								</div>
							</div>
						</div>
