                ...
                <div class="data-price">
                    @if($item->retirado_asigl0 == 'N')
                        ...
                        @if($item->tipo_sub != 'V')
								...
								<span style="visibility: {{ $item->ocultarps_asigl0 != 'S' ? 'visible' : 'hidden'}}">{{ trans(\Config::get('app.theme').'-app.lot.lot-price') }}
									@if($precio_salida ==0)
										{{ trans(\Config::get('app.theme').'-app.lot.free') }}
									@else
										<span> {{$precio_salida}} {{ trans(\Config::get('app.theme').'-app.subastas.euros') }}</span></span>
									@endif
								@endif

                        @elseif($item->subc_sub !='H')
                            <span style="visibility: {{ $item->ocultarps_asigl0 != 'S' ? 'visible' : 'hidden'}}">{{ trans(\Config::get('app.theme').'-app.subastas.price_sale') }} <span>{{$precio_salida}}  {{ trans(\Config::get('app.theme').'-app.subastas.euros') }}</span></span>
                        @endif
						</p>
                        ...
                        @if(\Config::get('app.awarded'))
                            <p class="salida" data-position="button" style="height: 34px">
						...
						{{-- Por el momento no están utilizando este tipo de subastas, por lo que aprovecho el espacio --}}
						{{-- <p class="salida" data-position="clock">
                    ...
                    @else
						...
						<p class="salida" style="text-align:right" data-position="number-bids"></p>

                        @if(\Config::get('app.awarded'))
						<p class="salida" data-position="button" style="height: 34px"></p>
                        @endif

						{{-- <p class="salida" data-position="clock"></p> --}}
