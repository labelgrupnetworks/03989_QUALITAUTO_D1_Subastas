...
@section('content')
	@php
		#Define las variables iniciales para los totales de todas las subastas
		$totalLotes = 0;
		$totalPsalida = 0;
		$totalPremate = 0;
		$lotsSold = 0;
			...
			<div class="col-lg-9">
				...
				<div class="accordion">
					@foreach ($subastas as $cod_sub => $lotes)
						@php
							#Define las variables iniciales para los totales de cada subasta
							$totalLotesSub = 0;
							$totalPsalidaSub = 0;
							$totalPremateSub = 0;
							$lotsSoldSub = 0;
						...
						<div>
							<h2 class="accordion-item accordion-header" id="{{ $cod_sub }}-heading">
								<button class="accordion-button" data-bs-toggle="collapse" data-bs-target="#{{ $cod_sub }}-collapse"
									type="button" aria-expanded="true" aria-controls="{{ $cod_sub }}-collapse">
									{{ $lotes[0]->name }}
								</button>
							</h2>
							...
							<div class="accordion-collapse collapse show" id="{{ $cod_sub }}-collapse"
											...
											<tbody>
												@foreach ($lotes as $lote)
													@php
														...
														#Calcula los totales de todas las subastas
														$totalLotes++;
														$totalPsalida += $lote->impsalhces_asigl0;
														$totalPremate += $lote->implic_hces1;

														#Calcula los totales de cada subasta
														$totalLotesSub++;
														$totalPsalidaSub += $lote->impsalhces_asigl0;
														$totalPremateSub += $lote->implic_hces1;
													...
													<tr>
														<td class="td-img">
															<a href="{{ $url_friendly }}">
																<img class="img-fluid" src="{{ \Tools::url_img('lote_small', $lote->num_hces1, $lote->lin_hces1) }}">
															</a>
														</td>
														...
														<td>
															<div class="btn-group">
																<button class="btn btn-sm d-flex align-items-center p-2 rounded-circle" data-bs-toggle="dropdown"
								...
								@php
									// Tabla total de cada subasta
								@endphp

								<div class="ms-3 me-3">
									<h5 id="totals-{{ $cod_sub }}-heading">
										{{ trans("$theme-app.global.totals") }} {{ $lotes[0]->name }}
									</h5>
											...
											<tbody>
												<tr>
													...
													<td data-title="{{ trans("$theme-app.sheet_tr.in_auction") }}">
														{{ $lotsInAuctionSub }}</td>
													<td data-title="{{ trans("$theme-app.lot.lot-price") }}">
														{{ Tools::moneyFormat($totalPsalidaSub, trans("$theme-app.subastas.euros")) }}
													</td>
													<td data-title="{{ trans("$theme-app.user_panel.price") }}">
														{{ Tools::moneyFormat($totalPremateSub, trans("$theme-app.subastas.euros")) }}
													</td>
												</tr>
											</tbody>
										</table>
					...
					@php
						// Tabla total de todas las subastas
					@endphp

					<div>
						<h2 class="accordion-item accordion-header" id="totals-heading">
							<button class="accordion-button" data-bs-toggle="collapse" data-bs-target="#totals-collapse" type="button"
								aria-expanded="true" aria-controls="totals-collapse">
								Totales
							</button>
						</h2>

						<div class="accordion-collapse collapse show" id="totals-collapse" aria-labelledby="#totals-heading">
							<div class="accordion-body p-0">
								<div class="table-to-columns">
									<table class="table table-sm align-middle">
										<thead class="table-light">
