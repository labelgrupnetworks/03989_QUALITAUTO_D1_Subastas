		...
		@foreach ($all_adj as $key_sub => $all_inf)
			...
			<h2 class="accordion-item accordion-header" id="{{ $all_inf['inf']->cod_sub }}-heading">
				<button class="accordion-button" type="button" data-bs-toggle="collapse"
					data-bs-target="#pagar_lotes_{{ $all_inf['inf']->cod_sub }}" aria-expanded="true"
					aria-controls="pagar_lotes_{{ $all_inf['inf']->cod_sub }}">
					{{ $all_inf['inf']->name }}
			...
			<form id="pagar_lotes_{{ $all_inf['inf']->cod_sub }}" class="accordion-collapse collapse show"
				aria-labelledby="#{{ $all_inf['inf']->cod_sub }}-heading">
						...
						<tbody>
							@foreach ($all_inf['lotes'] as $inf_lot)
								...
								<tr>
									@if ($all_inf['inf']->compraweb_sub == 'S')
										<div class="hidden">
											<input type="checkbox" checked="" id="add-carrito-{{ $inf_lot->cod_sub }}-{{ $inf_lot->ref_asigl0 }}"
												class="filled-in add-carrito form-control"
												name="carrito[{{ $inf_lot->sub_csub }}][{{ $inf_lot->ref_csub }}][pagar]">
										</div>
									@endif
									<input class="hide envios_{{ $inf_lot->sub_csub }}_js" type="hidden"
										name="carrito[{{ $inf_lot->sub_csub }}][{{ $inf_lot->ref_csub }}][envios]"
										value='{{ Config::get('app.web_gastos_envio') ? '5' : '1' }}'>
									<input class="hide seguro_lote_{{ $inf_lot->sub_csub }}_js" type="hidden"
										name="carrito[{{ $inf_lot->sub_csub }}][{{ $inf_lot->ref_csub }}][seguro]" value='0'>
									<td class="td-img">
										<a href="{{ $url_friendly }}">
									...
									<td>
										<div class="btn-group">
											<button type="button" class="btn btn-sm d-flex align-items-center p-2 rounded-circle"
				...
				@if ($hasPaymentMethod)
					...
					@if (config('app.web_gastos_envio', false) && !empty($data['address']))
						<div class="row">
							<div class="col-sm-5 gastos_envio">
								...
								<select id="clidd_{{ $all_inf['inf']->cod_sub }}" name="clidd_{{ $all_inf['inf']->cod_sub }}"
									class="change_address_js " data-sub="{{ $all_inf['inf']->cod_sub }}" style="width: 90%;">
							...
							<div class="col-sm-4 gastos_envio">
								...
								<div id="envioPosible_{{ $all_inf['inf']->cod_sub }}_js">
									<input type="radio" checked="checked" class=" change_envio_js" data-sub="{{ $all_inf['inf']->cod_sub }}"
										id="envio_agencia_{{ $all_inf['inf']->cod_sub }}_js" name="envio_{{ $all_inf['inf']->cod_sub }}"
										value="1"> <label for="envio_agencia_{{ $all_inf['inf']->cod_sub }}_js">
										{{ trans(\Config::get('app.theme') . '-app.user_panel.gastos_envio') }}: <span
											id="coste-envio-{{ $all_inf['inf']->cod_sub }}_js"> </span>
										{{ trans(\Config::get('app.theme') . '-app.lot.eur') }}</label>

									@if (!empty(Config::get('app.porcentaje_seguro_envio')))
										<br>
										<input type="checkbox" style="top: 0px;height: 10px;margin-right: 0px;" class="check_seguro_js"
											data-sub="{{ $all_inf['inf']->cod_sub }}" id="seguro_{{ $all_inf['inf']->cod_sub }}_js"
											name="seguro_{{ $all_inf['inf']->cod_sub }}" value="1"> <label
											for="seguro_{{ $all_inf['inf']->cod_sub }}_js">
											{{ trans(\Config::get('app.theme') . '-app.user_panel.seguro_envio') }}: <span
												id="coste-seguro-{{ $all_inf['inf']->cod_sub }}_js"> </span>
											{{ trans(\Config::get('app.theme') . '-app.lot.eur') }}</label>
										<input type="hidden" id="porcentaje-seguro-{{ $all_inf['inf']->cod_sub }}_js"
											value="{{ \Config::get('app.porcentaje_seguro_envio') }}">
										<input type="hidden" id="iva_aplicable-{{ $all_inf['inf']->cod_sub }}_js"
											value="{{ $data['ivaAplicable'] }}">
									@endif
								...
								<input type="radio" class=" change_envio_js" data-sub="{{ $all_inf['inf']->cod_sub }}"
									id="recogida_almacen_{{ $all_inf['inf']->cod_sub }}_js" name="envio_{{ $all_inf['inf']->cod_sub }}"
									value="0"> <label
									for="recogida_almacen">{{ trans(\Config::get('app.theme') . '-app.user_panel.sala_almacen') }}</label>
					...
					<div class="total-price checkout mb-2">
						<div>
							...
							<h4><span class="precio_final_{{ $all_inf['inf']->cod_sub }}">0</span>
								{{ trans(\Config::get('app.theme') . '-app.lot.eur') }}</h4>
						</div>
						...
						@if (\Config::get('app.PayBizum') || \Config::get('app.PayTransfer') || \Config::get('app.paymentPaypal'))
							...
							<div class="btn-group gap-1" role="group" aria-label="Pay method">
								...
								@if (Config::get('app.paymentUP2') || Config::get('app.paymentRedsys'))
									<input type="radio" class="btn-check" name="paymethod" id="paycreditcard" value="creditcard"
								...
								@if (Config::get('app.PayBizum', false))
									<input type="radio" class="btn-check" name="paymethod" id="paybizum" value="bizum"
										autocomplete="off">
									<label class="btn btn-outline-lb-secondary" for="paybizum">
										<img src="/default/img/logos/bizum-blue.png" style="height: 20px;margin: 0px 6px;">
								...
								@if (Config::get('app.paymentPaypal', false))
									<input type="radio" class="btn-check" name="paymethod" id="paypaypal" value="paypal"
										autocomplete="off">
						...
						@else
							<input id="paytransfer" type="hidden" name="paymethod"
								value="{{ config('app.paymentPaypal', 0) ? 'paypal' : 'creditcard' }}">
						@endif
						...
						@if (\Config::get('app.urlToPackengers'))
							...
							<a class="packengers-button-adjudicaciones btn btn-outline-lb-secondary" href="{{ $urlCompletePackengers }}"
								target="_blank">
								<svg class="bi" width="16" height="16" fill="currentColor">
						...
						<button type="button" class="submit_carrito btn btn-lb-primary" cod_sub="{{ $all_inf['inf']->cod_sub }}"
							class="btn btn-step-reg" disabled>
