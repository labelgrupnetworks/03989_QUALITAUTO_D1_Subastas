        ...
        <div class="item_lot">
            ...
            <div class="data-price text-center">
                @if(!$retirado && !$fact_devuelta)
					...
					@elseif ($sub_historica)
						<p class="salida-title">{{ trans(\Config::get('app.theme').'-app.subastas.price_sale') }}</p>
                        <p>{{$item->formatted_impres_asigl0}}  {{ trans(\Config::get('app.theme').'-app.subastas.euros') }}</p>
                    @else
                        ...
                        <p style="visibility: {{ $item->ocultarps_asigl0 != 'S' ? 'visible' : 'hidden'}}">{{$precio_salida}} {{ trans(\Config::get('app.theme').'-app.subastas.euros') }}</p>
                    @endif
                    ...
                    @if( $awarded || $devuelto)
						...
						@elseif($sub_historica)
							<p class="btn-bid-lotlist btn-consult" data-lot="{{ $item->ref_asigl0 }}" data-toggle="modal" data-target="#modalConsulta">{{ trans("$theme-app.lot.consult") }}</p>
				...
				@else
                    ...
                    @else
                        ...
                        <p>{{$precio_salida}} {{ trans(\Config::get('app.theme').'-app.subastas.euros') }}</p>
                    @endif

					<p class="salida-title extra-color-one"></p>
                    <p class="salida-title extra-color-one"></p>

					<p class="btn-bid-lotlist">{{ trans(\Config::get('app.theme').'-app.lot.retired') }}</p>

                @endif

                </div>
                @if($subasta_online && !$cerrado && !$retirado && !$fact_devuelta)
