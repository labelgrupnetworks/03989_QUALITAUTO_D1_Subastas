			...
			<ul class="menu-principal-content d-flex justify-content-center align-items-center">
				...
				<?php
                  ...
                  if(empty($has_subasta) && Session::get('user.admin')){
				   ...
				   $has_history = $subastaObj->auctionList ('H');
				   $has_subasta_oh= array_merge($has_subasta,$has_history);
                ?>
				@if(!empty($has_subasta_oh))
				<li>
					<a class="color-letter d-flex link-header justify-content-center align-items-center" onclick="javascript:$('#menu_desp').toggle('blind',100)" style="cursor: pointer;">
					...
					<div id="menu_desp">
						@if(!empty($has_subasta))
						<a class="color-letter flex-display link-header link-header-second"
							href="{{ \Routing::translateSeo('subastas-online') }}"><span>{{ trans(\Config::get('app.theme').'-app.foot.online_auction')}}</span></a>
						@endif

						@if(!empty($has_history))
						<a class="color-letter flex-display link-header link-header-second"
							href="{{ \Routing::translateSeo('subastas-historicas') }}"><span>{{ trans(\Config::get('app.theme').'-app.foot.historico')}}</span></a>
						@endif
					</div>
				</li>
				@endif
                ...
                * */
				 ...
				 <li><a class="color-letter flex-display link-header justify-center align-items-center"
					href="https://www.sorolla.com/"><span>{{ trans(\Config::get('app.theme').'-app.foot.direct_sale')}}</span></a>
				</li>
				<li>
					<a class="color-letter flex-display link-header justify-center align-items-center" title=""
						href="{{ \Routing::translateSeo('valoracion-articulos', null) }}"><span>
							{{ trans(\Config::get('app.theme').'-app.home.free-valuations') }}</span></a>
				</li>
				<li>
					<a class="color-letter flex-display link-header justify-center align-items-center"
						title="{{ trans(\Config::get('app.theme').'-app.foot.how_to_buy') }}"
						href="{{ Routing::translateSeo('pagina').trans(\Config::get('app.theme').'-app.links.how_to_buy')  }}"><span>{{ trans(\Config::get('app.theme').'-app.foot.how_to_buy') }}</span></a>
				</li>
				<li>
					<a class="color-letter flex-display link-header justify-center align-items-center"
						title="{{ trans(\Config::get('app.theme').'-app.foot.how_to_sell') }}"
						href="<?php echo Routing::translateSeo('pagina').trans(\Config::get('app.theme').'-app.links.how_to_sell')  ?>"><span>{{ trans(\Config::get('app.theme').'-app.foot.how_to_sell') }}</span></a>
				</li>
				<li>
					<a class="color-letter d-flex link-header justify-content-center align-items-center"
