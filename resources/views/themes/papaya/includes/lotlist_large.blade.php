...
<div class="{{$class_square}} large_square hidden">
            ...
            <div class="col-xs-12 col-sm-7 col-lg-8">
                <div class="data-container">
                        ...
                        <div class="title_item">
                            <span class="seo_h4">{{ trans("$theme-app.lot.lot-name") }} {{ $item->ref_asigl0 }} - {{ $titulo }}</span>
                        </div>
                        <div class="desc_lot">
                            <?= $item->desc_hces1 ?>
                        </div>
				 ...
				 @if( !$retirado && !$fact_devuelta)
                    ...
                    @if($subasta_venta)
                            <p class="salida-title">
                                {{ trans(\Config::get('app.theme').'-app.subastas.price_sale') }}:<span> {{\Tools::moneyFormat( $item->imptas_asigl0, false, 2) }} {{ trans(\Config::get('app.theme').'-app.subastas.euros') }}</span>
                            </p>
					@else
						@if(!$cerrado)
                         <p class="salida-title">{{ trans(\Config::get('app.theme').'-app.lot.lot-price') }}:<span> {{\Tools::moneyFormat( $item->imptas_asigl0, false, 2) }}{{ trans(\Config::get('app.theme').'-app.subastas.euros') }}</span></p>
						@endif
					@endif
					{{--
