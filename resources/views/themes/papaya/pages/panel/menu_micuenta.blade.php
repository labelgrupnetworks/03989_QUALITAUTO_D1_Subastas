<div class="tabs-custom filters-auction-content">
	<div id="button-open-user-menu" class="tabs-custom-responsive visible-xs visible-sm"><i
	...
	<ul id="user-account-ul" class="ul-format color-letter">
		<li class="text-uppercase<?php if($tab == 'orders'){ echo(' tab-active'); } ?>" role="presentation">
			...
			<a class="" data-title="{{ trans(\Config::get('app.theme').'-app.panel.my_orders') }}"
				...
				<img width="20px" src="/themes/{{\Config::get('app.theme')}}/assets/img/hammer-grey.png"
					alt="{{ trans(\Config::get('app.theme').'-app.panel.my_orders') }}">
		...
		<li class="text-uppercase @if($tab == 'allotments') tab-active @endif" role="presentation">
			<a class="" href="{{ \Routing::slug('user/panel/allotments') }}"
				data-title="{{ trans(\Config::get('app.theme').'-app.panel.my_adj') }}">
		...
		<li class="text-uppercase<?php if($tab == 'datos-personales'){ echo(' tab-active'); } ?>" role="presentation">
			<a class="" href="{{ \Routing::slug('user/panel/info') }}">
				...
				{{ trans(\Config::get('app.theme').'-app.user_panel.info') }}
			</a>
		</li>

		<li class="text-uppercase @if($tab == 'represented') tab-active @endif" role="presentation">
			<a class="" href="{{ route('panel.represented.list', ['lang' => Config::get('app.locale')]) }}">
				<i class="fa fa-users"></i>
				{{ trans("$theme-app.user_panel.represented_link") }}
			</a>
		</li>

		<li class="text-uppercase"><a href="{{ \Routing::slug('logout') }}"><i
					class="fa fa-sign-out-alt"></i>{{ trans(\Config::get('app.theme').'-app.user_panel.exit') }}</a>
		</li>
	</ul>
</div>
