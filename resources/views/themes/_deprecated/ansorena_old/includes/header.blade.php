	...
	<nav class="menu-header">
		...
		<div class="menu-principal">
			...
			<ul class="menu-principal-content d-flex justify-content-center align-items-start">
				...
				@if(count($has_subasta)== 1)
				@php
				$subasta = head($has_subasta);
				$url_web = \Tools::url_auction($subasta->cod_sub,$subasta->name,$subasta->id_auc_sessions,
				$subasta->reference);
				@endphp
				@else
				@php $url_web = \Routing::translateSeo('presenciales'); @endphp
				@endif
				<li>
				...
				@php
				$subasta = head($has_subasta);
				$url_online = \Tools::url_auction($subasta->cod_sub,$subasta->name,$subasta->id_auc_sessions,
				$subasta->reference);
				@endphp
				@else
				@php $url_online = \Routing::translateSeo('subastas-online'); @endphp
				@endif
				<li>
					<a class="color-letter flex-display link-header justify-center align-items-center"
						href="{{ $url_online }}"><span>{{ trans(\Config::get('app.theme').'-app.foot.online_auction')}}</span></a>
				</li>
				@endif
				<?php
