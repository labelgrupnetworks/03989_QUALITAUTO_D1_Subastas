                    ...
                    <div class="data-price">
                        <div class="row">
                            <div class="col-xs-4 text-center">
                                ...
                                @else
                                <p class="salida-title">{{ trans(\Config::get('app.theme').'-app.lot.lot-price') }}
                                    <br>
                                    <span> {{$item->formatted_impsalhces_asigl0}} {{ trans(\Config::get('app.theme').'-app.subastas.euros') }}</span>
                                    <div class="js-divisa" value="{{ $item->impsalhces_asigl0 }}"></div>
                                </p>
                            ...
                            <div class="col-xs-4 text-center">
                                ...
                                @elseif ($subasta_web && $subasta_abierta_O && !empty($item->open_price) && !$cerrado)
                                        ...
                                        <p class="salida-title">{{ trans(\Config::get('app.theme').'-app.lot.puja_actual') }}
                                        <br>
                            ...
                            <div class="col-xs-4 text-center">
                                @if($devuelto)
                                <p class="salida-title">{{ trans(\Config::get('app.theme').'-app.subastas.dont_available') }}</p>
                                @elseif($cerrado && $remate &&  (!empty($precio_venta) ) || ($sub_historica && !empty($item->impadj_asigl0)) )    
                                    ...
                                    @endif
                                <p class="salida-title">{{ trans(\Config::get('app.theme').'-app.subastas.buy_to') }}
                                    <br>
                                    <span class="pill">{{ \Tools::moneyFormat($precio_venta) }} {{ trans(\Config::get('app.theme').'-app.subastas.euros') }}</span>
