...
<div id="reload_inf_lot" class="col-xs-12 info-ficha-buy-info no-padding">
	<div class="col-xs-12">
		...
		<div class=" col-xs-4 info-ficha-buy-info-price">
			<div class="pre">
				...
				<p class="pre-price">{{$lote_actual->formatted_impsalhces_asigl0}} {{ trans(\Config::get('app.theme').'-app.subastas.euros') }} 
					<div value="{{$lote_actual->impsalhces_asigl0}}" class="js-divisa"></div>
				</p>
			</div>
		</div>
		@if (!empty($lote_actual->imptas_asigl0))
		<div class=" col-xs-4 no-padding info-ficha-buy-info-price d-flex">
			<div class="pre">
				...
				<p class="pre-price">{{ \Tools::moneyFormat($lote_actual->imptas_asigl0)}} {{ trans(\Config::get('app.theme').'-app.subastas.euros') }}
					<div value="{{$lote_actual->imptas_asigl0}}" class="js-divisa"></div>
				</p>
			</div>				  
		</div>
		@endif
...
@if ($lote_actual->tipo_sub == 'W' && $lote_actual->subabierta_sub == 'O' && $lote_actual->cerrado_asigl0 == 'N'  ) 
        <div class="price">
            <span id="text_actual_max_bid" class="pre <?=  $lote_actual->open_price >0? '':'hidden' ?>">{{ trans(\Config::get('app.theme').'-app.lot.puja_actual') }}</span>
            <span id="actual_max_bid" class="pre <?= (count($data['ordenes']) > 0 && head($data['ordenes'])->cod_licit == $data['js_item']['user']['cod_licit'])? 'winner':'no_winner' ?>">{{\Tools::moneyFormat($lote_actual->open_price) }}{{trans(\Config::get('app.theme').'-app.subastas.euros')}}</span>
            @if (isset($data['js_item']['user']))
                <small class="winner <?= (count($data['ordenes']) > 0 && head($data['ordenes'])->cod_licit == $data['js_item']['user']['cod_licit'])? '':'hidden' ?>" ><?php /* trans(\Config::get('app.theme').'-app.subastas.exceeded') */ ?></small> 
                <small  class="no_winner <?= (count($data['ordenes']) > 0 && head($data['ordenes'])->cod_licit == $data['js_item']['user']['cod_licit'])? 'hidden':'' ?>"><?php /* trans(\Config::get('app.theme').'-app.subastas.not_exceeded') */ ?></small>
            @endif
        </div>
	...
	<div class="col-xs-12 no-padding info-ficha-buy-info-price border-top-bottom">            
		<div class="pre d-flex mt-1 ">
			<div  id="text_actual_no_bid" class="price-title-principal pre col-xs-12 col-sm-4 no-padding <?=  count($lote_actual->pujas) >0? 'hidden':'' ?>"> {{ trans(\Config::get('app.theme').'-app.lot_list.no_bids') }} </div>
			<div class="col-xs-12 col-sm-8 no-padding">
			@if ($hay_pujas)
					<span class='explanation_bid t_insert pre-title' >{{ trans(\Config::get('app.theme').'-app.lot.next_min_bid') }}  </span>
					&nbsp;&nbsp;&nbsp;
					<strong><span class="siguiente_puja">{{ trans(\Config::get('app.theme').'-app.subastas.euros') }} </span></strong> 
				@else
					<span class='explanation_bid t_insert pre-title'>{{ trans(\Config::get('app.theme').'-app.lot.min_puja') }}  </span>
					&nbsp;&nbsp;&nbsp;
					<strong><span class="siguiente_puja">{{ trans(\Config::get('app.theme').'-app.subastas.euros') }} </span></strong>
				@endif 
			</div>
		...
		<div class="insert-bid-input no-padding col-xs-12 d-flex justify-content-center flex-column">
			...
			<div class="input-group d-block group-pujar-custom ">
				...
				<div class="input-group-btn">
					<button type="button" data-from="modal" class="<?php if (\Session::has('user')) {echo 'lot-action_pujar_on_line';} else {echo 'signIn';}?> ficha-btn-bid ficha-btn-bid-height button-principal <?= Session::has('user')?'add_favs':''; ?>" type="button" ref="{{ $lote_actual->ref_asigl0 }}" ref="{{ $lote_actual->ref_asigl0 }}" codsub="{{ $lote_actual->cod_sub }}" >{{ trans(\Config::get('app.theme').'-app.lot.pujar') }}</button>
				</div>
			</div>
