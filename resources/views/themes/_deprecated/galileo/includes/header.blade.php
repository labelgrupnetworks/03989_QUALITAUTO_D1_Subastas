                    ...
                    <div class="google_translate2">
                        ...
                        <script type="text/javascript">
                            function googleTranslateElementInit() {
                                new google.translate.TranslateElement({pageLanguage: 'es', includedLanguages: 'es,de,en,it,ja,zh-TW', layout: google.translate.TranslateElement.InlineLayout.SIMPLE}, 'google_translate_element');
                            }
                        </script>
                    ...
                    <?php foreach(Config::get('app.locales') as $key => $value) { ?>
                        <ul class="ul-format list-lang d-inline-flex">
                            ...
                            @if(\App::getLocale() != $key)
                                <li>
                                    <a title="<?= trans(\Config::get('app.theme').'-app.head.language_es') ?>" class="link-lang color-letter" href="/<?=$key;?>">
                                        <?=$key;?>
                                    </a>
                                </li>
                            @else
                            <li>
                                <a title="<?= trans(\Config::get('app.theme').'-app.head.language_es') ?>" class="link-lang active color-letter">
                                    <?=$key;?>
                                </a>
                            </li>
                            @endif
            ...
            <ul class="menu-principal-content d-flex justify-content-center align-items-center">
                ...
                <?php //   <li><a title="{{ trans(\Config::get('app.theme').'-app.home.home')}}" href="/">{{ trans(\Config::get('app.theme').'-app.home.home')}}</a></li> ?>
                    <li class="flex-display">
                        <a class="color-letter-header flex-display link-header justify-center align-items-center" title="{{ trans(\Config::get('app.theme').'-app.home.home')}}" href="/">
                ...
                @if(!empty($has_subasta) && $not_finished)
                  <li>
                      <a class="color-letter-header d-flex link-header justify-content-center align-items-center" href="{{ \Routing::translateSeo('presenciales') }}?finished=false">
                ...
                @if(!empty($has_subasta) && $finished)
                  <li>
                        <a class="color-letter-header d-flex link-header justify-content-center align-items-center" href="{{ \Routing::translateSeo('presenciales') }}?finished=true">
                            <span style="text-align: center;"><?= trans(\Config::get('app.theme').'-app.foot.auctions-finished') ?></span>
                        </a>
                    </li>
                @endif
                ...
                @if(!empty($has_subasta))
                    <li>
                        <a class="color-letter-header flex-display link-header justify-center align-items-center" href="{{ \Routing::translateSeo('subastas-online') }}"><span>{{ trans(\Config::get('app.theme').'-app.foot.online_auction')}}</span></a>
                    </li>
                @endif
                <li>
                    <a class="color-letter-header flex-display link-header justify-center align-items-center" title=""  href="{{ \Routing::translateSeo('subastas-historicas') }}">{{ trans(\Config::get('app.theme').'-app.foot.historico')}}</a>
                </li>
                ...
                ?>
                @if(!empty($has_subasta))
                    <li><a class="color-letter-header flex-display link-header justify-center align-items-center" href="{{ \Routing::translateSeo('venta-directa') }}"><span>{{ trans(\Config::get('app.theme').'-app.foot.direct_sale')}}</span></a></li>
                @endif
               <?php /*
                ...
                * */
                  ?>
	      <li>
                    <a class="color-letter-header flex-display link-header justify-center align-items-center" title="" href="{{ \Routing::translateSeo('calendar') }}"><span>{{ trans(\Config::get('app.theme').'-app.foot.calendar')}}</span></a>
                </li>
                 <li>
                    <a class="color-letter-header flex-display link-header justify-center align-items-center" title="" href="{{ \Routing::translateSeo('valoracion-articulos') }}"><span> {{ trans(\Config::get('app.theme').'-app.home.free-valuations') }}</span></a>
                </li>
                <?php /*
                <li>
                    <a class="color-letter-header d-flex link-header justify-content-center align-items-center" title="{{ trans(\Config::get('app.theme').'-app.foot.about_us')}}" href="<?= \Routing::translateSeo('pagina').trans(\Config::get('app.theme').'-app.links.about_us')?>"><span>{{ trans(\Config::get('app.theme').'-app.foot.about_us')}}</span></a>
                </li>

                */ ?>
                <li>
                    <a class="color-letter-header d-flex link-header justify-content-center align-items-center" title="{{ trans(\Config::get('app.theme').'-app.foot.contact')}}" href="<?= \Routing::translateSeo(trans(\Config::get('app.theme').'-app.links.contact') )?>"><span>{{ trans(\Config::get('app.theme').'-app.foot.contact')}}</span></a>
                </li>
            </ul>
        </div>
        <div class="search-header-container  d-flex justify-content-center align-items-center hidden-xs" role="button">
                <div class="search-header d-flex justify-content-center align-items-center flex-column">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 29.17 29.861">
                  ...
                  <div class="btn-searc-text">Buscar</div>
                </div>
        ...
        <div class="user-account">
                @if(!Session::has('user'))
                    <div class="user-account-login">
                        <a class="flex-display justify-center align-items-center btn_login_desktop btn_login color-letter-header" title="<?= trans(\Config::get('app.theme').'-app.login_register.login') ?>" href="javascript:;">
                    ...
                    @else
                    <a class="my-account color-letter-header d-flex" style='background: #CBB997; max-width: 175px;' >
                        ...
                        @if(!empty($name))
                            ...
                            <div class="hidden-xs">{{ trans(\Config::get('app.theme').'-app.login_register.welcome')}}</div>
                            <div class="hidden-xs" style='font-size: 10px'><?= $name ?></div>
                            </span>
                        @else
                            <span class="hidden-xs" style='font-size: 10px'>{{ trans(\Config::get('app.theme').'-app.login_register.my_panel') }}</span>
                        @endif
                        </div>
...
<div class="menu-principal-search d-flex align-items-center justify-content-center">
        <form id="formsearchResponsive" role="search" action="{{ \Routing::slug('busqueda') }}" class="search-component-form flex-inline">
                ...
                <button role="button" type="submit" class="btn btn-custom-search background-principal">{{ trans(\Config::get('app.theme').'-app.head.search_button') }}</button>
            </form>
</div>
    ...
    <div class="login_desktop_content">
        <div class="only-login">
            ...
            <div class="login_desktop_title">
                <?= trans(\Config::get('app.theme').'-app.login_register.generic_name') ?>
            </div>
            ...
            <div class="create-account-link">
                <a class="color-letter" style="color: #101010;text-transform: uppercase; " title="{{ trans(\Config::get('app.theme').'-app.login_register.register') }}" href="{{ \Routing::slug('login') }}">{{ trans(\Config::get('app.theme').'-app.login_register.register') }}</a>
            </div>
        </div>
    </div>
    </div>
...
<div id='not-register-error' style='display:none'>{{ trans(\Config::get('app.theme').'-app.msg_error.not_register') }}</div>
