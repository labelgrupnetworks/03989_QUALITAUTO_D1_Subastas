...
@endif
@php ($bread[] = array("name" =>trans(\Config::get('app.theme').'-app.home.calendar')))

<div class="container">
        <div class="row">
            <div class="col-xs-12 col-sm-12 text-center color-letter">
                <h1 class="titlePage">{{ trans(\Config::get('app.theme').'-app.home.calendar') }}</h1>
...
<div class="container content calendar-page">
    <div class="row">
        ...
        <div class='col-md-3'>
            @foreach($data as $sub)
                @if($sub->tipo_sub == 'W' && strtotime($sub->session_end) > time())
                <div class="bs-callout bs-callout-default" id="callout-alerts-dismiss-plugin" <?=!empty($sub->colorcal_sub)?'style="border-left-color: '.$sub->colorcal_sub.'"':''?>> 
                    <h5>{{$sub->des_sub}}</h5> 
                    <?php /* ocultamos las ordenes
                    <p>{{ trans(\Config::get('app.theme').'-app.calendar.start_orders') }} {{ date("d/m/Y H:m:s", strtotime($sub->orders_start))}} </p> 
                    <p>{{ trans(\Config::get('app.theme').'-app.calendar.end_orders') }} {{ date("d/m/Y H:m:s", strtotime($sub->orders_end))}} </p> 
        ...
        <?php foreach($data as $sub){
                  ...
                  ?>
            {
                ...
                color: '<?= !empty($sub->colorcal_sub)?$sub->colorcal_sub:'#777' ?>',
                    ...
                    <?php 
                    //De momento noeseñamos las ordenes
                    /*
            {
                title: '{{ trans(\Config::get('app.theme').'-app.calendar.orders') }} <?= $sub->des_sub ?>',
                start: '<?= $sub->orders_start ?>',
                end: '<?= $sub->orders_end ?>',
                color: '<?= !empty($sub->colorcal_sub)?$sub->colorcal_sub:'#777' ?>'
            },
                    */?>
