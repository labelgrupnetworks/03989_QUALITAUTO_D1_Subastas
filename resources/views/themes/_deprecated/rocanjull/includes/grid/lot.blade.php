            ...
            <div class="data-container">
				...
				@if (!$devuelto && !$retirado && !$sub_historica)
					...
					@elseif($subasta_venta  && !$cerrado)
						@if(!$end_session && $item->opcioncar_sub == 'N')
							<p class="btn-bid-lotlist">{{ trans(\Config::get('app.theme').'-app.subastas.buy_lot') }}</p>
						@elseif(!$end_session && $item->opcioncar_sub == 'S')
							<p style="background: red" class="btn-bid-lotlist">{{ trans(\Config::get('app.theme').'-app.subastas.contact') }}</p>
						@endif
					@elseif(!$cerrado )
