 <div class="sidebar_lot">
	...
	<form id="form_lotlist" method="get" action="{{ $data['url'] }}">
		<input type="hidden" name="only_salable" value="<?= request("only_salable") ?>">
		<input type="hidden" name="no_award" value="<?= request("no_award") ?>">
		<?php $cod_sec_url = request("cod_sec")?>
        ...
        <div class="form-group">
            ...
            <input id="input_reference" placeholder="{{ trans(\Config::get('app.theme').'-app.lot_list.reference') }}" name="reference" type="text" class="form-control input-sm" value="{{ app('request')->input('reference') }}">
                    ...
                    <option value="ref" @if (empty(app('request')->input('order')) || app('request')->input('order') == 'ref_desc') selected @endif >
...
<?php
    ...
    $subasta->where_filter = "AND ORTSEC1.SUB_ORTSEC1 = ASIGL0.SUB_ASIGL0  AND \"id_auc_sessions\" =  ".  $data['id_auc_sessions']. " AND SEC.BAJAT_SEC = 'N' AND SEC.GEMP_SEC = '". Config::get('app.gemp')."'";
	if(request("only_salable")=='on'){
		$subasta->where_filter .= " AND (SUB.TIPO_SUB != 'V' OR cerrado_asigl0 = 'N') AND ASIGL0.RETIRADO_ASIGL0 = 'N' AND  HCES1.FAC_HCES1 = 'N'";
	}
                        ...
                        @if( count($subcategories) > 0)
                         <label>{{ trans(\Config::get('app.theme').'-app.lot_list.subcategories') }}  </label>
                            <select class="form-control " id="subcategory" name="cod_sec" >
                                   <option value="" >{{ trans(\Config::get('app.theme').'-app.lot_list.all_subcat') }} </option>
                                       @foreach($subcategories as $subcategory)
                                           <option value="{{$subcategory->cod_sec}}" @if($cod_sec_url == $subcategory->cod_sec) selected  @endif >{{ ucfirst(mb_strtolower(trim ($subcategory->des_sec)))}} ({{$subcategory->cuantos}})  </option>
                                       @endforeach
                            </select>
                        @endif
                    @endif
