...
@if(strtoupper($data['subasta_info']->lote_actual->tipo_sub) == 'O' || strtoupper($data['subasta_info']->lote_actual->tipo_sub) == 'P')
...
$lote_actual = $data['subasta_info']->lote_actual;
    ...
    if(!empty($lote_actual)){
		if($lote_actual->tipo_sub == "V"){
			$lote_actual->url_subasta.="?only_salable=on";
		}elseif(strtotime($lote_actual->end_session) < time()){
			$lote_actual->url_subasta.='?no_award=on';
		}
