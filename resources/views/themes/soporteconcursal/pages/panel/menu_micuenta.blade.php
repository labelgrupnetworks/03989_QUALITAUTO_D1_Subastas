	...
	<ul id="user-account-ul" class="ul-format color-letter">
		<li class="text-uppercase<?php if($tab == 'orders'){ echo(' tab-active'); } ?>" role="presentation">
			...
			<a class="" data-title="{{ trans(\Config::get('app.theme').'-app.panel.my_orders') }}"
				...
				@else
				<img width="20px" src="/themes/{{\Config::get('app.theme')}}/assets/img/hammer.png"
					alt="{{ trans(\Config::get('app.theme').'-app.panel.my_orders') }}">
				@endif
		...
		<?php /*<li role="presentation" <?php if($tab == 'datos-personales'){ echo('class="active"'); } ?>><a
			href="{{ \Routing::slug('user/panel/info') }}">{{ trans(\Config::get('app.theme').'-app.user_panel.info') }}</a>
		</li> */?>
		<li class="text-uppercase"><a href="{{ \Routing::slug('logout') }}"><i
					class="fas fa-sign-out-alt"></i>{{ trans(\Config::get('app.theme').'-app.user_panel.exit') }}</a>
		</li>
	</ul>
</div>
