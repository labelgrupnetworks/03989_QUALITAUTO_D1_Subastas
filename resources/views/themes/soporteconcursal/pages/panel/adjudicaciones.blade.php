...
/* En principio no tendran adjudicaciones pagadas
			...
			<div class="col-xs-12 col-md-9 col-lg-9 ">
				<div class="user-account-title-content mb-3">
					<div class="user-account-menu-title">
						{{ trans(\Config::get('app.theme').'-app.user_panel.allotments') }}</div>
				</div>
				...
				<div class="col-xs-12 no-padding in" aria-expanded="true" id="lot_pend">
					...
					<div class="panel-group" id="accordion">
                            ...
                            ?>
								<div class="panel-heading">
										...
										<a class="d-flex justify-content-space-between" data-toggle="collapse"
											...
											<div>
												...
												<span class="total-price-sup"> /
													{{ trans(\Config::get('app.theme').'-app.user_panel.total_price') }}
													<span class="precio_final_{{$all_inf['inf']->cod_sub}}">0</span>
									...
									<div id="{{$all_inf['inf']->cod_sub}}"
										class="table-responsive panel-collapse collaps {{$all_inf['inf']->compraweb_sub == 'S' ? 'in': 'collapse'}}">
										@if(!$tipo_pago_global)
										<form id="pagar_lotes_{{$all_inf['inf']->cod_sub}}">
											...
											@if(!$tipo_pago_global)
											<div
												class="adj color-letter d-flex align-items-center justify-content-space-between">
												@if($all_inf['inf']->compraweb_sub == 'S' &&
												!empty(Config::get('app.merchantIdUP2')))
												<div class="total-price">
													{{ trans(\Config::get('app.theme').'-app.user_panel.total_price') }}
													<span class="precio_final_{{$all_inf['inf']->cod_sub}}">0</span>
													{{ trans(\Config::get('app.theme').'-app.lot.eur') }} </div>
												<button style="margin-left: 15px;" type="button"
													class="secondary-button   submit_carrito btn btn-step-reg2"
													cod_sub="{{$all_inf['inf']->cod_sub}}" class="btn btn-step-reg"
                                            ...
                                            ?>
												<div class="user-accout-item-wrapper  col-xs-12 no-padding">
													<div class="d-flex">
														<div
															class="col-xs-12 col-sm-7 col-lg-8 col-one user-account-item ">
															<div class="col-xs-12 col-sm-1 no-padding">
																<div class="checkbox" style="margin-top: 0px;">
																	@if($all_inf['inf']->compraweb_sub == 'S')
																	<input type="checkbox" checked=""
																		id="add-carrito-{{$inf_lot->cod_sub}}-{{$inf_lot->ref_asigl0}}"
															...
															<div class="col-xs-12 col-sm-8 col-sm-offset-1 no-padding">
																@if(strtoupper($inf_lot->tipo_sub) == 'O' ||
																strtoupper($inf_lot->tipo_sub) == 'P')
														...
														<div class="col-xs-12 col-sm-2 col-lg-2 account-item-border">
															<div
																class="user-account-item-date d-flex flex-direction-column align-items-center justify-content-center">
																...
																<small
																	class="comision-title">{{ trans(\Config::get('app.theme').'-app.user_panel.price_comision') }}</small>
																<div>+ <?=  $comision ?>
																	{{ trans(\Config::get('app.theme').'-app.lot.eur') }}
																</div>
															</div>
														...
														<div class="col-xs-12 col-sm-3 col-lg-2 account-item-border">
															<div
																class="user-account-item-price  d-flex align-items-center justify-content-center">

																<div class="visible-xs">
																	{{ trans(\Config::get('app.theme').'-app.user_panel.mi_puja') }}
																</div>
																<div><strong><?= \Tools::moneyFormat($precio_limpio_calculo,false,2); ?>
																		{{ trans(\Config::get('app.theme').'-app.lot.eur') }}</strong>
																</div>
															</div>
														</div>
						...
						<div class="adj color-letter d-flex align-items-center justify-content-space-between">
							<div class="total-price">
								{{ trans(\Config::get('app.theme').'-app.user_panel.total_price') }} <span
									class="precio_final_global">0</span>
								{{ trans(\Config::get('app.theme').'-app.lot.eur') }} </div>
							@if(!empty(Config::get('app.merchantIdUP2')))
							<button style="margin-left: 15px;" type="button"
								class="secondary-button   submit_carrito btn btn-step-reg2" cod_sub="global"
