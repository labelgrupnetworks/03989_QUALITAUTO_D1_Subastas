    ...
    <div class="col-xs-12">
        <div class="info_single_title hist_new <?= !empty($data['js_item']['user']['ordenMaxima'])?'':'hidden'; ?> ">
            ...
            <strong>
                ...
                {{trans(\Config::get('app.theme').'-app.subastas.euros')}}</strong><br><br>
        </div>
	</div>

	<!-- precios -->
    <div class=" col-xs-12 no-padding info-ficha-buy-info-price d-flex info-ficha-prices">
            <div class="pre">
                <p class="pre-title">{{ trans(\Config::get('app.theme').'-app.subastas.price_sale') }}</p>
                <p class="pre-price">{{$lote_actual->formatted_imptas_asigl0}} {{ trans(\Config::get('app.theme').'-app.subastas.euros') }} </p>
			</div>

            <div class="pre">
                <p class="pre-title">{{ trans(\Config::get('app.theme').'-app.lot.estimate') }}</p>
                <p class="pre-price">{{ !empty($lote_actual->formatted_impres_asigl0) ? $lote_actual->formatted_impres_asigl0 : 0 }} {{ trans(\Config::get('app.theme').'-app.subastas.euros') }} </p>
			</div>
	</div>

	<!-- porcentajes -->
	<div class=" col-xs-12 no-padding info-ficha-buy-info-price d-flex info-ficha-percents">
		<div class="pre">
			<p class="pre-title">{{ trans(\Config::get('app.theme').'-app.user_panel.price_comision') }}</p>
			<p class="pre-price">{{$lote_actual->comision}} {{ trans(\Config::get('app.theme').'-app.lot.percentage') }} </p>
		</div>

		{{-- Los depositos los dejamos desactivados, pendientes de crear la columna de deposito y querer añadirlo por parte del cliente
		<div class="pre">
			<p class="pre-title">{{ trans(\Config::get('app.theme').'-app.lot.deposit') }}</p>
			<p class="pre-price">{{ $lote_actual->deposito_asigl0 ?? 0 }} {{ trans(\Config::get('app.theme').'-app.lot.percentage') }} </p>
		</div>
		--}}
	</div>
    ...
    <div class="col-xs-12 no-padding info-ficha-buy-info-price border-top-bottom">
        <div class="pre d-flex mt-2 mb-2 ">
            ...
            @if ($hay_pujas)
                    ...
                    <strong><span class="siguiente_puja"> </span>{{ trans(\Config::get('app.theme').'-app.subastas.euros') }}</strong>
                @else
        ...
        <div class="insert-bid-input col-lg-10 col-lg-offset-1 d-flex justify-content-center flex-column">
			...
			@if ( strtotime($lote_actual->start_session) <  strtotime("now") )
