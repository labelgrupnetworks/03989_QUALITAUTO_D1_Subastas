...
//Las adjudicaciones de venta directa siempre estaran pagadas.
$vd_adj = array();

//dd($data['adjudicaciones'], $data['adjudicaciones_pag']);
foreach(collect($data['adjudicaciones_pag'])->where('tipo_sub', 'V') as $temp_adj){
	$temp_adj->paid = true;
    $vd_adj[$temp_adj->cod_sub]['lotes'][] = $temp_adj;
}
foreach($vd_adj as $key_inf => $value){
    $sub->cod = $key_inf;
    $vd_adj[$key_inf]['inf'] = $sub->getInfSubasta();
}

//adjudicaciones subastas estaran las pagadas y las no pagadas juntas
foreach(collect($data['adjudicaciones'])->where('tipo_sub', 'O') as $temp_adj){
	$temp_adj->paid = false;
	$all_adj[$temp_adj->cod_sub]['lotes'][] = $temp_adj;
}
foreach(collect($data['adjudicaciones_pag'])->where('tipo_sub', 'O') as $temp_adj){
	$temp_adj->paid = true;
    $all_adj[$temp_adj->cod_sub]['lotes'][] = $temp_adj;
}
...
<script>
    var info_lots = @json($data["js_item"]);   //$.parseJSON('<?php echo str_replace("\u0022","\\\\\"",json_encode($data["js_item"],JSON_HEX_QUOT)); ?>');
</script>
        ...
        <div class="row">
            ...
            <div class="col-xs-12 col-md-9 col-lg-9 ">
                <div class="user-account-title-content">
					...
					<p>Vehículos que te has adjudicado (comprado) en venta directa o subasta en los que ya has realizado el pago de la señal.</p>
				</div>
                ...
                <div class="col-xs-12 no-padding" id="lot_pend">
                            ...
                            <div class="panel-heading">
								...
								<div class="panel-title">
									<a class="d-flex justify-content-space-between" data-toggle="collapse" href="#{{$all_inf['inf']->cod_sub}}">
                                        <div>
                                            <span class="title-sub-list">{{ trans("$theme-app.user_panel.auction") }}</span>
                                        </div>
                                    </a>
                                </div>

									<div id="{{$all_inf['inf']->cod_sub}}"  class="table-responsive panel-collapse collaps collapse in">
                                    ...
                                    <form id="pagar_lotes_{{$all_inf['inf']->cod_sub}}" >
                                        ...
                                        <div class="user-account-heading hidden-xs d-flex align-items-center justify-content-space-between">
                                            <div class="col-xs-12 col-sm-7 col-one user-account-item">
                                                    {{ trans(\Config::get('app.theme').'-app.user_panel.lot') }}
                                            </div>
                                            <div class="col-xs-12 col-sm-3 col-one user-account-fecha text-center">
                                                    {{ trans(\Config::get('app.theme').'-app.user_panel.price') }}
                                            </div>
                                            <div class="col-xs-12 col-sm-3 col-lg-2 col-one user-account-max-bid text-center">
                                                    {{ trans(\Config::get('app.theme').'-app.user_panel.paid') }}
                                            </div>
                                        </div>
                                                    ...
                                                    <div class="user-accout-item-wrapper  col-xs-12 no-padding">
                                                        <div class="d-flex">
															...
															<div class="col-xs-12 col-sm-7 col-one user-account-item ">
																...
																<div class="col-xs-12 col-sm-1 no-padding hidden">
																...
																<div class="col-xs-12 col-sm-8 account-item-description">

																		<div class="user-account-item-lot">
																			<span>{{ trans(\Config::get('app.theme').'-app.user_panel.offer') }}
																			@php
														...
														<div class="col-xs-12 col-sm-3 account-item-border">
                                                        ...
                                                        <div class="col-xs-12 col-sm-3 col-lg-2 account-item-border">
                                                                <div class="user-account-item-price  d-flex align-items-center justify-content-center">
                                                                        ...
                                                                        <div class="visible-xs">{{ trans(\Config::get('app.theme').'-app.user_panel.paid') }}</div>

																		<div>
																			<strong>
																				{{ $inf_lot->paid ? 'REALIZADO' : 'PENDIENTE' }}
																			</strong>
																		</div>
                                                                </div>
                                                        </div>
									...
									<div class="adj color-letter  align-items-center justify-content-space-between">
										@if($all_inf['inf']->compraweb_sub == 'S' && (!empty(Config::get('app.merchantIdUP2')) || !empty(Config::get('app.paymentRedsys'))))
											...
											<div class="col-xs-12 col-sm-4 total-price">
												...
												@endif
												<br/><br/>
												<button style="margin-left: 15px;" type="button" class="secondary-button   submit_carrito "  cod_sub="{{$all_inf['inf']->cod_sub}}" class="btn btn-step-reg" disabled>{{ trans(\Config::get('app.theme').'-app.user_panel.pay') }}</button>
                    ...
                    <div class="col-xs-12 no-padding" aria-expanded="false" id="lot_pag">

                        <div class="panel-group" id="accordion">
                            <div class="panel panel-default">

								@foreach($vd_adj as $key_sub => $all_inf)
									...
									<div class="panel-heading">
										<div class="panel-title">
											<a class="d-flex justify-content-space-between" data-toggle="collapse" href="#{{$all_inf['inf']->cod_sub}}_pag">
												<div>
													<span class="title-sub-list">{{ trans("$theme-app.lot_list.direct_sale") }}</span>
													...
													?>
															<div class="user-accout-item-wrapper  col-xs-12 no-padding">
																...
																<div class="col-xs-12 col-sm-7 col-lg-8 col-one user-account-item ">
																	...
																	<div class="col-xs-12 col-sm-8 col-sm-offset-1 no-padding account-item-description">

																		<div class="user-account-item-lot"><span>{{ trans(\Config::get('app.theme').'-app.user_panel.offer') }} {{$inf_lot->ref_asigl0}}</span></div>
																		<div class="user-account-item-title">{{$inf_lot->titulo_hces1?? $inf_lot->descweb_hces1  }}</div>
																		{{--	<div class="user-account-item-text"><div>{{$inf_lot->cod_sub}}</div></div> --}}
																	</div>
																</div>
