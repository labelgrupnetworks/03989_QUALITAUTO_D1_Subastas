 <div class="col-xs-12 col-sm-6 col-lg-4 order-auction-lot mb-1">
...
<div class="col-xs-12 col-sm-6 col-lg-4 mb-1">
...
<div class="col-xs-12 pt-1 d-flex align-items-center flex-wrap">
    <?php
    $count_lots = 0;
			...
			@if(!empty($idValueFeature) && !is_array($idValueFeature))
				<span data-del_filter="#feature_{{$idValueFeature}}" class="del_filter_js badge " style="padding: 1rem; cursor: pointer;">X&nbsp;&nbsp;&nbsp;
					@if(!empty($featuresCount[$idFeature]) && !empty($featuresCount[$idFeature][$idValueFeature]))
						{{ $features[$idFeature] }} - {{$featuresCount[$idFeature][$idValueFeature]["value_caracteristicas_value"]}}
					@else
						{{ $features[$idFeature] }} - {{$idValueFeature}}
					@endif
				</span>
				<?php $show_hr = true; ?>
			@endif
