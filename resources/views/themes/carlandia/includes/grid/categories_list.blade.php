...
<div class="auction__filters-categories">
        <div class="auction__filters-collapse d-flex justify-content-space-between" role="button" data-toggle="collapse" href="#auction_categories" aria-expanded="false" aria-controls="auction_categories">
            <div>{{ trans(\Config::get('app.theme').'-app.lot_list.categories') }}</div>
            <i class="fa fa-sort-down"></i>
        </div>

        <div class="auction__filters-type-list mt-1 collapse @if(!empty($filters["category"]))) in @endif" id="auction_categories">
            ...
            @foreach($categories as $category)
				...
				?>
                    ...
                    @if($linOrtsec0 == $filters["category"] && $numCategoryLots > 0)
						...
						{{-- bloque con modelo --}}
                        <div class="input-category auction__filters-collapse d-flex justify-content-space-between" role="button" data-toggle="collapse" href="#sections_{{$category["key_ortsec0"]}}" aria-expanded="true" aria-controls="sections_{{$category["key_ortsec0"]}}">
                            <div class="category_level_01 d-flex align-items-center justify-content-space-between">
                                <div class="radio">
                                    <input type="radio" name="category" id="category_{{$linOrtsec0}}" value="{{$linOrtsec0}}" class="filter_lot_list_js"  checked="checked" />
                                    <label for="category_{{$linOrtsec0}}" class="radio-label">{{$category["des_ortsec0"]}} ({{Tools::numberformat($numCategoryLots)}})</label>
