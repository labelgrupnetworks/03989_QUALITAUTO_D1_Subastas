    ...
    <div class="container">
        <div class="row">
           ...
           ?>
                ...
                <div class="col-xs-12 no-padding col-sm-2 col-md-2 slider-thumnail-container">
                        ...
                        <div class="owl-theme owl-carousel visible-xs" id="owl-carousel-responsive">
                            ...
                            @foreach($lote_actual->imagenes as $key => $imagen)
									...
									@if($key == 2 && !empty($lote_actual->videos) && count($lote_actual->videos) > 0)
                ...
                <div class="favoritos visible-xs hidden-sm hidden-md hidden-lg">
					<a class="@if($lote_actual->favorito) hidden @endif" id="add_fav_xs" href="javascript:action_fav_modal('add')">
						<i class="fa fa-heart-o" aria-hidden="true"></i>
					</a>
					<a class="@if(!$lote_actual->favorito) hidden @endif" id="del_fav_xs" href="javascript:action_fav_modal('remove')">
                    ...
                    <div class="img-global-content position-relative">
						...
						<div id="img_main" class="img_single">
							<a title="{{$lote_actual->titulo_hces1}}" href="javascript:action_fav_modal('remove')">
								<img class="img-responsive" src="" alt="{{$lote_actual->titulo_hces1}}">
							</a>
						</div>
						...
						@if(Session::has('user') &&  !$retirado)
							<div class="favoritos">
								<a class="@if($lote_actual->favorito) hidden @endif" id="add_fav_lg" href="javascript:action_fav_modal('add')">
									<i class="fa fa-heart-o" aria-hidden="true"></i> {{ trans(\Config::get('app.theme').'-app.lot.add_to_fav') }}
								</a>
								<a class="@if(!$lote_actual->favorito) hidden @endif" id="del_fav_lg" href="javascript:action_fav_modal('remove')">
									<i class="fa fa-heart" aria-hidden="true"></i> {{ trans(\Config::get('app.theme').'-app.lot.del_from_fav') }}
								</a>
							</div>
                        @endif
                    </div>
                    ...
                    <div class="minis-content d-flex flex-wrap">
                        ...
                        @foreach($lote_actual->imagenes as $key => $imagen)
							...
							<div class="mini-img-ficha no-360">
                                ...
                                <a href="javascript:loadSeaDragon('{{ $imagen }}');" class="view-thumbs-open-dragon" data-index="{{ $key }}">
				...
				<div class="col-xs-12 mt-3">
					...
					<div class="next align-items-center d-flex justify-content-space-between">
						@if(!empty($data['previous']))
							<a class="color-letter nextLeft" title="{{ trans(\Config::get('app.theme').'-app.subastas.last') }}" href="{{$data['previous']}}"><i class="fa fa-angle-left fa-angle-custom"></i> {{ trans(\Config::get('app.theme').'-app.subastas.last') }} {{ trans("$theme-app.lot.lot-name") }}: {{ $previousRef }}</a>
						@else
						<span></span>
						@endif

						<p class="text-center">{{ trans("$theme-app.lot.lot-name") }} {{ $lote_actual->ref_asigl0 }} </p>

						@if(!empty($data['next']))
							<a class="color-letter nextRight text-right" title="{{ trans(\Config::get('app.theme').'-app.subastas.next') }}" href="{{$data['next']}}">{{ trans(\Config::get('app.theme').'-app.subastas.next') }} {{ trans("$theme-app.lot.lot-name") }}: {{ $nextRef }} <i class="fa fa-angle-right fa-angle-custom"></i></a>
						@else
						<span></span>
						@endif
					</div>
...
<div class="container">
	{{-- ¿? <div class="@if($subasta_oline && !$cerrado) col-sm-7 @endif col-xs-12 no-padding ficha-tipo-v"> --}}
