								...
								@foreach ($data['values'] as $key_sub => $all_inf)
									...
									<div class="panel-heading">
										<div class="panel-title">
											...
											<a class="d-flex justify-content-space-between" id="open_collapse" aria-expanded="true" data-toggle="collapse"
												href="#{{ $all_inf['inf']->cod_sub }}">
												<span class="title-sub-list">{{ $all_inf['inf']->name }} <span class="date-sub-list">{{ date('Y-m-d H:i', strtotime($all_inf['inf']->start)) }}</span></span>
												<span>
													<span class="label-open-{{$all_inf['inf']->cod_sub}}"
														@if ($count_collapse == 1) style="display: none" @endif>{{ trans($theme . '-app.user_panel.open') }}</span>
													<span class="label-close-{{$all_inf['inf']->cod_sub}}"
														@if ($count_collapse != 1) style="display: none" @endif>{{ trans($theme . '-app.user_panel.hide') }}</span>
													<img width=10
															...
															<div class="col-xs-12 col-sm-2  account-item-border ">
																<div class="user-account-item-date d-flex align-items-center justify-content-center">
																	...
																	@if (!empty($inf_lot->implic_hces1))
																		{{ \Tools::moneyFormat($inf_lot->implic_hces1, trans($theme . '-app.subastas.euros'), 0) }}
																	@else
																		-
																	@endif
																</div>
																	...
																	<div style="text-align: center;"
																		@if (
																			($inf_lot->cod_licit == $inf_lot->licit_winner_bid &&
																				...
																				($inf_lot->cod_licit == $inf_lot->licit_winner_order &&
																					($all_inf['inf']->tipo_sub == 'W' && $all_inf['inf']->subabierta_sub != 'P'))) class="mine"
																				@else
																					class="other" @endif>
																		...
																		@if (\Config::get('app.DeleteOrders') && (empty($inf_lot->implic_hces1) || $inf_lot->imp > $inf_lot->implic_hces1))
																			<input class="btn btn-danger delete_order_panel" type="button" ref="{{ $inf_lot->ref_asigl0 }}"
																				sub="{{ $inf_lot->cod_sub }}"
																				value="{{ trans($theme . '-app.user_panel.delete_orden') }}">
																		@endif
																	</div>
