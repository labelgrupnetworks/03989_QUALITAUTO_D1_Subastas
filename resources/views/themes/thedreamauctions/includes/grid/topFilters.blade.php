...
<div class="col-xs-12 pt-1 d-flex align-items-center paginator-container">
    ...
    <p class="mr-2 mb-0">{{ Tools::numberformat($count_lots) }} {{ trans(\Config::get('app.theme').'-app.lot_list.results') }}</p>
	...
	@if(!empty($auction) && $auction->tipo_sub == \App\Models\V5\FgSub::TIPO_SUB_PRESENCIAL && strtotime($auction->session_end) > time())
	<div class="bid-life d-block text-right" style="flex: 1">
		<a
			style="color:#FFFFFF"
			class="btn btn-bid-life auction-life"
			href="{{ \Tools::url_real_time_auction($auction->cod_sub,$auction->name,$auction->id_auc_sessions) }}"
			title="{{ trans(\Config::get('app.theme').'-app.lot_list.from') }} {{ date_format(date_create_from_format('Y-m-d H:i:s',$auction->session_start),'d/m/Y H:i') }} {{ trans(\Config::get('app.theme').'-app.lot_list.to') }} {{ date_format(date_create_from_format('Y-m-d H:i:s', $auction->session_end),'d/m/Y H:i') }}"
			target="_blank">{{ trans(\Config::get('app.theme').'-app.lot.bid_live') }}
		</a>
	</div>
	@endif
