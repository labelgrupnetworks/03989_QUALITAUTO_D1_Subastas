            ...
            <ul class="menu-principal-content d-flex justify-content-center align-items-center">
				...
				<li>
					...
					<div class="submenuDefault ">
						<div class=" pt-1 pb-1" >
							@if($global['subastas']->has('S') && ($global['subastas']['S']->has('O') || $global['subastas']['S']->has('W')))
								<div class="categoryOption"><a href="{{ \Routing::translateSeo('subastas-activas') }}">{{ trans(\Config::get('app.theme').'-app.artist.actualAuctions')}}</a></div>
							@endif
								<div class="categoryOption"><a href="{{ \Routing::translateSeo('subastas-historicas') }}">{{ trans(\Config::get('app.theme').'-app.subastas.historic_auctions') }}</a></div>
						</div>
					</div>
				</li>
