...
@if (Session::has('user'))
    @php
        $newslettersAccepted = (new App\Models\Newsletter())->getIdSuscriptions(session('user.usrw', ''));
    @endphp

    @if (!in_array(20, $newslettersAccepted))
                ...
                <div class="modal-content modal-content-tr">
                    ...
                    <div class="modal-body">
                        ...
                        <form id="acceptedTermsForm">
                            ...
                            <label for="acceptedTerms" class="d-flex alig-items-center">
                                <input type="checkbox" id="acceptedTerms" value="1" name="families[20]">
                                {{ trans("$theme-app.login_register.read_conditions") }}
                            </label>

                            @if (!in_array(App\Models\V5\Fx_Newsletter::GENERAL, $newslettersAccepted))
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-custom-primary"
                            id="acceptNewTerms">{{ trans("$theme-app.home.confirm") }}</button>
                        <button type="button" class="btn btn-default"
                            data-dismiss="modal">{{ trans("$theme-app.login_register.cancel") }}</button>
                    </div>
                </div>
            </div>
        </div>
