<div class="tr_proyector_info">
    <div class="lot">
        <span id="lote_actual_main" class="">{{ trans(\Config::get('app.theme').'-app.sheet_tr.lot') }} <strong><span id="info_lot_actual">{{ $data['subasta_info']->lote_actual->ref_asigl0 }} </span></strong></span>
    </div>
    <div class="starting-price">
        <div id="precioSalida" class="precioSalida salida text-center">
            <p>{{ trans(\Config::get('app.theme').'-app.sheet_tr.start_price') }}:</p>
            <p>{{ $data['js_item']['subasta']['currency']->symbol }}<span>{{ $data['subasta_info']->lote_actual->formatted_impsalhces_asigl0 }}</span> </p>
        </div>
    </div>
	...
	<div class="next-bid">
        <div id="next-bid_JS" class=" text-center">
            ...
            <p>{{ $data['js_item']['subasta']['currency']->symbol }}<span id="next_bid_JS">{{ \Tools::moneyFormat($data['subasta_info']->lote_actual->importe_escalado_siguiente) }}</span> </p>
        </div>
    </div>
                        ...
                        <?php foreach ($data['subasta_info']->lote_actual->pujas as $puja) : ?>
                            <div class="pujas_model row">
                                ...
                                <div class="col-md-4 col-lg-4 importePuja">
                                    <p><span>{{ $data['js_item']['subasta']['currency']->symbol }}</span><span>{{ $puja->formatted_imp_asigl1 }}</span> </p>
                                </div>
