        ...
        <div class="item_home">
            <a title="{{ $titulo}}" <?= $url ?> >
                <div class="border_item_img">
                    <div class="item_img d-flex justify-content-center">
                        <img src="{{Tools::url_img('lote_medium_large',$bann->num_hces1,$bann->lin_hces1)}}" alt="{{ $titulo}}">
                        @if( $bann->retirado_asigl0 !='N')
            ...
            @if( $bann->retirado_asigl0 =='N')
                @if( \Config::get('app.estimacion') || \Config::get('app.impsalhces_asigl0'))
                    <p class="salida">
						@if($bann->tipo_sub == "V")
						{{ trans(\Config::get('app.theme').'-app.subastas.price_sale') }} {{ \Tools::moneyFormat($bann->no_formated_impsalhces_asigl0 + ($bann->no_formated_impsalhces_asigl0 * ($bann->comlhces_asigl0/100) * 1.21)," €",2  ) }}
                        ...
                        @elseif( \Config::get('app.impsalhces_asigl0'))
                            {{ trans(\Config::get('app.theme').'-app.lot.lot-price') }}  <span class=""> {{$precio_salida}} {{ trans(\Config::get('app.theme').'-app.subastas.euros') }}</span>
                        @endif
                    </p>
                @endif
