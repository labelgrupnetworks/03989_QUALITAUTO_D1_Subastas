	...
	<div class="col-lg-12">
		<div class="info_single_title">
			<div id="text_actual_max_bid" class=" <?=  $lote_actual->open_price >0? '':'hidden' ?>">
				...
				<span
					class="winner <?= (count($data['ordenes']) > 0 && head($data['ordenes'])->cod_licit == $data['js_item']['user']['cod_licit'])? '':'hidden' ?>">{{ trans($theme.'-app.subastas.exceeded') }}</span>
				<span
					class="no_winner <?= (count($data['ordenes']) > 0 && head($data['ordenes'])->cod_licit == $data['js_item']['user']['cod_licit'])? 'hidden':'' ?>">{{ trans($theme.'-app.subastas.not_exceeded') }}</span>
				@endif
			</div>
