@php
	$mostrarCheck = !empty($efec) && $bill->compraweb_sub != "N";
@endphp

<div class="factura col-md-12 d-flex" data-anum="{{$anum}}" data-num="{{$num}}"  data-efec="@if(!empty($efec)){{$efec}}@endif">
	<div class="factura_check margin-auto" style="position: relative">
		@if ($mostrarCheck)
		<label for="checkFactura-{{$anum}}-{{$num}}-{{$efec}}">
			<input name="factura[{{$anum}}][{{$num}}]" id="checkFactura-{{$anum}}-{{$num}}-{{$efec}}" type="checkbox" class="{{-- hide --}} add_factura" checked>
		</label>
		@endif
	</div>
	<div class="factura_box d-flex flex-wrap align-items-center col-xs-12 col-md-12" style="margin-left: {{$mostrarCheck ? '10' : '24'}}px;">

		<div class=" factura_icon col-xs-2 ">
			@if(!empty($bill->factura) && file_exists($bill->factura) )
				<a style="width: 100%" target="_blank" href="/factura/{{$anum}}-{{$num}}">
		...
		<div class="factura_datos col-xs-10 col-sm-6 ">
		...
		<div class="col-xs-12  col-sm-2  account-item-border">
			<div class="user-account-item-date d-flex align-items-center">
				<div class="producto_resumen_info w-100 margin-auto-sm">
