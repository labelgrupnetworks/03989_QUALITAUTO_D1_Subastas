@php
    ...
    //$nowPlus = strtotime(date('Y-m-d H:i:s')) - getdate()[0] + 6; //para test
	$init = strtotime($item->fini_asigl0) - getdate()[0];
	$end = strtotime($item->close_at) - getdate()[0];
    $isInit = strtotime($item->fini_asigl0) < $now;
@endphp
        ...
        <div class="item_lot">
            @if ($subasta_online && !$cerrado)
                <p class="salida-time">
					...
					<span class="open-timer {{ $isInit ? 'hidden' : '' }}">Próxima apertura<br></span>

					<i class="fa fa-clock-o"></i>
                    <span class="timer open-timer {{ $isInit ? 'hidden' : '' }}"
						data-countdown="{{ $init }}"
                        data-format="{{ Tools::down_timer($item->fini_asigl0) }}"
                    ...
                    <span
						class="timer close-timer {{ $isInit ? '' : 'hidden' }}"
						data-countdown="{{ $end }}"
                        data-format="{{ Tools::down_timer($item->close_at) }}">
                    </span>
                </p>
            @endif
            <div class="item_img">
            ...
            <div class="data-container">
                    ...
                    @if (!$retirado && !$devuelto)
                        <p style="visibility: {{ $item->ocultarps_asigl0 != 'S' ? 'visible' : 'hidden' }}">
                            @if (!$subasta_make_offer)
                                @if ($subasta_venta)
                                    <span
                                        class="salida-title">{{ trans(\Config::get('app.theme') . '-app.subastas.price_sale') }}</span>
                                @else
                                    <span
                                        class="salida-title">{{ trans(\Config::get('app.theme') . '-app.lot.lot-price') }}</span>
                                @endif
                                @if ($precio_salida == 0)
                                    {{ trans(\Config::get('app.theme') . '-app.lot.free') }}
                                @else
                                    <span class="salida-price">{{ $precio_salida }}
                        ...
                        @if (($subasta_online || ($subasta_web && $subasta_abierta_P)) && !$cerrado && $hay_pujas)
                            <p>
                                <span
                                    class="salida-title">{{ trans(\Config::get('app.theme') . '-app.lot.puja_actual') }}</span>
                                <span class="salida-price {{ $winner }}">{{ $maxPuja }}
                                    {{ trans(\Config::get('app.theme') . '-app.subastas.euros') }}</span>
                            </p>
                        @endif
                        @if ($awarded && !$devuelto && !$retirado)
                            @if (($cerrado && $remate && !empty($precio_venta)) || ($sub_historica && !empty($item->impadj_asigl0)))
                                ...
                                <p>
                                    <span
                                        class="salida-title soldGrid">{{ trans(\Config::get('app.theme') . '-app.subastas.buy_to') }}</span>
                                    <span class="salida-price  soldGrid">{{ $precio_venta }}
                                        {{ trans(\Config::get('app.theme') . '-app.subastas.euros') }}</span>
                                </p>
                            @elseif($cerrado && empty($precio_venta) && !$compra)
                                <p> <span
                                        class="salida-title notSold">{{ trans(\Config::get('app.theme') . '-app.subastas.dont_buy') }}</span>
                                </p>
                            @endif
                        @endif
                    @endif
                ...
                @if (!$devuelto && !$retirado && !$sub_historica)
                    ...
                    @elseif($subasta_venta && !$cerrado)
                        @if (!$end_session)
                            <p class="btn-bid-lotlist">{{ trans(\Config::get('app.theme') . '-app.subastas.buy_lot') }}
                            </p>
                        @endif
                    @elseif(!$cerrado && (!$subasta_online || $inicio_pujas_online))
                        @php
                            /* Si no está cerrado saldrá el botón o si la subasta online está abierta y es superior a la fecha de inicio también aparecerá */
                        @endphp
                        <p class="btn-bid-lotlist">
                            {{ $subasta_online ? trans("$theme-app.lot.pujar") : trans("$theme-app.lot.submit_offer_concursal") }}
                        </p>
                    @endif
                @endif
