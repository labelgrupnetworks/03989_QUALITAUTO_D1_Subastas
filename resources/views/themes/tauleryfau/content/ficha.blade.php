...
$minMaxLot = \App\Models\V5\FgAsigl0::joinSessionAsigl0()->where('SUB_ASIGL0', $lote_actual->sub_hces1)->selectRaw(" MIN(ref_asigl0) AS min, MAX(ref_asigl0) AS max")->get();
					...
					<div class="col-xs-12 no-padding hidden-lg">
						<div class="btn-responsive flex">
							...
							<div class="btn-add-fav flex valign">
								...
								<a class="inline-flex valign btn add_fav-responsive <?= $data['subasta_info']->lote_actual->favorito? 'hidden':'' ?>"
									href="javascript:action_fav_modal('add')">
									<i class="far fa-star"></i>
								</a>
								<a class="inline-flex valign btn del_fav-responsive <?= $data['subasta_info']->lote_actual->favorito? '':'hidden' ?>"
					...
					<div class="single-lot-bar inline-flex valign hidden-xs hidden-sm hidden-md">
						<div class="slider-images-mini">
							...
							<div class="btn-slider-mini">
								<div class="btn-left d-flex align-items-center" style="display: none"><i class="fa fa-chevron-circle-left"></i></div>
									...
									<div class="col-sm-3-custom">
										{{--<a href="javascript:loadSeaDragon('{{ $imagen }}', {{ $key }});">--}}
											<div class="img-openDragon img-thumb-item flex valign" data-pos="{{ $key }}">
												<img loading="lazy" src="{{ \Tools::url_img("lote_small", $lote_actual->num_hces1, $lote_actual->lin_hces1, $key) }}"
													data-image="{{ $imagen }}">
											</div>
										{{--</a>--}}
									</div>
						...
						<div class="btn-single-lot inline-flex">
							...
							<!-- Video -->
							@if(count($lote_actual->videos ?? []) > 0)
							<a href="javascript:loadVideo('{{ $lote_actual->videos[0] }}','{{$lote_actual->ref_asigl0}}','{{$lote_actual->cod_sub}}');"
			...
			<div class="col-xs-12 col-md-5 col-lg-4">
				<div class="single-lot-des">
					<div class="single-lot-desc-wrapper">
						...
						$lote_actual->compra_asigl0 == 'S' && $lote_actual->fac_hces1=='N' &&
						...
						$lote_actual->subabierta_sub == 'P')&& $lote_actual->cerrado_asigl0 == 'N' &&
            ...
            scroll: function(e){
                ...
                if(this.scrollLeft >= 0 && this.scrollLeft <= this.scrollTotal){
                    if((e.currentTarget.className.includes('btn-right'))){
						...
						(this.scrollLeft == this.scrollTotal) ? $(e.currentTarget).hide() : $(e.currentTarget).show();
            	...
            	if((e.currentTarget.className.includes('btn-left'))){
					...
					(this.scrollLeft == 0) ? $(e.currentTarget).hide() : $(e.currentTarget).show();
					(this.scrollLeft == this.scrollTotal) ? $('.btn-right').hide() : $('.btn-right').show();
            	}
            	this.move();
            }
        },
...
/* $('.zoomImgClose').click(function(){
	...
	if($(window).width() < 1200){
		$("#chevron-left-button, #chevron-right-button, #toolbarDiv").addClass('hidden-xs').addClass('hidden-sm').addClass('hidden-md');
	}
