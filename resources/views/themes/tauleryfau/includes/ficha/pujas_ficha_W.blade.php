<div>
    <div class="single-lot-desc-title desc">
        ...
        <p class="sub-o">{{ trans(\Config::get('app.theme').'-app.subastas.lot_subasta_presencial') }}</p>
    </div>
    <div>
        ...
        {{trans(\Config::get('app.theme').'-app.subastas.euros')}}</strong>
            ...
            <div class="info_single_content_button">
                @if( $lote_actual->cerrado_asigl0=='N' && $lote_actual->fac_hces1=='N' &&  strtotime("now") > strtotime($lote_actual->orders_start)  &&   strtotime("now") < strtotime($lote_actual->orders_end))
                    <small><strong><?=trans(\Config::get('app.theme').'-app.lot.insert_max_puja_start')?></strong></small>
                    <div class="input-group direct-puja">
                            ...
                            <button id="pujar_ordenes_w" data-from="modal" type="button" class="btn-color" ref="{{ $data['subasta_info']->lote_actual->ref_asigl0 }}" codsub="{{ $data['subasta_info']->lote_actual->cod_sub }}">{{ trans(\Config::get('app.theme').'-app.lot.place_bid') }}</button>
                    </div>
                @endif
                ...
                <div class="">
                     <a href='{{  Routing::translateSeo('api/subasta').$data['subasta_info']->lote_actual->cod_sub."-".str_slug($data['subasta_info']->lote_actual->name)."-".$data['subasta_info']->lote_actual->id_auc_sessions }}'>
                            <span class="btn btn-custom live-btn mb-1"><?=trans(\Config::get('app.theme').'-app.lot.bid_live')?></span>
                     </a>
                    </div>
                @endif
