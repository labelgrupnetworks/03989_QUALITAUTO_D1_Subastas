    ...
    <div class="item_lot lot">
        ...
        <div class="lot-wrapper">
			...
			@if( Config::get('app.icon_multiple_images') && count($item->imagenes) > 1)
				<span class="more-picture @if (count($item->videos) > 0) more-picture2 @endif" href="#" data-toggle="tooltip" title="{{ trans(\Config::get('app.theme').'-app.lot_list.more_pictures') }}">
                    ...
                    @if( $item->retirado_asigl0 =='N' && $item->fac_hces1 != 'D' && $item->fac_hces1 != 'R')
                        ...
                        @else
                        	...
                        	<div class="price-content">
                            	...
                            	@endif
                            	<p class="price <?=!empty($item->oferta_asigl0) && $item->oferta_asigl0 == 1?'tachado':'';?> gold">{{$item->formatted_imptas_asigl0}}  {{ trans(\Config::get('app.theme').'-app.subastas.euros') }}</p>
                        	</div>
                    	...
                    	@if( \Config::get('app.awarded'))
	                        ...
	                        <div class="btn-pujar">
                            	...
                            	@elseif($item->cerrado_asigl0 == 'S' && $item->remate_asigl0 =='S' &&  (!empty($precio_venta) ) || ($item->subc_sub == 'H' && !empty($item->impadj_asigl0)) )
                                	...
                                	<a class="btn btn-custom buy-to" <?= $url?>>{{ trans(\Config::get('app.theme').'-app.subastas.buy_to') }}: <span >{{ \Tools::moneyFormat($precio_venta) }} {{ trans(\Config::get('app.theme').'-app.subastas.euros') }}</a>
					...
					@elseif($item->tipo_sub == 'P' || $item->tipo_sub == 'O' || $item->tipo_sub == 'V' || $item->tipo_sub == 'W')
                        ...
                        <div class="timeLeft d-flex align-items-center">
							@if ($item->tipo_sub != 'V')
								...
								href="javascript:modalVideo('{{$item->videos[0]}}', '{{$item->ref_asigl0}}', '{{$item->cod_sub}}', '{{$titulo}}', '{{$item->descweb_hces1}}', '{{ $item->cerrado_asigl0 == 'S' ? trans(\Config::get('app.theme').'-app.lot.view_lot') : trans(\Config::get('app.theme').'-app.lot.pujar') }}', '{{$url_friendly}}')">
										{{--<img src="/themes/{{\Config::get('app.theme')}}/assets/img/more-video.png" />--}}
										<i class="fa fa-play" aria-hidden="true"></i>
                                ...
                                <img src="/themes/{{\Config::get('app.theme')}}/assets/img/auction.png" width="16px" height="16px" style="margin-right: 5px;"/>{{ $item->total_pujas }}
                                <img src="/themes/{{\Config::get('app.theme')}}/assets/img/man-user.png" width="16px" height="16px" style="margin-left: 10px; margin-right: 5px;" />{{ $item->total_postores }}
                            @endif
                        </div>
