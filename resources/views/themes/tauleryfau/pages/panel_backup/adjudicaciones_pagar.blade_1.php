        ...
        <div class="row">
            ...
            <div class="col-xs-10 col-sm-10 col-md-9">
                <div class="user-datas-title flex">
                    <p>{{ trans($theme.'-app.user_panel.allotments') }}</p>
                    ...
                    <div class="btns-pay flex">
                        ...
                        <div>
                        <a href="{{ \Routing::slug('user/panel/allotments/paid') }}" class="btn-payed"  style="color: #283747">{{ trans($theme.'-app.user_panel.bills') }}</a>
                        </div>
                    </div>
                </div>
                         ...
                         <!-- /Cabeceras grices con titulos-->
                          <?php $countBid=1; ?>
                            ...
                            @foreach($all_inf['lotes'] as $inf_lot)
                             ...
                             <!-- Lotes en vista desktop-->
                                 <div class="custom-wrapper flex  valign">
                                    ...
                                    <div class="auc-data-custom">
                                        <p><?=  $comision ?> {{ trans($theme.'-app.lot.eur') }}</p>
                                    </div>
                                     <div class="auc-data-custom">
                                         <p><?= \Tools::moneyFormat($precio_limpio_calculo,false,2); ?> {{ trans($theme.'-app.lot.eur') }}</p>
                                     </div>
                            ...
                            @if($data['user']->envcorr_cli != 'B')
                                <div class="adj" >
                                    <h1 class="titlecat"><span class="precio_final_{{$all_inf['inf']->cod_sub}}">0</span> {{ trans($theme.'-app.lot.eur') }}</h1>
                                    @if($all_inf['inf']->compraweb_sub == 'S')
                                    <button type="button" class="submit_carrito btn btn-step-reg"  cod_sub="{{$all_inf['inf']->cod_sub}}" class="btn btn-step-reg" disabled>{{ trans($theme.'-app.user_panel.pay') }}</button>
                                    @endif
                                    <div class="info-modal-pay">
                                        ...
                                        <div class="info-pay-modal" style="display: none">
                                            ...
                                            <div class="price flex">
                                                ...
                                                <div class="money">{{\Tools::moneyFormat($total_remate,false,2)}} {{ trans($theme.'-app.lot.eur') }}</div>
                                            </div>
                                            <div class="price flex">
                                                <div class="title">{{ trans($theme.'-app.user_panel.base') }}</div>
                                                <div class="money">{{\Tools::moneyFormat($total_base,false,2)}} {{ trans($theme.'-app.lot.eur') }}</div>
                                            </div>
                                            <div class="price flex">
                                                <div class="title">{{ trans($theme.'-app.user_panel.tax') }}</div>
                                                <div class="money">{{\Tools::moneyFormat($total_iva,false,2)}} {{ trans($theme.'-app.lot.eur') }}</div>
                                            </div>
                                            <div class="price flex">
                                            ...
                                            <div class="price flex text-bold">
                                                ...
                                                <div class="money flex"><b style="display: inline-block" class='precio_final_{{$all_inf['inf']->cod_sub}}'></b><b style="display: inline-block"> {{ trans($theme.'-app.lot.eur') }}</b></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            @elseif($data['user']->envcorr_cli == 'B')
                            <div class="adj"><p style="color: #283747;font-size: 18px;">{{ trans($theme.'-app.user_panel.contact_tauler') }}</p></div>
                            @endif
                        </div>
                        </form>
                        @endforeach
