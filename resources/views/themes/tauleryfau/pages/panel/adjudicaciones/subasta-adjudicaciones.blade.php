...
<div id="{{$all_inf['inf']->cod_sub}}_{{$pagada}}" class="table-responsive-custom panel-collapse collapse">
	...
	<div class="custom-wrapper valign">
		...
		<div class="account-lot-wrapper font-data-custom" style="grid-area: lot;">
			<p><span class="visible-expand">{{ trans($theme.'-app.lot.lot-name') }}</span> {{$inf_lot->ref_asigl1}}</p>
		</div>
	...
	<div class="text-right factura-buttons">
		@if (!$pagada)
			...
			@if (!empty($all_inf['lotes'][0]->prefactura))
				<a href="/prefactura/{{ $all_inf['inf']->cod_sub }}" download class="btn btn-color factura-button mb-1">{{ trans($theme.'-app.user_panel.proforma_invoice') }}</a>
			@endif
			...
			@if($all_inf['inf']->compraweb_sub == 'S')
				...
				cod_sub="{{$all_inf['inf']->cod_sub}}">{{ trans($theme.'-app.user_panel.pay_now') }}</a>
			@endif
		...
		@else
			...
			@if (!empty($all_inf['lotes'][0]->factura))
				<a href="/factura/{{$all_inf['lotes'][0]->afral_csub.'-'.$all_inf['lotes'][0]->nfral_csub}}" download class="btn btn-color factura-button mb-1">{{ trans($theme.'-app.user_panel.invoice_pdf') }}</a>
			@endif
