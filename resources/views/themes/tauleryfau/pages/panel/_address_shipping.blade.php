	...
	<div class="tabs-deliverea">
		...
		<div class="content-tabs pl-0 col-xs-12 col-sm-5 col-md-3">
			<div class="plus-address"><a class="color-letter seeshippingaddress" cod="new"><i
					...
					<span>{{ trans($theme.'-app.user_panel.add_address') }}</span></a>
			</div>
			...
			<div class="content-address">
				...
				<ul class="nav nav-tabs">
					...
					<div class="title-address">
						{{ trans($theme.'-app.user_panel.address_predeterminated') }}
					</div>
					...
					<div class="title-address">
						{{ trans($theme.'-app.user_panel.address_normal') }}</div>
					@else
					<li role="presentation"
						class='<?=$data['codd_clid'] == $address->codd_clid?'selected':''; ?> address'>
			...
			<div class="inputs-custom-group">
				<form class="col-xs-12 save_address_shipping">
					...
					<div class="deliverea_tit">
						{{ trans($theme.'-app.user_panel.title_envio') }}
					</div>
					@else
					<div class="deliverea_tit">
						{{ trans($theme.'-app.user_panel.title_envio_predeterminated') }}
					</div>
					@endif
					...
					<div class="form-group col-xs-12 col-md-6 col-lg-6">
						<label
							for="nombre">{{ trans($theme.'-app.login_register.nombre_apellido') }}</label>
						<input type="text" class="form-control" name="usuario"
							placeholder="{{ trans($theme.'-app.user_panel.name') }}" required
							value="<?= !empty($address->nomd_clid)?$address->nomd_clid:''; ?>">
					</div>
					<div class="form-group col-xs-12 col-md-6 col-lg-6">
						<label for="telefono">{{ trans($theme.'-app.user_panel.phone') }}</label>
						<input type="text" name="telefono" class="form-control"
							placeholder="{{ trans($theme.'-app.user_panel.phone') }}" required=""
					...
					<div class="form-group col-xs-12 col-md-6 col-lg-10">
						...
						<input type="text" name="clid_direccion" class="form-control dreccion_cambiar" id="direccion"
							placeholder="{{ trans($theme.'-app.user_panel.address') }}" required
							maxlength="60"
					...
					<div class="form-group col-xs-12 col-md-6 col-lg-6">
						...
						<input id="codigo_postal" type="text" name="clid_cpostal" class="form-control"
							placeholder="{{ trans($theme.'-app.user_panel.zip_code') }}" required
					...
					<div class="form-group col-xs-12 col-md-6 col-lg-6">
						...
						<input id="clid_poblacion" type="text" name="clid_poblacion" class="form-control"
							placeholder="{{ trans($theme.'-app.user_panel.city')}}" required
					...
					<div class="deliverea-buttons col-xs-12 no-padding">
						...
						<div class="btn-delete">
							<button class="btn btn-danger delete-address" type="button"
								cod="<?= !empty( $data['codd_clid'])? $data['codd_clid']:'';?>">{{ trans($theme.'-app.user_panel.delete') }}</button>
						</div>
						@endif
						<div class="btn-save">
						...
						<div class="btn-save ">
							<button class="button-principal btn fav-address" type="button"
								cod="<?= !empty( $data['codd_clid'])? $data['codd_clid']:'';?>">{{ trans($theme.'-app.user_panel.define_default') }}</button>
						</div>
						@endif
					</div>
				</form>
