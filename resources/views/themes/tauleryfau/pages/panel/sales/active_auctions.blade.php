    ...
    @foreach ($subastas->keys() as $cod_sub)
        <div class="sales-auction-wrapper" data-sub="{{ $cod_sub }}">
            <div class="sales-auction">
                ...
                <p>
                    <span class="visible-md visible-lg">{{ $subastas[$cod_sub]->first()->des_sub }}</span>
                    <span class="hidden-md hidden-lg">{{ $subastas[$cod_sub]->first()->cod_sub }}</span>

                </p>
                <p>{{ $statistics['auction'][$cod_sub]['consigned_lots'] }}</p>
                <p class="js-divisa visible-md visible-lg"
                    value="{{ $statistics['auction'][$cod_sub]['starting_price'] }}">
                    {!! $currency->getPriceSymbol(2, $statistics['auction'][$cod_sub]['starting_price']) !!}
                </p>
                <p class="js-divisa visible-md visible-lg"
                    value="{{ $statistics['auction'][$cod_sub]['estimate_price'] }}">
...
<section class="tab-content" id="auction-details">
    ...
    @foreach ($subastas as $cod_sub => $lotes)
        <div class="tab-pane" id="auction-details-{{ $cod_sub }}" role="tabpanel">
            <h4 class="sales-lots-title">{{ $lotes->first()->des_sub }}</h4>
                        ...
                        <div class="sales-lot">
                            <div class="sales-lot_img">
                                <img class="img-responsive"
                                    src="{{ Tools::url_img('lote_medium', $lote->num_hces1, $lote->lin_hces1) }}"
                                    alt="">
                            </div>
                            ...
                            <div class="sales-lot_actual-price">
                                <p class="js-divisa" value="{{ $lote->implic_hces1 }}">
                                    {!! $currency->getPriceSymbol(2, $lote->implic_hces1) !!}
                                </p>
                            </div>
