			...
			<form class="form-inline" method="POST" action="{{route('cookieConfig', ['lang' => Config::get('app.locale')])}}">
				...
				<div class="col-xs-12">
					...
					<ul class="cookies-list">
						...
						<li>
							<div class="row">
								<div class="col-xs-4"><span>{{trans(\Config::get('app.theme')."-app.cookies.cookie_name_$value")}}</span></div>
							...
							<div class="row mt-2">
								...
								<div class="col-xs-6">{{ trans(\Config::get('app.theme').'-app.cookies.cookie_description'.$cookie) }}</div>
							</div>
							@endforeach
