								...
								@if(!$tipo_pago_global)
									<div class="adj color-letter  align-items-center justify-content-space-between">
										@if($all_inf['inf']->compraweb_sub == 'S' && (!empty(Config::get('app.merchantIdUP2')) || !empty(Config::get('app.paymentRedsys'))))
											...
											<div @class([
												'col-xs-12 total-price',
												'col-sm-4' => !empty(Config::get('app.web_gastos_envio')),
												'col-sm-6 col-sm-offset-6' => empty(Config::get('app.web_gastos_envio'))
											])>
												<p class="mb-4">
													{{ trans($theme.'-app.user_panel.remate_comision') }}<br>
													<span class="precio_final_remate_{{$all_inf['inf']->cod_sub}}">0</span> {{ trans($theme.'-app.lot.eur') }}
												</p>
												<p class="mb-4">
													{{ trans($theme.'-app.user_panel.iva_comision') }}<br>
													<span class="iva_final_{{$all_inf['inf']->cod_sub}}">0</span> {{ trans($theme.'-app.lot.eur') }}
												</p>
												<p class="mb-4">
													{{ trans($theme.'-app.user_panel.total_price') }}<br>
												...
												<p class="mb-4 text-justify">
													{!! trans("$theme-app.user_panel.payment_limit_notice") !!}
												</p>
												...
												<div class="d-flex gap-3 flex-wrap">
													@if (Config::get("app.urlToPackengers"))
														...
														<a class="btn btn-small btn-outline-lb-primary btn-packengers w-auto gap-2 me-auto" href="{{$urlCompletePackengers}}" target="_blank">
															<x-icon.boostrap icon="truck" />
															{{ trans("$theme-app.lot.packengers_adjudicaciones") }}
														</a>
													@endif
													<button type="button" class="secondary-button submit_carrito" cod_sub="{{$all_inf['inf']->cod_sub}}" class="btn btn-step-reg" disabled>
