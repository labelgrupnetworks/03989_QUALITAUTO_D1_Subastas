...
<div class="tabs-custom filters-auction-content">
	...
	<div class="<?php if($tab != 'shopping_orders' && $tab !='shopping_cart'){echo('collapse'); } ?>" id="collapse_shop">
		<ul class="sub_menu_adj">
			<a class="@if($tab == 'shopping_cart') active @endif" style="text-transform: uppercase" data-title="{{ trans(\Config::get('app.theme').'-app.panel.pending_bills') }}" href="{{ \Routing::slug('user/panel/showArticleCart') }}" >{{ trans("$theme-app.shopping_cart.myCart") }}</a>
			<a class="@if($tab == 'shopping_orders') active @endif" style="text-transform: uppercase" data-title="{{ trans(\Config::get('app.theme').'-app.panel.my_adj') }}" href="{{ route('panel.shopping_orders', ['lang' => config('app.locale')]) }}" >{{ trans("$theme-app.shopping_cart.shopping_orders") }}</a>
		</ul>
	</div>
