							...
							<div class="col-xs-12 col-md-3 mb-3">
								...
								<div class="textAuctionList">
									<p class="textAuctionList-auction">
										{{ trans(\Config::get('app.theme').'-app.subastas.inf_subasta_subasta') }} {{$subasta->cod_sub }}
									</p>
									<p class="textAuctionList-date">
										{{$subasta->des_sub }}
									</p>
									<p>
										<a href="/catalogos/{{$subasta->cod_sub }}" class="btn mini-button" target="_blank">{{ trans("$theme-app.lot_list.ver_catalogo") }}</a>
									</p>
									@if( strtotime($subasta->session_end) > time())
										<p class="realTime">
											<a class=" btn realTimeButton" href="{{ \Tools::url_real_time_auction($subasta->cod_sub,$subasta->name,$subasta->id_auc_sessions)}}" target="_blank" >
