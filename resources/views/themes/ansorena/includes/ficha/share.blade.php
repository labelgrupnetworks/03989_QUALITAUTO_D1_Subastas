...
<div class="zone-share-social col-xs-12 no-padding d-flex flex-row-reverse justify-content-space-between">
			...
			<ul class="red ul-format d-flex align-items-center">
				...
				<li class="social-link">
					<a class=" d-flex align-items-center justify-content-center color-letter" title="{{ trans($theme.'-app.lot.share_twitter') }}" href="http://twitter.com/share?text=<?= $titulo .' '.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']?>&url=<?=$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'] ?>"><i class="fab fa-twitter"></i></a>
				</li>
				<li class="social-link">
					<a class=" d-flex align-items-center justify-content-center color-letter" title="{{ trans($theme.'-app.lot.share_pintarest') }}" href="http://www.pinterest.com/pin/create/button/?&url=<?=$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'] ?>&description=<?= $titulo .' '.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']?>"><i class="fab fa-pinterest"></i></a>
				</li>
				<li class="social-link">
