...
<div class="ficha-content color-letter">
        ...
        <div class="row">
           ...
           ?>
                ...
                <div class="col-xs-12 no-padding hidden-xs">
                    ...
                    @elseif($cerrado && \Config::has("user") &&  (!empty($lote_actual->himp_csub) || ($sub_historica && !empty($lote_actual->impadj_asigl0))))
                        <div class="retired" style ="">
                    ...
                    <div id="img_main" class="img_single">
                            ...
                            <img class="img-responsive" src="{{Tools::url_img_friendly('lote_large',$lote_actual->num_hces1,$lote_actual->lin_hces1,0,$lote_actual->webfriend_hces1)}}" alt="{{$lote_actual->webfriend_hces1}}">
    ...
    <div class="col-sm-5 col-xs-12 content-right-ficha d-flex justify-content-space-between flex-column">
       ...
       <div class="d-flex  flex-column">
            <div class="ficha-info-title col-xs-12 no-padding mb-2">
					...
					@php
						 ...
						 * */
					@endphp
					<div class="secondary-color-text ficha-autor">
						<p><b>{{$caracteristicas[1]->value_caracteristicas_hces1 ?? '' }}</b></p>
					</div>
					<div class="secondary-color-text ficha-fecha-autor">
						<p>{{$caracteristicas[4]->value_caracteristicas_hces1 ?? '' }}</p>
					</div>
					{{-- <div class="secondary-color-text ficha-title">
						<p>{!! $lote_actual->descweb_hces1 !!}</p>
					</div> --}}
					<div class="desc-lot-profile-content ficha-title">
						<p>{!! $lote_actual->desc_hces1 !!}</p>
					</div>
					<div class="secondary-color-text ficha-tecnica">
						<p>{{$caracteristicas[2]->value_caracteristicas_hces1 ?? '' }}</p>
					</div>
					<div class="secondary-color-text ficha-medidas">
						<p>{{$caracteristicas[3]->value_caracteristicas_hces1 ?? '' }}</p>
					</div>
					{{-- <div class="secondary-color-text ficha-contentextra text-transform-original">
						<p>{{$lote_actual->contextra_hces1 ?? '' }}</p>
					</div> --}}
...
<div class="container">
	<div class="row">
		<div class="@if($subasta_online && !$cerrado) col-sm-7 @endif col-xs-12 ficha-tipo-v">
		<?php # han pedido quitar la descripción
		/*
			<div class="desc-lot-title d-flex justify-content-space-between mb-2">
				<p class="desc-lot-profile-title m-0">{{ trans(\Config::get('app.theme').'-app.lot.description') }}</p>
...
$( document ).ready(function() {
	...
	const has360 = @json(!empty($lote_actual->contextra_hces1));


	//desactivamos 360 ya que tira de contextra y ellos lo utilizan para otra información
    if (has360 && false) {
