...
//dd($data['subasta_info']->lote_actual);
	$ordenes = App\Models\V5\FgOrlic::select("count(ref_orlic) as cuantos")->where("sub_orlic", $data['subasta_info']->lote_actual->cod_sub)->where("ref_orlic", $data['subasta_info']->lote_actual->ref_asigl0)->where("tipop_orlic", 'T')  ->first();
@endphp
        ...
        @if( $cerrado_N && $fact_N &&  $start_orders  &&   !$end_orders && !$subasta_abierta_P)
            ...
            <div class="input-group group-pujar-custom d-flex justify-content-space-between">
                    ...
                    <div class="input-group-btn">
                        <button id="pujar_ordenes_w_ansorena" data-from="modal" type="button" class="ficha-btn-bid button-principal" ref="{{ $data['subasta_info']->lote_actual->ref_asigl0 }}" codsub="{{ $data['subasta_info']->lote_actual->cod_sub }}">{{ trans(\Config::get('app.theme').'-app.lot.place_bid') }}</button>
                    </div>



					{{-- Mónica ha pedido que las pujas telefónicas se permitan hasta las 14:00 h del día de la subasta --}}
					@if(strtotime(date("Y-m-d 14:00:00",strtotime($data['subasta_info']->lote_actual->start_session))) > time())
					{{-- no puede haber más de 10 pujas telefónica y el preci odebe ser mayor de 300 --}}
						@if($ordenes->cuantos <10 && $lote_actual->impsalhces_asigl0 >= 300)
							<div>
