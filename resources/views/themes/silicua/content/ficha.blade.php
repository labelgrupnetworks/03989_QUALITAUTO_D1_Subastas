    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-sm-7 col-xs-12" style="position: relative">
                ...
                <div class="col-xs-12 no-padding slider-thumnail-container">
                    ...
                    @elseif(
                        $cerrado &&
                            ($desadjudicado || !empty($lote_actual->himp_csub) || ($sub_historica && !empty($lote_actual->impadj_asigl0))))
                        <div class="retired" style="">
                    ...
                    <div class="owl-theme owl-carousel {{-- visible-xs --}}" id="owl-carousel-responsive">
                        @foreach ($lote_actual->videos as $key => $resource)
                            <div class="item_content_img_single" style="position: relative;"
                                data-hash="video_{{ $key }}">

                                @if (Tools::fileNameIsVideo($resource))
                                    <video width="100%" controls muted preload="auto" class="video-item">
                                        <source src="{{ $resource }}" type="video/mp4">
                                    </video>
                                @else
                                    <img class="img-responsive" src="{{ $resource }}"
                        ...
                        @foreach ($lote_actual->imagenes as $key => $imagen)
                            <a href="{{ Tools::url_img('real', $lote_actual->num_hces1, $lote_actual->lin_hces1, $key) }}"
                                target="_blank" data-hash="image_{{ $key }}">
                                <div class="item_content_img_single">
                                    <img class="img-responsive"
                                        src="{{ Tools::url_img('real', $lote_actual->num_hces1, $lote_actual->lin_hces1, $key) }}"
                                        alt="{{ $lote_actual->titulo_hces1 }}">
                                </div>
                            </a>
                        @endforeach
                ...
                <div class="col-xs-12 no-padding hidden-xs hidden-sm mt-2">
                                ...
                                <a href="#video_{{ $key }}">
                                    @if (Tools::fileNameIsVideo($resource))
                                        <div class="img-openDragon"
                                            style="background-image:url('{{ asset('/img/icons/video_thumb.png') }}'); background-size: contain; background-position: center; background-repeat: no-repeat;"
                                            alt="{{ $lote_actual->titulo_hces1 }}"></div>
                                    @else
                                        <div class="img-openDragon"
                                            style="background-image:url('{{ $resource['src'] }}'); background-size: contain; background-position: center; background-repeat: no-repeat;"
                                            alt="{{ $lote_actual->titulo_hces1 }}"></div>
                                    @endif
                                </a>
                            </div>
...
@if (Session::has('user') && !$retirado)
    <div class="container">
        <div class="col-xs-12 no-padding favoritos">
            <a class="secondary-button col-xs-12 col-md-2 <?= $lote_actual->favorito ? 'hidden' : '' ?>" id="add_fav"
                href="javascript:action_fav_modal('add')">
                {{ trans(\Config::get('app.theme') . '-app.lot.add_to_fav') }}
            </a>
            <a class="secondary-button col-xs-12 col-md-2 <?= $lote_actual->favorito ? '' : 'hidden' ?>" id="del_fav"
        ...
        <div class="single">
            ...
            <div class="col-xs-12 col-sm-12 lotes_destacados">
                <div class="mas-pujados-title color-letter">
                    <span>{{ trans(\Config::get('app.theme') . '-app.lot.recommended_lots') }}</span>
                </div>
