                    ...
                    @if (\App::getLocale() != $key)
                        <li>
                            <a translate="no" title="<?= trans(\Config::get('app.theme') . '-app.head.language_es') ?>"
                                class="link-lang color-letter" href="/<?= $key ?>">
...
<header>
    <nav class="menu-header">
		...
		{{-- solo esta para que el logo se centre --}}
		<div class="menu-responsive hidden-lg"></div>
		<div class="menu-responsive hidden-xs hidden-lg"></div>
		<div class="menu-responsive hidden-xs hidden-lg"></div>
        ...
        <div class="menu-principal">
            ...
            <ul class="menu-principal-content d-flex justify-content-center align-items-center">
                ...
                <li style="position:relative" class="hidden-sm hidden-xs hidden-sm  hidden-md  ">
                    <a id="JSmenu-subastas"
                        class="color-letter d-flex link-header justify-content-center align-items-center"
                        href="#">
                        <span>{{ trans(\Config::get('app.theme') . '-app.foot.auctions') }}</span>
                    </a>
                    <div class="menu-subastas">
                        <a class="color-letter d-flex link-header"
                            href="{{ \Routing::translateSeo('presenciales') }}?f=2">
                            <span>{{ trans(\Config::get('app.theme') . '-app.foot.active_auctions') }}</span>
                        </a><br>
                        <a class="color-letter d-flex link-header"
                            href="{{ \Routing::translateSeo('subastas-historicas') }}">
                            <span>{{ trans(\Config::get('app.theme') . '-app.foot.historico') }}</span>
                        </a>
                    </div>
                </li>
                <li class="flex-display hidden-lg">
                    <a class="color-letter flex-display link-header justify-center align-items-center"
                        title="{{ trans(\Config::get('app.theme') . '-app.home.home') }}"
                        href="{{ \Routing::translateSeo('presenciales') }}?f=2">
                        <span>{{ trans(\Config::get('app.theme') . '-app.foot.active_auctions') }}</span>
                    </a>
                </li>
                ...
                <li>
                    <a class="color-letter flex-display link-header justify-center align-items-center" title=""
                        href="{{ \Routing::translateSeo('valoracion-articulos') }}"><span>
                            {{ trans(\Config::get('app.theme') . '-app.home.free-valuations') }}</span></a>
                </li>
                ...
                @if (!empty($has_subasta))
                    <li>
                        <a class="color-letter flex-display link-header justify-center align-items-center"
                            href="{{ \Routing::translateSeo('subastas-online') }}?f=1"><span>{{ trans(\Config::get('app.theme') . '-app.foot.online_auction') }}</span></a>
                    </li>
                @endif
        ...
        <div class="user-account">
            @if (!Session::has('user'))
                <div class="user-account-login d-flex">
                    <a class="d-flex justify-center align-items-center btn_login_desktop btn_login"
                        title="{{ trans("$theme-app.login_register.generic_name") }}"
                        href="javascript:;">{{ trans("$theme-app.login_register.generic_name") }}
                    </a>
					<a class="d-flex justify-center align-items-center hidden-xs"
                        title="{{ trans("$theme-app.login_register.register") }}"
                        href="{{ Routing::slug('register') }}">{{ trans("$theme-app.login_register.register") }}
                    </a>
                </div>
            @else
                <a class="my-account color-letter">
                    <img width="25px;" class="logo-company"
                        ...
                        alt="{{ \Config::get('app.name') }}">
                    <span
                        class="hidden-xs">{{ trans(\Config::get('app.theme') . '-app.login_register.my_panel') }}</span>
                ...
                <div class="mega-menu background-body rigth-0">
                    ...
                    <a class="color-letter" href="{{ \Routing::slug('user/panel/favorites') }}">
...
<script>
    $('#JSmenu-subastas').click(function(e) {
        $('.menu-subastas').toggle();
        e.stopPropagation();
    });
    $('body').click(function() {
        if ($('.menu-subastas').is(':visible')) {
            $('.menu-subastas').fadeOut();
        };
    });
</script>
