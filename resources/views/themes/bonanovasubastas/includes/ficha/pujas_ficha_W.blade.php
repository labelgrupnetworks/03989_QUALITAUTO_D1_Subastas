        ...
        <div class="info_single_title">
            <div id="text_actual_max_bid" class=" <?=  $lote_actual->open_price >0? '':'hidden' ?>">
                ...
                @if (isset($data['js_item']['user']))
                    <?php
                    if(count($data['ordenes']) > 0 && head($data['ordenes'])->cod_licit == $data['js_item']['user']['cod_licit']){
                        $licit_win = 'winner';
                    }else{
                        $licit_win = 'no_winner';
                    }
        ...
        <div class="col-xs-12">
            <div class="info_single_content">
                ...
                @if( $lote_actual->cerrado_asigl0=='N' && $lote_actual->fac_hces1=='N' &&  (time() > strtotime($lote_actual->orders_start))  &&   (time() < strtotime($lote_actual->orders_end)) && $lote_actual->subabierta_sub != 'P')
                    <?php
                        $siguiente_puja = $data['precio_salida'];
                        if($lote_actual->subabierta_sub=='O' ){
                            $subastaobj= new App\Models\Subasta();
                            //calcular siguiente escalado de subasta abierta tipo O
                            if($lote_actual->open_price != 0){
                                $puja_actual=$lote_actual->open_price;
                                $siguiente_puja = $subastaobj->NextScaleBid($lote_actual->impsalhces_asigl0,$puja_actual );
                            }
                        }
