            ...
            <div class="row">
                <div class="col-xs-12">
                    <div class="navbar-top-wrapper">
                        <ul>
                            @if(!Session::has('user'))
                                <li class="hidden-xs hidden-sm"><a class="btn-color" title="{{ trans($theme.'-app.login_register.register') }}" href="{{ \Routing::slug('register') }}">{{ trans($theme.'-app.login_register.register') }}</a></li>
                                <li class="hidden-xs hidden-sm"><a class="btn_login_desktop btn-color" title="<?= trans($theme.'-app.login_register.login') ?>" href="javascript:;"><?= trans($theme.'-app.login_register.login') ?></a></li>
                            ...
                            @else
                                ...
                                <li class="hidden-xs hidden-sm"><a class="btn-color" href="{{ \Routing::slug('user/panel/orders') }}" >{{ trans($theme.'-app.login_register.my_panel') }}</a></li>
                            ...
                            @if(Session::get('user.admin'))
                                <li class="hidden-xs hidden-sm"><a class="btn-color" href="/admin"  target = "_blank"> {{ trans($theme.'-app.login_register.admin') }}</a> /</li>
                                <li class="hidden-lg hidden-md sesion-responsive"><a href="/admin"  target = "_blank"> {{ trans($theme.'-app.login_register.admin') }}</a></li>
                            @endif
                            <li class="hidden-xs hidden-sm"><a class="btn-color" href="{{ \Routing::slug('logout') }}" >{{ trans($theme.'-app.login_register.logout') }}</a></p></li>
                            @endif
                            ...
                            <div class="login_desktop" style="display:none;">
                                ...
                                <form data-toggle="validator" id="accerder-user-form">
                                    ...
                                    <label for="usuario">{{ trans($theme.'-app.login_register.user') }}</label>
                                        <input id="usuario" class="form-control" placeholder="{{ trans($theme.'-app.login_register.user') }}" type="email" name="email" type="text" autocomplete="username">
                                    </div>
                                    <div class="form-group">
                                        <label for="contraseña">{{ trans($theme.'-app.login_register.contraseña') }}</label>
                                        <input id="contraseña" class="form-control" placeholder="{{ trans($theme.'-app.login_register.contraseña') }}" type="password" name="password" autocomplete="current-password" maxlength="20">
                                    </div>
                                    <p>
                                        <a
                                        onclick="cerrarLogin();"
                                    ...
                                    <button id="accerder-user" class="btn btn-login-desktop btn-color" type="button"><span>{{ trans($theme.'-app.login_register.acceder') }}</span><div style="display: none;" class="loader mini"></div></button>
            ...
            <div class="col-xs-12">
                <div class="header-content">
                    <div class="logo">
                        ...
                        ?>
                        <a title="{{(\Config::get( 'app.name' ))}}" href="/{{$lang}}"><img src="/themes/{{$theme}}/assets/img/logo.png"  alt="{{(\Config::get( 'app.name' ))}}"></a>
                    </div>
