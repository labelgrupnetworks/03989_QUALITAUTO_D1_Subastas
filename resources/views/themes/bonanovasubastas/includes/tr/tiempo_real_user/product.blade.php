<div class="tr_user_product">
    ...
    <div class="product_lot">
        <span id="lote_actual_main" class="">{{ trans($theme.'-app.sheet_tr.lot') }}
            <strong><span id="info_lot_actual">{{ $data['subasta_info']->lote_actual->ref_asigl0 }}</span></strong>
        </span>
    </div>

    <!-- titulo y descripcion lote -->
    <div class="product_desc">
        <span id="actual_titulo" class="actual_titulo">
			<?php echo $data['text_lang'][$data['js_item']['lang_code']]->titulo_hces1 ?>
        </span>
        <div id="actual_descripcion" class="actual_descripcion">
            <?php echo $data['text_lang'][$data['js_item']['lang_code']]->desc_hces1 ?>
        </div>
        <div id="count_down_msg" class="hidden notranslate count_down_msg">
            <span class="final_auction">{{ trans($theme.'-app.sheet_tr.final_auction') }}</span><br>
            <p class="count"></p>
		</div>
