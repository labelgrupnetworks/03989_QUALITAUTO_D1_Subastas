...
#si hay opción a compra pero no hay stock, marcamos compra a falso
...
<div class="ficha-content color-letter">
	...
	@php
		...
		if(!empty($caracteristicas[$idCaracteristicaAutor])){
			$artistName = $caracteristicas[$idCaracteristicaAutor]->value_caracteristicas_hces1;
		}
	@endphp

    <div class="col-sm-5 col-xs-12 content-right-ficha d-flex justify-content-space-between flex-column">

       <div class="d-flex flex-column ficha-info-wrapper">

		<div class="ficha-info-title">
					<h1 class="ficha-artist-name">
						{{ $artistName }}
					</h1>

                    <div class="titleficha secondary-color-text max-line-2">
						{{-- @php
						...
						@endphp
						{{$refLot}} --}}
						<h2>{!!$lote_actual->descweb_hces1 ?? $lote_actual->titulo_hces1!!}</h2>
		...
		<div class="ficha-features">
			<p class="feature-title">{{ trans("$theme-app.shopping_cart.features") }}</p>
			...
			<div class="desc-lot-profile-content mt-3 text-justify">
				<div id="js-read-desc" style="--max-line: 2;" class="desc-lot">
					<div>
						{!!$lote_actual->desc_hces1!!}
					</div>
				</div>
				<p id="js-read-more" class="read-more">
					<small>{{ trans($theme.'-app.blog.more') }}</small>
				</p>
				<p id="js-read-less" class="hidden read-less">
					<small>{{ trans($theme.'-app.blog.less') }}</small>
				</p>
            </div>
...
<div class="container">
	...
	@php
		$artist = App\Models\V5\Web_Artist::where("ID_ARTIST", 2529/*  $caracteristicas[$idCaracteristicaAutor]->idvalue_caracteristicas_hces1 */)->first();
	@endphp

	@if($artist)
	<div class="row mt-3 artist-wrapper">
		<div class="col-xs-12 col-sm-6">
			<img class="img-responsive" src="/img/autores/{{$artist->id_artist}}.jpg" alt="">
		</div>
		<div class="col-xs-12 col-sm-1"></div>
		<div class="col-xs-12 col-sm-5">
			<p class="artist-name max-line-2">{{ $artist->name_artist}}</p>
			<p class="artist-info">{{ $artist->info_artist }}</p>
			<a class="mt-2 custom-button" href="{{route("artist",["name" => \Str::slug($artist->name_artist), "idArtist" => $artist->id_artist] )}}">Saber más</a>
		</div>
	</div>
	@endif
