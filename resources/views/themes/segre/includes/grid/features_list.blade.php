@php
    ...
    $minMaxRanges = \App\Models\V5\FgCaracteristicas_Value::selectRaw(
        'max(cast(value_caracteristicas_value as int)) as max, min(cast(value_caracteristicas_value as int)) as min',
    )
...
@foreach ($features as $idFeature => $feature)
    ...
    @if (!empty($featuresRequest[$idFeature]))
        @if (is_array($featuresRequest[$idFeature]))
            @foreach ($featuresRequest[$idFeature] as $requestValue)
                <input id="feature_{{ $idFeature }}" name="features[{{ $idFeature }}][]" type="hidden"
                    value="{{ $requestValue }}">
            @endforeach
        @else
            <input id="feature_{{ $idFeature }}" name="features[{{ $idFeature }}]" type="hidden"
                value="{{ $featuresRequest[$idFeature] }}">
        @endif
    @endif
    ...
    @if ((empty($featuresRequest[$idFeature]) && !empty($featuresCount[$idFeature])) || in_array($idFeature, $rangesIds))
        <div class="auction__filters-features">
            ...
            <div class="auction__filters-type-list mt-2" id="auction_feature_{{ $idFeature }}" aria-expanded="true">
                ...
                @if (in_array($idFeature, $radioIds))
                    <div class="radio-wrapper">
                        @foreach ($featuresCount[$idFeature] as $featureValue)
                            @php
                                $checked =
                                    !empty($featuresRequest[$idFeature]) &&
                                    $featuresRequest[$idFeature] == $featureValue['id_caracteristicas_value'];
                            @endphp
                ...
                @elseif(in_array($idFeature, $pillsIds))
                    ...
                    <div class="pills-wrapper">
                        @foreach (range(1, $maxValue) as $pillNumber)
                            ...
                            @endphp
                            <label @class(['pill-input', 'pill-input--disabled' => !$inValues])>
