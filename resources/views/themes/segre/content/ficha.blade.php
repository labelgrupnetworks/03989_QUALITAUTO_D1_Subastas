@php
    ...
    $subasta_online =
        $lote_actual->tipo_sub == 'P' || ($lote_actual->tipo_sub == 'O' && $lote_actual->inversa_sub != 'S');
    ...
    $deposito = (new FgDeposito())->isValid(
        $userSession['cod'] ?? null,
        $lote_actual->cod_sub,
...
<div class="ficha-content container">
    ...
    <div class="ficha-grid" data-tipe-sub="{{ $lote_actual->tipo_sub }}">
        ...
        <section class="ficha-bread">
            @include('includes.breadcrumb')
        </section>

        <section class="ficha-title">
            @include('includes.ficha.header_time')
            <h4 class="opacity-50">Lote {{ $refLot }}</h4>
            <h1 class="h4">{!! $lote_actual->descweb_hces1 ?? $lote_actual->titulo_hces1 !!}</h1>
        </section>
    ...
    <section class="ficha-recomendados px-3 px-md-0">
        <div class="lotes_destacados" id="lotes_recomendados-content">
            <h2 class="mas-pujados-title mb-3">{{ trans($theme . '-app.lot.recommended_lots') }}</h2>

            <div class='loader d-none'></div>
            <div id="lotes_recomendados" data-container="lotes_recomendados-content"></div>

			<div class="text-center mt-5">
				<a class="btn btn-outline-lb-primary px-md-5 rounded-0" href="#">Mostrar más</a>
			</div>
        </div>
    </section>
</div>
