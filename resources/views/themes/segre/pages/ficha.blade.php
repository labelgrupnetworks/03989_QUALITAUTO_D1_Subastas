        ...
        @section('header-extend-buttons')
            <div>
                <label class="form-label d-inline-flex align-items-center gap-2 mb-0">
                    ...
                    <select class="form-select" id="currencyExchange">
                        @foreach ($data['divisas'] as $divisa)
                            <option value='{{ $divisa->cod_div }}' @selected($data['js_item']['subasta']['cod_div_cli'] == $divisa->cod_div || ($divisa->cod_div == 'USD' && ($data['js_item']['subasta']['cod_div_cli'] == 'EUR' || $data['js_item']['subasta']['cod_div_cli'] == '')))>
                                {{ $divisa->cod_div }}
                            </option>
                        @endforeach
                    </select>
...
@section('content')
    ...
    <aside>
        @include('includes.ficha.top_filters')
    </aside>
