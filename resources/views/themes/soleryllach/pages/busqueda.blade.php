...
<main class="busqueda-page">
    <div class="container">
        <div class="row">
            <div class="col-xs-12 col-sm-12 resultok">
                <h1 class="titlePage">{{ trans(\Config::get('app.theme') . '-app.subastas.search-results') }} -
                    {{ $data['search'] }}</h1>
            </div>
                        ...
                        <div class="option-historic">
                            @if (Session::has('user'))
                                <div class="switch_content">
                                    <label class="switcher <?= $data['history'] == 'H' ? 'switcher-active' : '' ?>"
                                        for="onlyHistoric"><small></small></label>
                                    <small>{{ trans(\Config::get('app.theme') . '-app.head.search_historic') }}</small>
                                    <input type="checkbox" name="history" class="js-switch submit_on_change"
                                        id="onlyHistoric" style="display:none" value="H"
                                        <?= $data['history'] == 'H' ? 'checked' : '' ?> />
                                </div>
                            @endif
                            <div class="form-group search_content">
                                <input class="form-control input-custom"
                                    ...
                                    type="text" name="texto" id="textSearch" value="{{ $data['search'] }}">
                                <button type="submit" class="btn btn-custom-search"><i class="fa fa-search"></i>
                                    <div class="loader search-loader"
                                        style="display:none;position: absolute;top: -60.50px;right:-1px;width: 29px;height: 29px;">
                                    </div>
                                </button>
    ...
    <div class="container">
        @if (count($data['subastas']) == 0)
            <div class="row">
                <div class="alert alert-danger">{{ trans(\Config::get('app.theme') . '-app.subastas.search-no_results') }}
                </div>
            </div>
        @endif
        <div class="row">
                ...
                <div class="list_lot">
                                        ...
                                        <a href="{{ $url_lotes }}?description={{ $data['search'] }}" target="_blank">
                                            <div class="list_lot_search_content">
                                                <div class="img_search_lot">
                                                    <img src="/img/load/subasta_medium/AUCTION_{{ Config::get('app.emp') }}_{{ $item->cod_sub }}.jpg"
                                                ...
                                                <p class="num_lot">
                                                    ...
                                                    @else
                                                        {{ $item->cuantos }}
                                                        {{ trans(\Config::get('app.theme') . '-app.lot_list.lots') }}
                                                    @endif
                                                </p>
                                            </div>
                                        </a>
                    ...
                    @else
                        @foreach ($data['subastas'] as $key => $item)
							...
							$isITP = Tools::isITPLot($item->sub_hces1, $item->num_hces1);
                            ?>
