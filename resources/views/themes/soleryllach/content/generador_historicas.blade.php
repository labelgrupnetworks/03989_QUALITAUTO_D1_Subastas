{{-- NOMBRE DE LA BLADE: generador_historicas.blade.php --}}
                ...
                <div class="col-xs-12 col-sm-4 col-lg-3">
                    <div class="item_subasta">
                        ...
                        @endif
                            ...
                            <div class="img-lot">
                                    <img
                                        src="https://www.soleryllach.com/img/thumbs/263/SESSION_{{ $subasta->file_code}}.jpg"
                                        alt="{{ $subasta->name }}"
                            ...
                            @if( $subasta->tipo_sub =='W' &&   strtotime($subasta->session_end) > time() && $subasta->subastatr_sub == 'S' )
                                <p class="text-center" style="">
                                    <a class="btn btn-block btn-live" style=""   href="{{ $url_tiempo_real }}"  target="_blank">Puja en vivo</a>
                                </p>
                            @endif
        ...
        @elseif( Session::has('user') )
            ...
            @foreach ($historico as $key => $sub)
                ...
                @foreach ($sub as  $sessions)
                    ...
                    ?>
                    <div class="col-xs-12 col-sm-4 col-lg-3">
					@if ($subasta->upcatalogo == 'S' || $subasta->uppreciorealizado == 'S')
						<div class="item_subasta">
					@else
						<div class="item_subasta hidden">
					@endif
                            <div class="date-sub-content">
                            @if($subasta->session_start)
                            <?php
                            ...
                            <a title="{{ $subasta->name }}" >
								...
								@if($subasta->upcatalogo == 'S')
                                    <p class="text-center">
										@php // URL MANUAL DE EXPORTACIÓN DE PDF CAT
											$url = "files/" . Config::get('app.emp') . '_' . $subasta->cod_sub . '_' . $subasta->reference . '_' . 'cat' . '_' . \App::getLocale() . '.pdf';
										@endphp
                                        <a class="btn btn-subasta" title="{{ trans(\Config::get('app.theme').'-app.grid.pdf_catalog') }}" target="_blank" href="https://www.soleryllach.com{{'/' . $url . '?a=' . rand()}}">{{ trans(\Config::get('app.theme').'-app.subastas.pdf_catalog') }}</a> <br>
                                    </p>
                                @endif

                                @if( $subasta->uppreciorealizado == 'S')
                                    <p class="text-center">
										@php // URL MANUAL DE EXPORTACIÓN DE PDF PRE
											$url = "files/" . Config::get('app.emp') . '_' . $subasta->cod_sub . '_' . $subasta->reference . '_' . 'pre' . '_' . \App::getLocale() . '.pdf';
										@endphp
                                        <a class="btn btn-subasta" title="{{ trans(\Config::get('app.theme').'-app.grid.pdf_adj') }}" target="_blank" href="https://www.soleryllach.com{{'/' . $url . '?a=' . rand()}}">{{ trans(\Config::get('app.theme').'-app.subastas.pdf_adj') }}</a> <br>
                                    </p>
                                @endif
