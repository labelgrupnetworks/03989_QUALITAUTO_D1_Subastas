...
<div class="text-center mt-2 mb-2">
	<a href="{{ trans($theme.'-app.home.see_catalog_url') }}" class="button-principal" target="_blank">{{ trans($theme.'-app.home.see_catalog') }}</a>
</div>
...
<div class="container-fluid inner">
    ...
    <div class="row pd-top-40">
        <div class="col-xs-12 col-lg-3 col-md-4 col-sm-4">
            <div id="home-proxima-subasta">
                ...
                <div id="home-proxima-subasta-titulo">
                    <p>
                        <a class="color-letter" href="{{$url_lotes}}" title="{{$subasta_banner[0]->des_sub}}">{{$subasta_banner[0]->des_sub}}</a>
                    </p>
                </div>
                <div id="home-proxima-subasta-imagen">
                    <center>
                        <a class="enlace" href="{{$url_lotes}}" title="{{$subasta_banner[0]->des_sub}}">
                            <img class="img-responsive" src="{{\Tools::url_img_session('subasta_medium',$subasta_banner[0]->cod_sub,$subasta_banner[0]->reference)}}" alt="{{ $subasta_banner[0]->name }}">
                        </a>
                    </center>
                </div>
                @endif
			...
			<div class="row tour-container d-flex align-items-center flex-wrap justify-content-center">
				...
				<div class="text-center tour-video">
					<div class="tour-video-wrapper">
						<iframe src='{{ trans(\Config::get('app.theme').'-app.home.video_matteport_9') }}' frameborder='0' allowfullscreen allow='xr-spatial-tracking'></iframe>
				...
				<div class="text-center tour-video">
					<div class="tour-video-wrapper">
						<iframe src='{{ trans(\Config::get('app.theme').'-app.home.video_matteport_10') }}' frameborder='0' allowfullscreen allow='xr-spatial-tracking'></iframe>
						{{--<iframe src="https://www.youtube.com/embed/M9IHbzEukIE" frameborder="0" allow="accelerometer; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>--}}
					</div>
				</div>
				...
				<div class="tour-access d-flex flex-column align-items-center">
					...
					<div>
					<a href="{{ trans(\Config::get('app.theme').'-app.home.video_matteport_9') }}" class="button-principal" target="_blank">Núñez de Balboa 9</a>
					<a href="{{ trans(\Config::get('app.theme').'-app.home.video_matteport_10') }}" class="button-principal" target="_blank">Núñez de Balboa 10</a>
					</div>
				</div>
        ...
        <?php
			...
			$res = DB::table("WEB_BLOCK")->where("KEY_NAME","lotes_destacados")->where("ENABLED",1)->where("ID_EMP",\Config::get("app.emp"))->first();
            ...
            if ($lotes_aux && count($lotes_aux) > 1) {
                list($lotes1, $lotes2) = array_chunk($lotes_aux, ceil(count($lotes_aux) / 2));
            }
            else {
                $lotes1 = array();
                $lotes2 = array();
        ...
        <div class="col-xs-12 col-md-8">
            ...
            <div class="heading" style="position: relative">
                <div class="line"></div>
                <h3 class="background extra-font-family">{{ trans(\Config::get('app.theme').'-app.lot_list.lotes_destacados') }}</h3>
                <br><br>
            </div>
...
<div class="container container-custom inner">
    <div class="row pd-top-40 pd-bottom-40">
        ...
        <div class="col-md-3 col-xs-12">
            <div class="heading" style="position: relative">
                <div class="line"></div>
                <h3 class="background extra-font-family">{{trans(\Config::get('app.theme').'-app.foot.newsletter_title')}}</h3>
            </div>
        ...
        <div class="col-xs-12 col-md-9 hide">
            <div class="heading" style="position: relative">
                <div class="line"></div>
                <h3 class="background extra-font-family">{{trans(\Config::get('app.theme').'-app.home.news_home')}}</h3>
            </div>
            <div class="content_art_container d-flex flex-wrap">
                <?php
                $slidder_obj = new \App\Models\Banners;
                $key = "recursos_noticias_" . strtoupper(Config::get('app.locale'));
