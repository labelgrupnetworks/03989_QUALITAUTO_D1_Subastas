       ...
       <div class="d-flex  flex-column">
            <div class="ficha-info-title no-padding">
                <div class="titleficha col-xs-12 no-padding  secondary-color-text no-padding">
                  {{$lote_actual->ref_asigl0}}. {!!strip_tags($lote_actual->descweb_hces1,'<br>')!!}
                </div>
...
<div class="container">
    <div class="@if($subasta_online && !$cerrado) col-sm-7 @endif col-xs-12 no-padding ficha-tipo-v">
            ...
            <div class="col-xs-12 no-padding desc-lot-profile-content">
                <br>
                <p><b>{!!strip_tags($lote_actual->descweb_hces1,'<br>')!!}</b></p>
...
<br><br>
    <div class="row">
        <div class="single">
            ...
            <?php
            $res = DB::table("WEB_BLOCK")->where("KEY_NAME", "lotes_recomendados")->where("ENABLED", 1)->where("ID_EMP", \Config::get("app.emp"))->first();
            $res->products = str_replace("[emp]", Config::get('app.emp'), $res->products);
            $res->products = str_replace("[sec_hces1]", $lote_actual->sec_hces1, $res->products);
            $res->products = str_replace("[id_hces1]", $lote_actual->id_hces1, $res->products);
            $res->products = str_replace("[lang]", Config::get("app.lang"), $res->products);

            $lotes = DB::select($res->products);
            ?>
