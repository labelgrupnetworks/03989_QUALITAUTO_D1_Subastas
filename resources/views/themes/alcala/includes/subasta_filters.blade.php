                ...
                <div class="filter-section-checks filters-auction-label" style="margin-top: 2rem;">
                    <ul>
                        <li style="align-items: center;display: flex;">
                            ...
                            <label for="no_award" style="font-size: 15px;font-weight: 500">{{ trans(\Config::get('app.theme').'-app.lot_list.dont_sold_lots') }}</label>
                        </li>
                        <li style="align-items: center;display: flex;">
                            ...
                            <label for="award" style="font-size: 15px;font-weight: 500">{{ trans(\Config::get('app.theme').'-app.lot_list.sold_lots') }}</label>
                        </li>
    ...
    $categories= $subasta->getLots("small",true);
            ?>
				...
				<div id="collapseTwo" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="headingTwo">
					<div class="filters-auction-title d-flex align-items-center justify-content-space-between">
						<label class="filters-auction-label">{{ trans(\Config::get('app.theme').'-app.lot_list.categories') }}</label>
						<span></span><span id="selects" data-active='open' class="filters-auction-plus" role="button">+</span>
					</div>
					<div class="filters-auction-selects">
					 <select class="form-control filters-auction-select" id="category" name="catpers" >
						@foreach($categories as $category)
							<option value="{{$category->lin_ortsec1}}" <?=   $catpers_selected == $category->lin_ortsec1? "selected='selected'" : ""  ?> >{{ $category->des_ortsec0 }} ({{$category->cuantos}})</option>
						@endforeach
					</select>
						<br> @include('includes.select_filters')
