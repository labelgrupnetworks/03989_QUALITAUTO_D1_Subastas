...
<div  class="col-lg-8 col-md-12 info-ficha-buy-info no-padding">
    <div class=" col-xs-12 no-padding info-ficha-buy-info-price d-flex">
            ...
            <div class="pre">
                ...
                <p class="pre-price">{{ \Tools::moneyFormat($lote_actual->imptas_asigl0)}} {{ trans($theme.'-app.subastas.euros') }} </p>
    ...
    <div class="info_single_content">
        ...
        <div class="insert-max-bid"><?=trans($theme.'-app.lot.insert_max_puja_start')?></div>
            <div class="input-group group-pujar-custom d-flex justify-content-space-between">
                    <input id="bid_modal_pujar" placeholder="{{ $data['precio_salida'] }}" class="form-control control-number" value="{{ $data['precio_salida'] }}" type="text">
                    <div class="input-group-btn">
                        <button id="alerta" data-from="modal" type="button" class="ficha-btn-bid button-principal" onclick="javascript:mensaje_bloqueo();">{{ trans($theme.'-app.lot.place_bid') }}</button>
                    </div>
        </div>
        @endif
...
<script>
    ...
    function mensaje_bloqueo() {
        $("#insert_msgweb").html('{{ trans($theme.'-app.msg_error.usuario_pendiente_revision') }}');
