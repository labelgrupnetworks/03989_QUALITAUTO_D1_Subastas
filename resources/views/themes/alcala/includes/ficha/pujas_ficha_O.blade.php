...
<div id="reload_inf_lot" class="col-xs-12 info-ficha-buy-info no-padding">
    ...
    <div class="col-xs-12 no-padding info-ficha-buy-info-price border-top-bottom">
        <div class="pre d-flex mt-2 mb-2 ">
            <div  id="text_actual_no_bid" class="price-title-principal pre col-xs-12 col-sm-6 no-padding <?=  count($lote_actual->pujas) >0? 'hidden':'' ?>"> {{ trans($theme.'-app.lot_list.no_bids') }} </div>
            <div class="col-xs-12 col-sm-6 no-padding">
            @if ($hay_pujas)
                    <p class='explanation_bid t_insert pre-title' >{{ trans($theme.'-app.lot.next_min_bid') }}  </p>
                    <strong><span class="siguiente_puja">{{ trans($theme.'-app.subastas.euros') }} </span></strong>
                @else
