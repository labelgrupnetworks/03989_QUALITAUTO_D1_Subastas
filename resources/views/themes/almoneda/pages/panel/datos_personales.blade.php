                        ...
                        <form method="post" class="frmLogin" id="frmUpdateUserInfoADV" data-toggle="validator">
                            ...
                            <div class="inputs-custom-group d-flex justify-content-space-between flex-wrap">
                                ...
                                @else
                                    <div class="form-group input-group name_client col-xs-12 col-sm-4">
                                        <label class=""
                                            for="nombre">{{ trans(\Config::get('app.theme') . '-app.user_panel.name') }}</label>
                                        <input type="text" class="form-control" name="usuario"
                                            placeholder="{{ trans(\Config::get('app.theme') . '-app.user_panel.name') }}"
                                            required value="<?= $name[1] ?>">
                                    ...
                                    <div class="form-group input-group name_client col-xs-12 col-sm-4">
                                        <label class=""
                                            for="apellido">{{ trans(\Config::get('app.theme') . '-app.login_register.apellidos') }}</label>
                                ...
                                @if ($data['user']->fisjur_cli == 'R' || $data['user']->fisjur_cli == 'F')
                                    <div class="form-group input-group name_client col-xs-12 col-sm-3">
                                        <label class=""
                                            for="nombre">{{ trans(\Config::get('app.theme') . '-app.login_register.representar') }}</label>
                                        <select data-placement="right" class="form-control select2" type="select"
                                            name="representar" id="select__1__representar"
                                            onblur="comprueba_campo(this)">
                                            @if ($data['user']->fisjur_cli == 'F')
                                                <option value="N">
                                                    {{ trans(\Config::get('app.theme') . '-app.login_register.no') }}
                                                </option>
                                                <option value="S">
                                                    {{ trans(\Config::get('app.theme') . '-app.login_register.yes') }}
                                                </option>
                                            @else
                                                <option value="S">
                                                    {{ trans(\Config::get('app.theme') . '-app.login_register.yes') }}
                                    ...
                                    <div class="form-group input-group name_client col-xs-12 col-sm-9">
                                        ...
                                        <input type="text" class="form-control"
                                            placeholder="{{ trans(\Config::get('app.theme') . '-app.login_register.company') }}"
                                            type="text" name="rsoc_cli"
                                            @if ($data['user']->fisjur_cli == 'R') value="{{ $data['user']->nom_cli }}" required @endif>
                                        <input type="hidden" class="form-control" name="title_rsoc_cli"
                                            value="{{ trans(\Config::get('app.theme') . '-app.login_register.company') }}">
                                    </div>
                                @endif
                                ...
                                <div class="form-group form-group-custom col-xs-12 col-sm-3">
                                    ...
                                    <select name="divisa" class="form-control" required>
                                        @foreach ($data['divisa'] as $key => $value)
                                            <option
                                                <?= $data['user']->cod_div_cli == strtoupper($value->cod_div) ? 'selected' : '' ?>
                                ...
                                <div class="form-group col-xs-12 col-md-6">
                                    ...
                                    <p
                                        style="color: green; display: {{ isset($data['cifImages']['dni1']) ? 'block' : 'none' }};">
                                ...
                                <div class="form-group col-xs-12 col-md-6">
                                    ...
                                    <p
                                        style="color: green; display: {{ isset($data['cifImages']['dni2']) ? 'block' : 'none' }};">
                                        {{ trans("$theme-app.user_panel.uploaded_file") }}</p>
                                </div>

                                <div class="form-group input-group col-xs-12 col-sm-3">
                                    <label
                                        for="codigoVia">{{ trans(\Config::get('app.theme') . '-app.login_register.via') }}</label>
                                    <select name="codigoVia" class="form-control">
                                        <option value="">----</option>
                                        @if (!empty($data) && !empty($data['via']))
                                            @foreach ($data['via'] as $via)
                                                <option <?= $via->cod_sg == $data['user']->sg_cli ? 'selected' : '' ?>
