	...
	<div class="container register pb-5">
		<div class="row d-flex align-items-center justify-content-center">
			<div class="col-xs-12 general-container">
				...
				@elseif($isNftPage)
				<a id="logo_link" title="{{(\Config::get( 'app.name' ))}}" href="{{ request('context_url', '') }}">
					<img class="logo-landing" src="/themes/{{\Config::get('app.theme')}}/assets/img/logo_nft.png"  alt="{{(\Config::get( 'app.name' ))}}">
				</a>
				@else
				...
				<div class="create-account-link">
					<a class="" title="{{ trans(\Config::get('app.theme').'-app.login_register.register') }}" href="{{ \Routing::slug('register') }}?back={{$back}}&context_url={{ request('context_url', '') }}">{{ trans(\Config::get('app.theme').'-app.login_register.register') }}</a>
				</div>
