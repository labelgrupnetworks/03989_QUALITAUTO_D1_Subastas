...
<div class="ficha-info-item-for-pay col-xs-12 no-padding">
    <div class="info_single_content">
        ...
        @if( $cerrado_N && $fact_N &&  $start_orders  &&   !$end_orders && !$subasta_abierta_P)
            <div ><p class="pt-1 pre-title  pb-1"><?=trans($theme.'-app.lot.deja_puja')?> </p></div>
            <div class="input-group group-pujar-custom d-flex justify-content-space-between">
				...
				<div class="col-xs-5 no-padding text-right">
					<button id="pujar_ordenes_w" data-from="modal" type="button" class="ficha-btn-bid button-principal fondo_azul" ref="{{ $data['subasta_info']->lote_actual->ref_asigl0 }}" codsub="{{ $data['subasta_info']->lote_actual->cod_sub }}">{{ trans($theme.'-app.lot.pujar') }}</button>
				</div>
			</div>

			@if( $lote_actual->ordentel_sub !=0 && $lote_actual->ordentel_sub <=  $lote_actual->impsalhces_asigl0)
				<div class="input-group-btn ">
					<button id="pujar_orden_telefonica_duran" data-from="modal" type="button" class="ficha-btn-telephone-bid  button-principal hidden" ref="{{ $data['subasta_info']->lote_actual->ref_asigl0 }}" codsub="{{ $data['subasta_info']->lote_actual->cod_sub }}">{{ trans($theme.'-app.lot.puja_telefonica') }}</button>
					<a href="javascript:;" data-toggle="modal" data-target="#modalAjax" class="info-ficha-lot pt-1 c_bordered" data-ref="{{ Routing::translateSeo('pagina')."info-pujas-presencial"  }}?modal=1" data-title="{{ trans($theme.'-app.lot.title_info_pujas') }}"><i class="fas fa-info-circle"></i></a>
				</div>
			@endif
