                    ...
                    @if (isset($lote_actual->impres_asigl0) && $lote_actual->impres_asigl0 > 0 && Session::has('user'))
                        <div class="pre flex-column justify-content-space-between d-flex">
                            <div class="pre_min">
                                    ...
                                    <p class='pre-title'> {{ trans($theme.'-app.subastas.price_minim') }}: </p>
                                    <strong>
                                    <span class="precio_minimo_alcanzado mine hidden">{{ trans($theme.'-app.subastas.reached') }}</span>
                                    <span class="precio_minimo_no_alcanzado other hidden">{{ trans($theme.'-app.subastas.no_reached') }}</span>
                                    </strong>
		...
		<div class="col-lg-12 no-padding">
			...
			{{trans($theme.'-app.lot.max_puja')}}
				...
				<?php
					...
					//<input style="float: right;margin-top: -10px;" class="btn btn-danger delete_order" type="button" ref="{{$data['subasta_info']->lote_actual->ref_asigl0}}" sub="{{$data['subasta_info']->lote_actual->cod_sub}}" value="{{ trans($theme.'-app.user_panel.delete_orden') }}">
				?>
			</div>
		</div>
	</div>
        ...
        <div class="insert-bid-input col-lg-12 d-flex justify-content-center flex-column no-padding">
			...
			@if ($start_session)
				<div class="input-group d-block group-pujar-custom mr-1">
					<div>
						<div class="insert-bid insert-max-bid mb-1">{{ trans($theme.'-app.lot.insert_max_puja') }} 	<a href="javascript:;" data-toggle="modal" data-target="#modalAjax" class="info-ficha-lot pt-1 c_bordered" data-ref="{{ Routing::translateSeo('pagina')."info-pujas-online"  }}?modal=1" data-title="{{ trans($theme.'-app.lot.title_info_pujas') }}"><i class="fas fa-info-circle"></i></a>
						</div>
					</div>
					<div class="input-group-btn boton-pujas mt-1 boton-pujar-online mb-2">
						<button type="button" data-from="modal" value="{{$data['precio_salida']}}" class="lot-action_pujar_on_line ficha-btn-bid ficha-btn-bid-height button-principal js-lot-action_pujar_escalado <?= Session::has('user')?'add_favs':''; ?>" type="button" ref="{{ $lote_actual->ref_asigl0 }}" ref="{{ $lote_actual->ref_asigl0 }}" codsub="{{ $lote_actual->cod_sub }}" >{{ trans($theme.'-app.lot.pujar') }}
					...
					<div class="col-xs-5 no-padding text-right">
						<button type="button" data-from="modal" class="lot-action_pujar_on_line  ficha-btn-bid-height button-principal <?= Session::has('user')?'add_favs':''; ?>" type="button" ref="{{ $lote_actual->ref_asigl0 }}" ref="{{ $lote_actual->ref_asigl0 }}" codsub="{{ $lote_actual->cod_sub }}" >{{ trans($theme.'-app.lot.autopuja') }}</button>
					</div>
