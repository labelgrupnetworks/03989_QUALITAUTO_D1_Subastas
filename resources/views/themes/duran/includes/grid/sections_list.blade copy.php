    ...
    <div class="input-category d-flex align-items-center hidden">
        <div class="radio">
            ...
            <label for="all_sections" class="ratio-label">
                {{trans($theme.'-app.lot_list.all_subcategories')}} ({{$numCategoryLots }})
            </label>
        </div>
    </div>
     ...
     <?php $numSectionLots = Tools::showNumLots($numActiveFilters, $filters, "section", $sec["cod_sec"]); ?>
        @if($numSectionLots > 0)
            <div class="input-category d-flex align-items-center">
                <div class="radio">
                    <input type="radio" name="section" id="section_{{$sec['cod_sec']}}" value="{{$sec['cod_sec']}}" class="filter_lot_list" <?= ($sec["cod_sec"] ==  $filters["section"])?  'checked="checked"' : '' ?> />
