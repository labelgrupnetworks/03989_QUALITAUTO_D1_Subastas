	...
	<div class="filters-auction-content d-flex  flex-column bloque-fijo-grid">
		...
		<div class="form-group">
			...
			<div class=" search-control-input">
				<input  name="description" id="description_filter_grid" class="form-control input-sm search-input search-input_js" type="text" placeholder="{{ trans($theme.'-app.lot_list.search') }}" value="{{request('description')}}" >
			...
			@if(!empty($auction) && ( $auction->subc_sub == 'H' || ($auction->tipo_sub == 'W' && strtotime($auction->end) < time()  ) ) )
				...
				<div class="filters-auction-texts ">
					<span class="titulo-filtro">{{ trans($theme.'-app.lot_list.lots_status') }}</span>
					...
					<div>
						...
						<label for="award" class="radio-label">
							{{trans($theme.'-app.lot_list.ver_todos')}}
						</label>
					</div>
			...
			{{-- ocultanmos el filtro de referencia por que de momento no lo queremos
				<div class="filters-auction-texts">
					...
					<label class="filters-auction-label" for="input_reference">{{ trans($theme.'-app.lot_list.reference') }} (32)</label>
