...
return [
	...
	'loteFavoritoProximo' => 0, //Enviar mail cuando se acerca un lote favorito
	'maxium_time_increment' => 0, //Tiempo maximo de extension en un lote, marcado en segundos
	'max_orders_and_bids_ries_cli' => false, //Limite de puja con la suma de las puja máximas (puja u orden)
	'max_orders_ries_cli' => false, //Bloquear orden si la suma de estas supera el ries_cli del cliente
	'not_force_correct_price_tiempo_real' => true, //En tiempo real no fuerzar el escalado correcto en la primer puja
	'notice_over_bid' => false, //Avisa al usurio con un mensaje en la web cuando hace una orden y esta orden no supera una orden anterior
	'number_bids_lotlist' => false, //Realizar consultas para mostrar pujas y licitadores en grid y panel de usuario.
	'order_by_filter' => 'precio_salida', //Ordenar los lotes por (estimacion, precio_salida) - @todo - grid antiguo.
	'paginacion_grid_lotes' => 1, //si tiene valor 0 funcionara con scroll, si tiene valor 1 funcionará con paginación
	'pujas_enfirme' => false, // Habilitar pujas en firme
	'pujas_maximas_mostradas' => -1, //Establece las pujas máximas que se mostraran por lote (-1 Muestra todas las pujas de los lotes) @todo - todos tienen -1
	'restrictVisibility' => false, // Limita la visibilidad de los lotes y subastas mediante la tabla FGVISIBILIDAD
	'search_engine' => false, //hace busquedas por indice de texto con campo search_hces1
	'search_lots_cerrados' => false, //Buscar por lotes abiertos y cerrados. @todo - No he encontrado referencias en el códgio.
	'search_multiple_words' => true, //Permite buscar las palabras por separado en los buscadores de la web y el de lotes
	'send_email_cancel_bid' => 2, //Envia email de se ha cancelado una puja, con valor 1 envia al usuario que se le ha cancelado la puja, con valor 2 envia tambien al resto de usuarios
	'send_email_lot_increment_bid_to_all_users_with_deposit' => false, //Al realizarse una puja, enviar email a todos los usuarios que tengan deposito por ese lote.
	'send_valid_deposit_notification' => false //Enviar notificación al crear o editar un deposito a valido
