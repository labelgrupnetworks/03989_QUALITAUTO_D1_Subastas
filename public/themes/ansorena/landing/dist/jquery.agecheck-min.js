...
! function (e) {
  e.ageCheck = function (r) {
    const t = e.extend({
      ...
      redirectOnFail: "",
      title: "",
      copy: "Debes ser mayor de edad para entrar en este sitio web.",
      success: null,
      successMsg: {
        header: "¡Correcto!",
        body: "Abriendo la página..."
      },
      underAgeMsg: "Lo sentimos, no debes ser mayor de edad para acceder a esta página.",
      underAge: null,
      errorMsg: {
        invalidDay: "Día inválido o vacío",
        invalidYear: "Año inválido o vacío"
      },
    ...
    const o = {
      ...
      buildHtml() {
        ...
        let i = "";
        i += '<div class="ac-overlay"></div>', i += '<div class="ac-container">', i += `<h2>${t.title}</h2>`, i += `<p>${r.replace("[18]",`<strong>${t.minAge}</strong>`)}` + "</p>", i += '<div class="errors"></div>', i += '<div class="fields"><div class="firstrow">', i += '<div class="co-ag"><label>Día</label><input class="day" maxlength="2" placeholder="DD" /></div>',  i += '<div class="co-ag"><label>Mes</label><select class="month">';
        for (let e = 0; e < a.length; e++) i += `<option value="${e}">${a[e]}</option>`;
        i += "</select></div>", i += '<div class="co-ag"><label>Año</label><input class="year" maxlength="4" placeholder="AAAA"/></div>', i += '</div>', i += "<button>Enviar</button></div></div>", e("body").append(i), e(".ac-overlay").animate({
          opacity: .8
        }, 500, () => {
          o.reCenter(e(".ac-container")), e(".ac-container").css({
            opacity: 1
          })
        ...
        e(".ac-container").html(r), setTimeout(() => {
          e(".ac-container").animate({
            ...
            e(".ac-overlay").animate({
              ...
              "" !== t.redirectTo ? window.location.replace(t.redirectTo) : (e(".ac-overlay, .ac-container").remove(), t.success && t.success())
            })
          })
        }, 2e3)
      },
      handleUnderAge() {
        ...
        }, 2e3), t.underAge && t.underAge()
      }
    };
    if (o.getStorage()) return !1;
    o.buildHtml(), e(".ac-container button").on("click", () => {
      o.setValues(), !0 === o.validate() && (o.setAge(), o.age >= t.minAge ? (o.setStorage("ageVerified", "true", t.storageExpires) || console.log(t.storage + " not supported by your browser"), o.handleSuccess()) : o.handleUnderAge())
    }), e(window).resize(() => {
      o.reCenter(e(".ac-container")), setTimeout(() => {
        o.reCenter(e(".ac-container"))
      }, 500)
    })
  }
}(jQuery);
