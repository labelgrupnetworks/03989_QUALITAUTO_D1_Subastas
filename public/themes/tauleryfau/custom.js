...
$(function () {
	...
	$("#encap-service").submit(function (event) {


		event.preventDefault();
		formData = new FormData(this);
		$('.labels-form').addClass('d-none')
		$.ajax({
			type: "POST",
			url: "valoracion-encapsulacion",
			data: formData,
			...
			success: function (result) {
				if (result.status === 'success') {
					$("#insert_msgweb").html('');
					$("#insert_msgweb").html(result.msg);
					$.magnificPopup.open({ items: { src: '#modalMensajeWeb' }, type: 'inline' }, 0);
				} else {
					$('.label-danger').removeClass('d-none')
				}
	...
	$("#form-photographic-adv").submit(function (event) {
